...
class Diglin_Username_Model_Entity_Customer extends Mage_Customer_Model_Resource_Customer {
    ...
    protected function _beforeSave(Varien_Object $customer)
    {
        ...
        if (Mage::getStoreConfigFlag('username/general/enabled')) {
            ...
            $result = $model->customerUsernameExists($customer->getData('username'), $websiteId);
            if ($result && $result->getId() != $customer->getId() && $customer->getData('username')) {
                Mage::getSingleton('customer/session')->addError(Mage::helper('username')->__("Username already exists"));
                return false;
            }
        }
    ...
    public function loadByUsername(Mage_Customer_Model_Customer $customer, $username)
    {
        ...
        $select = $this->_getReadAdapter()->select()
            ...
            ->where("ea.attribute_code='username' AND (cev.value = '{$username}' or cev.value like '%,{$username},%' or cev.value like '{$username},%' or cev.value like '%,{$username}')");
//echo $select; die;
        if ($customer->getSharingConfig()->isWebsiteScope()) {
        ...
        if ($id = $this->_getReadAdapter()->fetchOne($select, 'entity_id')) {
            ...
            $customer->setData(array('found'=>0));
        }
        return $this;
    }
