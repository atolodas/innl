...
class  Mage_Adminhtml_Score_Oggetto_ReviewController extends Mage_Adminhtml_Controller_Action
{
    /**
    ...
    public function indexAction()
    {
        ...
        $this->loadLayout();
        $this->_setActiveMenu('score/review');

        $this->_addContent($this->getLayout()->createBlock('adminhtml/review_oggetto'));
    ...
    public function newAction()
    {
        ...
        $this->_addContent($this->getLayout()->createBlock('adminhtml/review_oggetto_grid'));
    ...
    public function saveAction()
    {
        if (($data = $this->getRequest()->getPost()) && ($reviewId = $this->getRequest()->getParam('id'))) {
            ...
            if (! $review->getId()) {
                $session->addError(Mage::helper('score')->__('The review was removed by another user or does not exist.'));
            } else {
                try {
                    ...
                    $session->addSuccess(Mage::helper('score')->__('The review has been saved.'));
                } catch (Mage_Core_Exception $e) {
                    $session->addError($e->getMessage());
                } catch (Exception $e){
                    $session->addException($e, Mage::helper('score')->__('An error occurred while saving this review.'));
                }
            }
    ...
    public function deleteAction()
    {
        ...
        try {
            ...
            $session->addSuccess(Mage::helper('score')->__('The review has been deleted'));
            ...
            $session->addException($e, Mage::helper('score')->__('An error occurred while deleting this review.'));
        }
    ...
    public function massUpdateStatusAction()
    {
        ...
        if(!is_array($reviewsIds)) {
            ...
            /* @var $session Shaurmalab_Score_Adminhtml_Model_Session */
            try {
    ...
    public function oggettoGridAction()
    {
        $this->getResponse()->setBody($this->getLayout()->createBlock('adminhtml/review_oggetto_grid')->toHtml());
    }

    public function reviewGridAction()
    {
        $this->getResponse()->setBody($this->getLayout()->createBlock('adminhtml/review_oggettogrid')->toHtml());
    }

    public function jsonOggettoInfoAction()
    {
        $response = new Varien_Object();
        $id = $this->getRequest()->getParam('id');
        if( intval($id) > 0 ) {
            $entity = Mage::getModel('score/oggetto')
                ->load($id);

            $response->setId($id);
            $response->addData($entity->getData());
            $response->setError(0);
        } else {
            $response->setError(1);
            $response->setMessage(Mage::helper('score')->__('Unable to get the entity ID.'));
        }
            ...
            try {
                ...
                foreach ($arrRatingId as $ratingId=>$optionId) {
                    Mage::getModel('rating/rating')
                       ->setRatingId($ratingId)
                       ->setReviewId($review->getId())
                       ->addOptionVote($optionId, $entityId);
                }
    ...
    protected function _isAllowed()
    {
        switch ($this->getRequest()->getActionName()) {
            case 'pending':
                return Mage::getSingleton('admin/session')->isAllowed('score/reviews_ratings/reviews/pending');
                break;
            default:
                return Mage::getSingleton('admin/session')->isAllowed('score/reviews_ratings/reviews/all');
                break;
        }
    }
}
