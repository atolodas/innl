...
class Mage_Catalog_Helper_Image extends Mage_Core_Helper_Abstract
{
    ...
    public function resizeImg($dir,$fileName, $width, $height = '')
    {
        $folderURL = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_SKIN).'/images/';
        $imageURL = $folderURL . $fileName;

        $basePath = $dir.DS.$fileName;
        if(!$height && file_exists($basePath) && is_file($basePath)) {
            $dimm = getimagesize($basePath);
            $height = $dimm[1] * $width / $dimm[0];
        }
        $dir = Mage::getBaseDir(Mage_Core_Model_Store::URL_TYPE_MEDIA) . DS . "resized".DS.$width.'x'.$height;
        if(!isset($dir) || !is_dir($dir)) mkdir($dir);

        $newPath = Mage::getBaseDir(Mage_Core_Model_Store::URL_TYPE_MEDIA) . DS . "resized" . DS . $width.'x'.$height . DS . $fileName;
        //if width empty then return original size image's URL
        if(is_file($newPath)) {
            $imageURL = str_replace(Mage::getBaseDir(Mage_Core_Model_Store::URL_TYPE_MEDIA), Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA), $newPath);
            return $imageURL;
        }
        if ($width != '') {
            ...
            $resizedURL = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . "resized" . DS . $width.'x'.$height . DS . $fileName;
        } else {
            $resizedURL = $imageURL;
        }
        return $resizedURL;
