...
class Mage_Core_Block_Html_Date extends Mage_Core_Block_Template
{
    ...
    protected function _toHtml()
    {
        ...
        $html .= 'value="' . $this->escapeHtml($this->getValue()) . '" class="' . $this->getClass() . '" ' . $this->getExtraParams() . ' 
                    onclick="jQuery(\'#' . $this->getId() . '_trig\').click()" /> ';

        $html .= '<i alt="' . $this->helper('core')->__('Select Date') . '" class="fa fa-calendar pointer f25" ';
        $html .= 'title="' . $this->helper('core')->__('Select Date') . '" id="' . $this->getId() . '_trig" ></i>';
