...
class Mage_Core_Model_Config extends Mage_Core_Model_Config_Base
{
    ...
    public function handleModulesPerWebsite($moduleFiles)
    {
        ...
        $store_code = Mage::registry('scode');
        $xml = simplexml_load_file(Mage::getBaseDir().'/modules.xml');
        $modules = (array)$xml;

        $arr1 = (array)$modules['DisablePerStore'];
        $arr2 = (array)$modules['EnablePerStore'];

        $completeDisable = array_keys((array)$modules['DisableCompletely']);
        if(isset($arr1[$store_code])) {
            $disableInStore = array_keys((array)$arr1[$store_code]);
        } else { $disableInStore = array(); }

        if(isset($arr2[$store_code])) {
            $enableInStore = array_keys((array)$arr2[$store_code]);
        } else { $enableInStore = array(); }
	    ...
	    foreach($moduleFilesX as $k => $moduleFile) {
            ...
            //Turn off all modules that are listed in modules.xml file
            if(
            ((in_array($moduleName, $completeDisable) || in_array($moduleName, $disableInStore)) && !in_array($moduleName,$enableInStore))
            ) {
                $disabled_modules[] = $moduleName;
                unset($moduleFiles[$k]);
            }

        }
        if(Mage::registry('disabled_modules'))  Mage::unregister('disabled_modules');
        Mage::register('disabled_modules',serialize($disabled_modules));
