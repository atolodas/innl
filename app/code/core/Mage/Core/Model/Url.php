...
class Mage_Core_Model_Url extends Varien_Object
{
    ...
    public function getUrl($routePath = null, $routeParams = null)
    {
		...
		if(!substr_count($url, '/admin')) {
    // TODO: sometimes frontend urs can have "/admin" in url. better to base it on ControllerName
                $url = str_replace(Mage::getBaseUrl(),Mage::getBaseUrl().Mage::registry('slang').'/', $url);
        }

   if(substr_count($url, '/ig_404tosearch/index/noRoute/')) {
       $url_parts = parse_url($url);
       $r_parts = explode('?',$_SERVER['REQUEST_URI']);
       $url = rtrim(Mage::getBaseUrl(),'/').$r_parts[0].((isset($url_parts['fragment']))?'?'.$url_parts['query']:'').((isset($url_parts['fragment']))?'#'.$url_parts['fragment']:'');
		}
