...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
    ...
    public function indexAction()
    {
        ...
        $this->getLayout()->getBlock('head')->setTitle(Mage::helper('core')->__('My Account'));
        $this->renderLayout();
    }
    ...
    public function loginPostAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!empty($login['username'])) {
                ...
                $session = Mage::getSingleton('core/session');
                $session->addError($this->__('Username is required.'));
            }
        }
    ...
    public function createPostAction()
    {
        ...
        if($back = $this->getRequest()->getPost('back')) {
          $errUrl = $back;
        } else {
          $errUrl = $this->_getUrl('*/*/create', array('_secure' => true));
        }
    ...
    protected function _getCustomerErrors($customer,$form_code)
    {
        ...
        $customerForm = $this->_getCustomerForm($customer,$form_code);
        $customerData =  $request->getPost(); // //  TODO: Fix it to allow saving any attributes for customer, but $customerForm->extractData($request);
        $customerErrors = $customerForm->validateData($customerData);
        if (isset($customerData['username'])) {
            $model = Mage::getModel('customer/customer');
             $websiteId = Mage::app()->getWebsite()->getId();
            $result = $model->customerUsernameExists($customerData['username'], $websiteId);
            if ($result && $customerData['username']) {
                $message = Mage::helper('username')->__('Username already exists');
               if ($customerErrors !== true) {
                $customerErrors = array_merge($customerErrors, array($message));
            } else { $customerErrors = array($message); }
            }
        }
        if ($customerErrors !== true) {
     ...
     */
    protected function _getCustomerForm($customer,$form_code = 'customer_account_create')
    {
        /* @var $customerForm Mage_Customer_Model_Form */
        $customerForm = $this->_getModel('customer/form');
        $customerForm->setFormCode($form_code);
    ...
    public function confirmAction()
    {
        ...
        try {
            ...
            if ($customer->getConfirmation()) {
                ...
                // activate customer
                try {
                    $customer->setConfirmation(null)->setIsActive(1);
                    $customer->save();
                }
                catch (Exception $e) {
    ...
    public function forgotPasswordPostAction()
    {
        ...
        if ($email) {
            ...
            Mage::getSingleton('core/session')->addSuccess($this->__('If there is an account associated with %s you will receive an email with a link to reset your password.', Mage::helper('customer')->escapeHtml($email)));
            $this->_redirect('*/*/');
            return;
        } else {
    ...
    public function resetPasswordAction()
    {
        ...
        try {
            ...
            Mage::getSingleton('core/session')->addError( $this->_getHelper('customer')->__('Your password reset link has expired.'));
            $this->_redirect('*/*/forgotpassword');
        }
    }
    ...
    public function resetPasswordPostAction()
    {
        ...
        try {
            ...
            $customer->save();
            Mage::getSingleton('core/session')->addSuccess( $this->_getHelper('customer')->__('Your password has been updated.'));
            $this->_redirect('*/*/login');
        } catch (Exception $exception) {
            Mage::getSingleton('core/session')->addException($exception, $this->__('Cannot save a new password.'));
            $this->_redirect('*/*/resetpassword', array(
                'id' => $customerId,
                'token' => $resetPasswordLinkToken
            ));
    ...
    public function publicEditPostAction()
    {
      $this->editPostAction('public_account');
    }
    /**
     * Change customer password action
     */
    public function editPostAction($form = 'customer_account_create')
    {
        ...
        $errors = array();
          ...
          if(is_object(Mage::getModel('magemlm/customer'))) {
        ...
        if(isset($_FILES['magemlm_customer_picture']['name']) and (file_exists($_FILES['magemlm_customer_picture']['tmp_name']))) {
            try {
                ...
                $this->_getSession()->setCustomerFormData($this->getRequest()->getPost());
                  Mage::getSingleton('core/session')->addException($e, $this->__('Cannot save the customer.'));
            }
        }
