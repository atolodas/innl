...
/**
 * Scoretags customer collection
 *
 * @category    Mage
 * @package     Mage_Scoretag
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Mage_Scoretag_Model_Resource_Customer_Collection extends Mage_Customer_Model_Resource_Customer_Collection
{
    /**
     ...
     * @var string
     */
    protected $_countAttribute           = 'tr.scoretag_id';
     ...
     * Prepare select
     *
     * @return Mage_Scoretag_Model_Resource_Customer_Collection
     */
    ...
    {
        $this->getSelect()
            ->where('tr.scoretag_id = ?', $scoretagId);
        return $this;
    }
    ...
    /**
     * Adds desc order by scoretag relation id
     *
     * @return Mage_Scoretag_Model_Resource_Customer_Collection
     */
    ...
    {
        $this->getSelect()
            ->order('tr.scoretag_relation_id desc');
        return $this;
    }
     ...
     * @return Mage_Scoretag_Model_Resource_Customer_Collection
     */
    public function addGroupByCustomerOggetto()
    {
    ...
    protected function _joinFields()
    {
        ...
        $this->getSelect()
        ->join(
            array('tr' => $scoretagRelationTable),
            'tr.customer_id = e.entity_id',
            array('scoretag_relation_id', 'oggetto_id', 'active', 'added_in' => 'store_id')
        )
        ->join(array('t' => $scoretagTable), 't.scoretag_id = tr.scoretag_id', array('*'));
    }
    ...
    /**
     * Adds Oggetto names to item
     *
     * @return Mage_Scoretag_Model_Resource_Customer_Collection
     */
    public function addOggettoName()
    {
        $oggettosId   = array();
        $oggettosData = array();

        foreach ($this->getItems() as $item) {
            $oggettosId[] = $item->getOggettoId();
        }

        $oggettosId = array_unique($oggettosId);

        /* small fix */
        if ( sizeof($oggettosId) == 0 ) {
            return;
        }
        ...
        $collection = Mage::getModel('score/oggetto')->getCollection()
            ...
            ->addIdFilter($oggettosId);

        $collection->load();

        foreach ($collection->getItems() as $item) {
            $oggettosData[$item->getId()] = $item->getName();
            $oggettosSku[$item->getId()] = $item->getSku();
        }

        foreach ($this->getItems() as $item) {
            $item->setOggetto($oggettosData[$item->getOggettoId()]);
            $item->setOggettoSku($oggettosSku[$item->getOggettoId()]);
        }
        return $this;
    }

    /**
     * Adds Oggetto names to select
     *
     * @return Mage_Scoretag_Model_Resource_Customer_Collection
     */
    public function addOggettoToSelect()
    {
        $resource = Mage::getModel('score/oggetto')->getResource();

        // add oggetto attributes to select
        foreach (array('name' => 'value') as $field => $fieldName) {
            $attr = $resource->getAttribute($field);
            $this->_select->joinLeft(
                array($field => $attr->getBackend()->getTable()),
                'tr.oggetto_id = ' . $field . '.entity_id AND ' . $field . '.attribute_id = ' . $attr->getId(),
                array('oggetto_' . $field => $fieldName)
            );
        }

        // add oggetto fields
        $this->_select->joinLeft(
            array('p' => $this->getTable('score/oggetto')),
            'tr.oggetto_id = p.entity_id',
            array('oggetto_sku' => 'sku')
        );
