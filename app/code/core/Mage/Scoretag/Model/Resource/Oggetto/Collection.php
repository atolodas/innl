<?php
/**
 * Magento
 *
 * NOTICE OF LICENSE
 *
 * This source file is subject to the Open Software License (OSL 3.0)
 * that is bundled with this package in the file LICENSE.txt.
 * It is also available through the world-wide-web at this URL:
 * http://opensource.org/licenses/osl-3.0.php
 * If you did not receive a copy of the license and are unable to
 * obtain it through the world-wide-web, please send an email
 * to license@magentocommerce.com so we can send you a copy immediately.
 *
 * DISCLAIMER
 *
 * Do not edit or add to this file if you wish to upgrade Magento to newer
 * versions in the future. If you wish to customize Magento for your
 * needs please refer to http://www.magentocommerce.com for more information.
 *
 * @category    Mage
 * @package     Mage_Scoretag
 * @copyright   Copyright (c) 2013 Magento Inc. (http://www.magentocommerce.com)
 * @license     http://opensource.org/licenses/osl-3.0.php  Open Software License (OSL 3.0)
 */


/**
 * Scoretagged Oggetto(s) Collection
 *
 * @category    Mage
 * @package     Mage_Scoretag
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Mage_Scoretag_Model_Resource_Oggetto_Collection extends Shaurmalab_Score_Model_Resource_Oggetto_Collection
{
    /**
     * Customer Id Filter
     *
     * @var int
     */
    protected $_customerFilterId;

    /**
     * Scoretag Id Filter
     *
     * @var int
     */
    protected $_scoretagIdFilter;

    /**
     * Join Flags
     *
     * @var array
     */
    protected $_joinFlags            = array();

    /**
     * Initialize collection select
     *
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    protected function _initSelect()
    {
        parent::_initSelect();

        $this->_joinFields();
        $this->getSelect()->group('e.entity_id');

        /*
         * Allow analytic function usage
         */
        $this->_useAnalyticFunction = true;

        return $this;
    }

    /**
     * Set flag about joined table.
     * setFlag method must be used in future.
     *
     * @deprecated after 1.3.2.3
     *
     * @param string $table
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    public function setJoinFlag($table)
    {
        $this->setFlag($table, true);
        return $this;
    }

    /**
     * Get flag's status about joined table.
     * getFlag method must be used in future.
     *
     * @deprecated after 1.3.2.3
     *
     * @param string $table
     * @return bool
     */
    public function getJoinFlag($table)
    {
        return $this->getFlag($table);
    }

    /**
     * Unset value of join flag.
     * Set false (bool) value to flag instead in future.
     *
     * @deprecated after 1.3.2.3
     *
     * @param string $table
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    public function unsetJoinFlag($table = null)
    {
        $this->setFlag($table, false);
        return $this;
    }

    /**
     * Add scoretag visibility on stores
     *
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    public function addStoresVisibility()
    {
        $this->setFlag('add_stores_after', true);
        return $this;
    }

    /**
     * Add scoretag visibility on stores process
     *
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    protected function _addStoresVisibility()
    {
        $scoretagIds = array();
        foreach ($this as $item) {
            $scoretagIds[] = $item->getScoretagId();
        }

        $scoretagsStores = array();
        if (sizeof($scoretagIds) > 0) {
            $select = $this->getConnection()->select()
                ->from($this->getTable('scoretag/relation'), array('store_id', 'scoretag_id'))
                ->where('scoretag_id IN(?)', $scoretagIds);
            $scoretagsRaw = $this->getConnection()->fetchAll($select);
            foreach ($scoretagsRaw as $scoretag) {
                if (!isset($scoretagsStores[$scoretag['scoretag_id']])) {
                    $scoretagsStores[$scoretag['scoretag_id']] = array();
                }

                $scoretagsStores[$scoretag['scoretag_id']][] = $scoretag['store_id'];
            }
        }

        foreach ($this as $item) {
            if (isset($scoretagsStores[$item->getScoretagId()])) {
                $item->setStores($scoretagsStores[$item->getScoretagId()]);
            } else {
                $item->setStores(array());
            }
        }

        return $this;
    }

    /**
     * Add group by scoretag
     *
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    public function addGroupByScoretag()
    {
        $this->getSelect()->group('relation.scoretag_relation_id');
        return $this;
    }

    /**
     * Add Store ID filter
     *
     * @param int|array $store
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    public function addStoreFilter($store = null)
    {
        if (!is_null($store)) {
            $this->getSelect()->where('relation.store_id IN (?)', $store);
        }
        return $this;
    }

    /**
     * Set Customer filter
     * If incoming parameter is array and has element with key 'null'
     * then condition with IS NULL or IS NOT NULL will be added.
     * Otherwise condition with IN() will be added
     *
     * @param int|array $customerId
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    public function addCustomerFilter($customerId)
    {
        if (is_array($customerId) && isset($customerId['null'])) {
            $condition = ($customerId['null']) ? 'IS NULL' : 'IS NOT NULL';
            $this->getSelect()->where('relation.customer_id ' . $condition);
            return $this;
        }
        $this->getSelect()->where('relation.customer_id IN(?)', $customerId);
        $this->_customerFilterId = $customerId;
        return $this;
    }

    /**
     * Set scoretag filter
     *
     * @param int $scoretagId
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    public function addScoretagFilter($scoretagId)
    {
        $this->getSelect()->where('relation.scoretag_id = ?', $scoretagId);
        $this->setFlag('distinct', true);
        return $this;
    }

    /**
     * Add scoretag status filter
     *
     * @param int $status
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    public function addStatusFilter($status)
    {
        $this->getSelect()->where('t.status = ?', $status);
        return $this;
    }

    /**
     * Set DESC order to collection
     *
     * @param string $dir
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    public function setDescOrder($dir = 'DESC')
    {
        $this->setOrder('relation.scoretag_relation_id', $dir);
        return $this;
    }

    /**
     * Add Popularity
     *
     * @param int $scoretagId
     * @param int $storeId
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    public function addPopularity($scoretagId, $storeId = null)
    {
        $scoretagRelationTable = $this->getTable('scoretag/relation');

        $condition = array(
            'prelation.oggetto_id=e.entity_id'
        );

        if (!is_null($storeId)) {
            $condition[] = $this->getConnection()->quoteInto('prelation.store_id = ?', $storeId);
        }
        $condition = join(' AND ', $condition);
        $innerSelect = $this->getConnection()->select()
        ->from(
            array('relation' => $scoretagRelationTable),
            array('oggetto_id', 'store_id', 'popularity' => 'COUNT(DISTINCT relation.scoretag_relation_id)')
        )
        ->where('relation.scoretag_id = ?', $scoretagId)
        ->group(array('oggetto_id', 'store_id'));

        $this->getSelect()
            ->joinLeft(
                array('prelation' => $innerSelect),
                $condition,
                array('popularity' => 'prelation.popularity')
            );

        $this->_scoretagIdFilter = $scoretagId;
        $this->setFlag('prelation', true);
        return $this;
    }

    /**
     * Add Popularity Filter
     *
     * @param mixed $condition
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    public function addPopularityFilter($condition)
    {
        $scoretagRelationTable = Mage::getSingleton('core/resource')
            ->getTableName('scoretag/relation');

        $select = $this->getConnection()->select()
            ->from($scoretagRelationTable, array('oggetto_id', 'popularity' => 'COUNT(DISTINCT scoretag_relation_id)'))
            ->where('scoretag_id = :scoretag_id')
            ->group('oggetto_id')
            ->having($this->_getConditionSql('popularity', $condition));

        $prodIds = array();
        foreach ($this->getConnection()->fetchAll($select, array('scoretag_id' => $this->_scoretagIdFilter)) as $item) {
            $prodIds[] = $item['oggetto_id'];
        }

        if (sizeof($prodIds) > 0) {
            $this->getSelect()->where('e.entity_id IN(?)', $prodIds);
        } else {
            $this->getSelect()->where('e.entity_id IN(0)');
        }

        return $this;
    }

    /**
     * Set scoretag active filter to collection
     *
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    public function setActiveFilter()
    {
        $active = Mage_Scoretag_Model_Scoretag_Relation::STATUS_ACTIVE;
        $this->getSelect()->where('relation.active = ?', $active);
        if ($this->getFlag('prelation')) {
            $this->getSelect()->where('prelation.active = ?', $active);
        }
        return $this;
    }

    /**
     * Add Oggetto Scoretags
     *
     * @param int $storeId
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    public function addOggettoScoretags($storeId = null)
    {
        foreach ($this->getItems() as $item) {
            $scoretagsCollection = Mage::getModel('scoretag/scoretag')->getResourceCollection();

            if (!is_null($storeId)) {
                $scoretagsCollection->addStoreFilter($storeId);
            }

            $scoretagsCollection->addPopularity()
                ->addOggettoFilter($item->getEntityId())
                ->addCustomerFilter($this->_customerFilterId)
                ->setActiveFilter();

            $scoretagsCollection->load();
            $item->setOggettoScoretags($scoretagsCollection);
        }

        return $this;
    }

    /**
     * Join fields process
     *
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    protected function _joinFields()
    {
        $scoretagTable           = $this->getTable('scoretag/scoretag');
        $scoretagRelationTable   = $this->getTable('scoretag/relation');

        $this->addAttributeToSelect('name')
            ->addAttributeToSelect('price')
            ->addAttributeToSelect('small_image');

        $this->getSelect()
            ->join(array('relation' => $scoretagRelationTable), 'relation.oggetto_id = e.entity_id', array(
                'oggetto_id'    => 'oggetto_id',
                'item_store_id' => 'store_id',
            ))
            ->join(array('t' => $scoretagTable),
                't.scoretag_id = relation.scoretag_id',
                array(
                    'scoretag_id',
                    'scoretag_status' => 'status',
                    'scoretag_name'   => 'name',
                    'store_id'   => $this->getConnection()->getCheckSql(
                        't.first_store_id = 0',
                        'relation.store_id',
                        't.first_store_id'
                    )
                )
            );

        return $this;
    }

    /**
     * After load adding data
     *
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    protected function _afterLoad()
    {
        parent::_afterLoad();

        if ($this->getFlag('add_stores_after')) {
            $this->_addStoresVisibility();
        }

        if (count($this) > 0) {
            Mage::dispatchEvent('scoretag_scoretag_oggetto_collection_load_after', array(
                'collection' => $this
            ));
        }

        return $this;
    }

    /**
     * Render SQL for retrieve oggetto count
     *
     * @return string
     */
    public function getSelectCountSql()
    {
        $countSelect = clone $this->getSelect();

        $countSelect->reset(Zend_Db_Select::COLUMNS);
        $countSelect->reset(Zend_Db_Select::ORDER);
        $countSelect->reset(Zend_Db_Select::LIMIT_COUNT);
        $countSelect->reset(Zend_Db_Select::LIMIT_OFFSET);
        $countSelect->reset(Zend_Db_Select::GROUP);

        if ($this->getFlag('group_scoretag')) {
            $field = 'relation.scoretag_id';
        } else {
            $field = 'e.entity_id';
        }
        $expr = new Zend_Db_Expr('COUNT('
            . ($this->getFlag('distinct') ? 'DISTINCT ' : '')
            . $field . ')');

        $countSelect->columns($expr);

        return $countSelect;
    }

    /**
     * Treat "order by" items as attributes to sort
     *
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    protected function _renderOrders()
    {
        if (!$this->_isOrdersRendered) {
            parent::_renderOrders();

            $orders = $this->getSelect()
                ->getPart(Zend_Db_Select::ORDER);

            $appliedOrders = array();
            foreach ($orders as $order) {
                $appliedOrders[$order[0]] = true;
            }

            foreach ($this->_orders as $field => $direction) {
                if (empty($appliedOrders[$field])) {
                    $this->_select->order(new Zend_Db_Expr($field . ' ' . $direction));
                }
            }
        }
        return $this;
    }

    /**
     * Set Id Fieldname as Scoretag Relation Id
     *
     * @return Mage_Scoretag_Model_Resource_Oggetto_Collection
     */
    public function setRelationId()
    {
        $this->_setIdFieldName('scoretag_relation_id');
        return $this;
    }
}
