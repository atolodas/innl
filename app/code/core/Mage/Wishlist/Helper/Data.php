...
class Mage_Wishlist_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getOggettoAddUrl($item)
    {
        return $this->getOggettoAddUrlWithParams($item);
    }
    ...
    public function getOggettoAddUrlWithParams($item, array $params = array())
    {
        $productId = null;
        if ($item instanceof Mage_Catalog_Model_Product) {
            $productId = $item->getEntityId();
