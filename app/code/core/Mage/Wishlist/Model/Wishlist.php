...
class Mage_Wishlist_Model_Wishlist extends Mage_Core_Model_Abstract
{
    ...
    protected function _addObject(Shaurmalab_Score_Model_Oggetto $object)
    {
        $item = null;
        // foreach ($this->getItemCollection() as $_item) {
        //     if ($_item->representProduct($product)) {
        ...
        //     }
        // }
        if(!in_array($object->getId(),Mage::getBlockSingleton('score/bookmarks')->getWishlistIds())) { 
            if ($item === null) {

                $storeId = $object->hasWishlistStoreId() ? $object->getWishlistStoreId() : $this->getStore()->getId();
                $item = Mage::getModel('wishlist/item');
                $item
                    ->setWishlistId($this->getId())
                    ->setAddedAt(now())
                    ->setStoreId($storeId)
                    ->setOggettoId($object->getId())
                    ->setQty(1)
                    ->save();
    ...
    public function addNewObject($oggetto, $buyRequest = null, $forciblySetQty = false)
    {
        /*
         * Always load oggetto, to ensure:
         * a) we have new instance and do not interfere with other oggettos in wishlist
         * b) oggetto has full set of attributes
         */
        if ($oggetto instanceof Shaurmalab_Score_Model_Oggetto) {
            $oggettoId = $oggetto->getId();
            // Maybe force some store by wishlist internal properties
            $storeId = $oggetto->hasWishlistStoreId() ? $oggetto->getWishlistStoreId() : $oggetto->getStoreId();
        } else {
            $oggettoId = (int) $oggetto;
            if ($buyRequest->getStoreId()) {
                $storeId = $buyRequest->getStoreId();
            } else {
                $storeId = Mage::app()->getStore()->getId();
