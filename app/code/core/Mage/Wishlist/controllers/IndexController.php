...
class Mage_Wishlist_IndexController extends Mage_Wishlist_Controller_Abstract
{
    ...
    protected function _addItemToWishList()
    {
        ...
        $objectId = (int)$this->getRequest()->getParam('oggetto');
        ...
        if($productId) { 
            ...
            $object = Mage::getModel('score/oggetto')->load($objectId);
            if (!$object->getId()) {
                $session->addError($this->__('Cannot specify object.'));
                $this->_redirect('*/');
                return;
            }
        }
        ...
        try {
            ...
            if($productId) { 
                ...
                $result = $wishlist->addNewObject($object, $buyRequest);
            ...
            if($productId) { 
                ...
                if($isAjax) { echo $message; } 
                else {  
                    $session->addSuccess($message);
                }
            } elseif($objectId) { 
                $message = $this->__('%1$s has been added to your bookmarks.',
                    $object->getName(), Mage::helper('core')->escapeUrl($referer));
                if($isAjax) { echo $message; } 
                else {  
                    $session->addSuccess($message);
                }
            ...
            if($productId) { 
                ...
                if($isAjax) { echo $message; } 
                  ...
                  $message = $this->__('An error occurred while adding item to bookmarks: %s', $e->getMessage());
                if($isAjax) { echo $message; } 
                else {  
                    $session->addError($message);
                }
    ...
    public function removeByOggettoIdAction() {
        $oid = (int) $this->getRequest()->getParam('oid');
        $customerId = Mage::getSingleton('customer/session')->getCustomer()->getId();

        $wishlist = Mage::getModel('wishlist/wishlist')->loadByCustomer($customerId, true);
        $items = $wishlist->getItemCollection();
        foreach ($items as $item) {
            $wishlistItems[$item->getId()] = $item->getOggettoId();
        }
        $id = array_search($oid, $wishlistItems);
