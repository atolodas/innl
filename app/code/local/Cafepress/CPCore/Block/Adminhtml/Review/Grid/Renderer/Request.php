...
class Cafepress_CPCore_Block_Adminhtml_Review_Grid_Renderer_Request extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Abstract
{
...
//        $storeId = (int)$this->getRequest()->getParam('store', 0);
        ...
        $format = Mage::getModel('cpcore/xmlformat')->load($formatId);
            ...
            switch ($formatType){
                case 'order':
                    if($orderIncrementId){ 
                        ...
                        $request = Mage::getModel('cpcore/order_observer')->getOrderXmlByFormat($order, $formatId);
                    } else{
                ...
                case 'creditmemo':
                    ...
                    if($orderIncrementId){ 
                        ...
                        if(isset($order) && $order->hasCreditmemos()){
                            ...
                            $request = Mage::getModel('cpcore/order_observer')->getXmlByFormat($order, $formatType, $creditMemo, $formatId);
                        } 
                    }
                    break;
                case 'orderstatus':
...
//        }
        ...
        return '<div class="wms-grid-request"><pre>'.Mage::helper('cpcore')->formatXml($request, '<br/>').'</pre></div>';
