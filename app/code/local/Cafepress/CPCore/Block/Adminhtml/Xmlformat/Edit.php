...
class Cafepress_CPCore_Block_Adminhtml_Xmlformat_Edit extends Mage_Adminhtml_Block_Widget
{
    ...
    protected function _prepareLayout()
    {
        ...
        $this->setChild('save_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')
                ->setData(array(
                    'label'     => Mage::helper('cpcore')->__('Save'),
        ...
        $this->setChild('save_and_edit_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')
                ->setData(array(
                    'label'     => Mage::helper('cpcore')->__('Save and Continue Edit'),
        ...
        if ($this->getXmlformat()->isDeleteable()) {
            ...
            $this->getLayout()->createBlock('adminhtml/widget_button')
                    ->setData(array(
                        ...
                        'onclick'   => 'confirmSetLocation(\''.Mage::helper('cpcore')->__('Are you sure?').'\', \''.$this->getDeleteUrl().'\')',
                        'class'  => 'delete'
                    ))
            );
        }
    ...
    public function getHeader()
    {
        ...
        $typeName = Mage::getModel('cpcore/resource_eav_mysql4_xmlformat_type')->getNameTypeById($typeCode);
        if($formatName){
            $header = Mage::helper('cpcore')->__('Edit').' "'.$formatName.'" (type: '.$typeName.')';
        } else{
            $header = Mage::helper('cpcore')->__('New Xml Format').' (type: '.$typeName.')';
        }
        return $header;
    }
    ...
    {
        return $this->getUrl('cpcore/index/getOrdersId',
            array(
                '_current'   => true,
            ));
    }
