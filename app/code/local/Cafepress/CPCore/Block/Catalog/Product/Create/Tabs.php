...
class Cafepress_CPCore_Block_Catalog_Product_Create_Tabs extends Mage_Adminhtml_Block_Widget_Tabs
{
        ...
        switch($activeTab){
            case 'setprint':
                $this->addTab('setprint', array(
                    'label'     => Mage::helper('cpcore')->__('CP Print'),
                    'content'   => $this->_translateHtml($this->getLayout()->createBlock('cpcore/catalog_product_create_tab_print')->toHtml()),
            ...
            case 'merchandise':
                ...
                foreach($categories as $category_id => $category){
                    ...
                    if($result['count'] > 0){
                        $this->addTab('merchandise_'.$category_id, array(
                            'label'     => Mage::helper('cpcore')->__($category),
                            'content'   => $result['html']
                        ));
                        if($first){
                            $first = false;
                ...
                break;
            case 'setparams':
                Mage::register('cp_design_id', $this->getRequest()->getParam('cp_design_id'));
                $this->addTab('setparams', array(
                    'label'     => Mage::helper('cpcore')->__('Product Data'),
                    'content'   => $this->_translateHtml($this->getLayout()->createBlock('cpcore/catalog_product_create_tab_setparams')->toHtml()),
            ...
            case 'createremote':
                ...
                $product = Mage::getModel('cpcore/cafepress_product')->createRemoteProduct($data);
...
//                    'label'     => Mage::helper('cpcore')->__('Product Data'),
//                    'content'   => $this->_translateHtml($this->getLayout()->createBlock('cpcore/catalog_product_create_tab_createremote')->toHtml()),
