...
class Cafepress_CPCore_Block_Catalog_Products_Copy extends Mage_Adminhtml_Block_Widget
{
    public function __construct()
    {
        parent::__construct();
        $this->setTemplate('cpcore/cafepress/products/copy.phtml');
        $this->setId('products_copy');
    }
        ...
        switch($this->getRequest()->getParam('action')){
            case '':
                $this->setChild('continue_button',
                    $this->getLayout()->createBlock('adminhtml/widget_button')
                        ->setData(array(
                        'label' => Mage::helper('catalog')->__('Set Store'),
            ...
            case 'select_section':
                $this->setChild('continue_button',
                    $this->getLayout()->createBlock('adminhtml/widget_button')
                        ->setData(array(
                        'label' => Mage::helper('catalog')->__('Set Section'),
                        'onclick' => 'productForm.submit()',
                        'class' => 'save'
                    ))
                );
            ...
            case 'select_products':
                $this->setChild('continue_button',
                    $this->getLayout()->createBlock('adminhtml/widget_button')
                        ->setData(array(
                        'label' => Mage::helper('catalog')->__('Copy Selected Products'),
                        'onclick' => 'productForm.submit()',
                        'class' => 'save'
                    ))
                );
            ...
            case 'products_copied':
                ...
                $this->setChild('continue_button',
                    $this->getLayout()->createBlock('adminhtml/widget_button')
                        ->setData(array(
                        'label' => Mage::helper('catalog')->__('To Products List'),
                        'onclick'   => 'setLocation(\''.Mage::getBaseUrl().'admin/catalog_product/\')',
                        'class' => 'save'
                    ))
                );
                break;
