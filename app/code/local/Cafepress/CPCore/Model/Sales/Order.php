...
class Cafepress_CPCore_Model_Sales_Order extends Mage_Sales_Model_Order {
    ...
    public function addWmsFile($filename) {
        $this->setData('cp_wms_file', str_replace($filename, '', $this->getData('cp_wms_file')) . ' ' . $filename);
    ...
    public function printError($responseXML) {
        Mage::log(
                sprintf("XML Error: %s at line %d. XML: %s", xml_error_string(xml_get_error_code($this->parser)), xml_get_current_line_number($this->parser), $responseXML), null, 'authorization.log'
        );
    }
    ...
    public function customAuthorize($order) {
        ...
        $response = Cafepress_CPCore_Model_Order_Observer::sendXmlOverPost($request, $storeId, array('url' => $orderStatus->getCustomUrl()));
    ...
    public function loadBySavedIncrementId($incrementId=null) {
        if (Mage::registry('number') && ($incrementId === null || $incrementId === 'null')) {
            echo 'LOAD ORDER FOR SAVE BY ID:'.$incrementId = Mage::registry('number');
        }
        return $this->loadByAttribute('increment_id', $incrementId);
    }

    public function getWmsRequestStatus($format) {
        $value = unserialize($this->getData('cp_wms_file_status'));
        if (!$value) {
            if ($format != 'last')
                return false;
            $statuses = Mage::helper('cpcore')->getStatuses();
            return $statuses[$this->getData('cp_wms_file_status')];
        }
        return isset($value[$format])?$value[$format]:null;
    }
    ...
    public function setCpWmsFileStatus($status, $format = false) {
        if (!$status) {
            return $this;
        }
        $value = array();
        if ($this->getData('cp_wms_file_status') && $this->getData('cp_wms_file_status') != '') {
            @$value = unserialize($this->getData('cp_wms_file_status'));
        }
        $statuses = Mage::helper('cpcore')->getStatuses();
        if (!$value) {
            if (is_numeric($status)) {
                ...
                $_SESSION['cp_log_status'] = $statuses[$status];
            } else {
                $value['last'] = $statuses[$this->getData('cp_wms_file_status')];
                $_SESSION['cp_log_status'] = $statuses[$this->getData('cp_wms_file_status')];
                $value[$format] = $status;
            }
        } else {
            if (!$format) {
        ...
        $this->setData('cp_wms_file_status', serialize($value));
        return $this;
    }

    public function setWmsRequestStatus($status, $format = false) {
//        Mage::log('START****', null, 'debug_o_status.log');
//        Mage::log('0:'.$this->getStatus(), null, 'debug_o_status.log');
//        $this->save();
//        Mage::log('1:'.$this->getStatus(), null, 'debug_o_status.log');
        $this->setCpWmsFileStatus($status, $format);
//        Mage::log('5000:'.$this->getStatus(), null, 'debug_o_status.log');
//        $this->save();
//        Mage::log('1000:'.$this->getStatus(), null, 'debug_o_status.log');
        if ($filename = basename(Mage::registry('cp_response_filename'))) {
//            Mage::log('op', null, 'debug_o_status.log');
            $this->setData('cp_wms_file', str_replace($filename, '', $this->getData('cp_wms_file')) . ' ' . $filename);
            $this->save();
        }
//        Mage::log('2000:'.$this->getStatus(), null, 'debug_o_status.log');
        return $this;
    }
    ...
    public function logStatus(){
        Mage::log($this->getStatus(), null, 'debug.log');
    }
