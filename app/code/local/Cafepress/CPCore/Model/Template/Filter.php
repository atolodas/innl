...
class Cafepress_CPCore_Model_Template_Filter extends Varien_Filter_Template
{
    /**
    ...
    const MODULE_PATH = 'Cafepress_CPCore';
    ...
    protected $_modifiers = array(
        ...
        'round'             => 'round',
        'str_pad'           => 'str_pad',
        'round02'           => array('Cafepress_CPCore_Helper_Data','round02'),
        'month'             => array('Cafepress_CPCore_Helper_Data','formatMonth'),
        'date'              => array('Cafepress_CPCore_Helper_Data','formatDate'),
        'time'              => array('Cafepress_CPCore_Helper_Data','formatTime'),
        'line1'             => array('Cafepress_CPCore_Helper_Data','getStreetLine1'),
        'line2'             => array('Cafepress_CPCore_Helper_Data','getStreetLine2'),
        'round_money'       => array('Cafepress_CPCore_Helper_Data','roundMoney'),
        'shipping_method'   => array('Cafepress_CPCore_Helper_Data','codeShippingMethod'),
        'shipping_method_relastin'   => array('Cafepress_CPCore_Helper_Data','codeShippingMethodRelastin'),
        'advanced_name'     => array('Cafepress_CPCore_Helper_Data','advancedCardName'),
        'double'            => array('Cafepress_CPCore_Helper_Data','double'),
        'decrypt'           => array('Mage_Core_Helper_Data','decrypt'),
        'toDate'            => array('Cafepress_CPCore_Helper_Data','datetimeToDate'),
        'no_space'          => array('Cafepress_CPCore_Helper_Data','noSpace'),
        'shipMethodEncode'  => array('Cafepress_CPCore_Helper_Data','shipMethodEncode'),
        'unCdata'           => array('Cafepress_CPCore_Helper_Data','unCdata'),
    ...
    const CONSTRUCTION_IF_PATTERN = '/{{if\s*(.*?)}}(.*?)(({{elseif.*?}}.*?)*)({{else}}(.*?))?{{\\/if\s*}}/si';
//    const CONSTRUCTION_ELSE_IF_PATTERN = '/{{elseif\s*(.*?)}}(.*?)/si';
    const CONSTRUCTION_ELSE_IF_PATTERN = '/(.*?)}}(.*)/si';
...
//            $value = parent::filter($value);
	...
	public function condDirective($construction)
    {
        ...
        $replaceLocal =array('from' => array(), 'to' => array());
		foreach($matches['1'] as $variable){
            $replaceLocal['from'][]  = "[[$variable]]";

            $intermCalc =  $this->filter('{{'.$variable.'}}');
            if(!$intermCalc){
                $intermCalc = 0;
            } elseif(is_string($intermCalc)){
                $intermCalc = "'$intermCalc'";
            }
            $replaceLocal['to'][]    = $intermCalc;
		}

		$expression = str_replace($replaceLocal['from'],$replaceLocal['to'],trim($variableName));
		$result = '';
echo $expression = str_replace('.0','',$expression);		
eval ("\$result = (float)($expression);");
    ...
    public function functionDirective($construction)
    {
        ...
        return Mage::helper('cpcore')->$method($variableName);
    }
    ...
    public function varDirective($construction)
    {
        ...
        return Mage::helper('cpcore')->addCData($var);
    }
    ...
    protected function _amplifyModifiers($value, $modifiers)
    {
        foreach (explode('|', $modifiers) as $part) {
            ...
            $replacerModel = $this->_getReplacerModel()->setFilterModel($this);
            $value = $replacerModel->replaceValueByHelper($modifier,$value,$this->getStoreId());
        }

        return $value;
    }

    public function _getReplacerModel(){
        return Mage::getSingleton('cpcore/replacer');
    }
    ...
    public function ifDirective($construction){
        ...
        if($this->_getVariable($construction[1], '') == '') {
            if (isset($construction[3]) && $construction[3]!==''){
                $conditions = explode('{{elseif',$construction[3]);
//                Mage::log('****************************************************', null, 'debug_elseif.log');
//                Mage::log($construction[3], null, 'debug_elseif.log');

                foreach($conditions as $condition){
                    if (!$condition) continue;
                    $elseifmatchesarray = array();
//                    Mage::log('CONDITION:'.$condition, null, 'debug_elseif.log');
                    preg_match_all(self::CONSTRUCTION_ELSE_IF_PATTERN, $condition, $elseifmatchesarray, PREG_SET_ORDER);
//                    Mage::log($elseifmatchesarray, null, 'debug_elseif.log');
//                    Mage::log('**1:', null, 'debug_elseif.log');
//                    Mage::log(trim($elseifmatchesarray[0][1]), null, 'debug_elseif.log');
//                    Mage::log('**2:', null, 'debug_elseif.log');
//                    Mage::log(trim($elseifmatchesarray[0][2]), null, 'debug_elseif.log');
//                    Mage::log($this->_getVariable(trim($elseifmatchesarray[0][1]), ''), null, 'debug_elseif.log');
                    if($this->_getVariable(trim($elseifmatchesarray[0][1]), '') !== '') {
//                        Mage::log('OK!', null, 'debug_elseif.log');
                        return trim($elseifmatchesarray[0][2]);
                    }
                }
            }

            if (isset($construction[5]) && isset($construction[6])) {
                return $construction[6];
            }
            return '';
        } else {
