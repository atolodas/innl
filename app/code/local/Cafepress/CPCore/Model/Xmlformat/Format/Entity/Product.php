...
class Cafepress_CPCore_Model_Xmlformat_Format_Entity_Product extends Cafepress_CPCore_Model_Abstract
{
   ...
   private $_productAttributeDef = array(
    ...
    {
        $this->_product = Mage::getModel('catalog/product')->setStoreId(0)->load(0);
    ...
    public function load($id) {
        $this->_product = Mage::getModel('catalog/product')->setStoreId(0)->load($id);
        return $this->_product;
    }
...
//        $this->_productData[trim((string)$name)] = trim($value);
    ...
    public function saveProduct()
    {
        ...
        try {
            ...
            if (!$this->_getProduct()){
               // echo "No product loaded <br/>"; return;
...
if(is_object($product)) {
	$productId = $product->getId();
            ...
            echo "Saving product id: ".$this->_getProduct()->getId()."<br/>";
}
            ...
            if (!$productId){
                ...
                try {
                    ...
                    if ($productData['type_id'] == 'configurable'){
                        ...
                        echo "Product Data to save: "; print_r($productData);
                    }
                    try {
                       // $product->setWebsiteIds($storeIds); // TODO: assign to websites automatically later
...
$connection = Mage::getModel('catalog/product')->getCollection()->getConnection();
     ...
     public function addImageToGalleryByUrl($image_url) {
         if(!$image_url) return $this->_product;

    	$imageUrl = explode('?',$image_url);
        ...
        $mediaGalleryAttribute->getBackend()->addImage($this->_product, $filepath, $mediaAttribute, 0, 0);
        return $this->_product;
