...
class Cafepress_CPCore_Adminhtml_DevController extends Mage_Adminhtml_Controller_Action
{
	...
	public function tailFileAction()
	{
		...
		if($f){
			...
			$tail = Mage::getModel('cpcore/tail', $f);
			if ($this->getRequest()->getParam('grep')) {
				$tail->setGrep($this->getRequest()->getParam('grep'));
			}

			if ($this->getRequest()->getParam('cpcore_show')){
				$tail->setNumberOfLines($this->getRequest()->getParam('cpcore_show'));
			}
    ...
    public function xmlTailFileAction()
    {
        $f = $this->getRequest()->getParam('file');
        ...
        if($f){
            ...
            $tail = Mage::getModel('cpcore/xmltail', $f);
            if ($this->getRequest()->getParam('grep')) {
                $tail->setGrep($this->getRequest()->getParam('grep'));
            }
