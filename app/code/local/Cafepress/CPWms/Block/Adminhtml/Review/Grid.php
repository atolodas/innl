...
class Cafepress_CPWms_Block_Adminhtml_Review_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
...
//            $collection->_setDefaultItems(); //is delete duplicate row
                ...
                foreach ($allStores as $store) {
                    $collection2 = Mage::getModel('cpwms/xmlformat')->getCollection()
                        ->addAttributeToSelect('*')
...
//        Zend_Debug::dump($collection->getItems());
    ...
    protected function _prepareColumns()
    {
        ...
        $this->addColumn('name', array(
            'header' => Mage::helper('cpwms')->__('Name'),
            'width' => '150px',
            'align' => 'right',
            'index' => 'name',
            'type'  => 'text',
        ...
        $this->addColumn('url', array(
            'header' => Mage::helper('cpwms')->__('Url'),
            'width' => '300px',
//            'align' => 'right',
            'index' => 'entity_id',
            'type'  => 'text',
            'renderer' => new Cafepress_CPWms_Block_Adminhtml_Review_Grid_Renderer_Url()
        ));
...
//            'align' => 'right',
            'index' => 'entity_id',
            'type'  => 'textarea',
            'renderer' => new Cafepress_CPWms_Block_Adminhtml_Review_Grid_Renderer_Request()
        ));
        
        $this->addColumn('schedule', array(
            'header' => Mage::helper('cpwms')->__('Schedule'),
            'width' => '50px',
            'align' => 'right',
            'index' => 'schedule',
            'type'  => 'options',
            'options' => Mage::getModel('cpwms/resource_eav_mysql4_xmlformat_schedule')->getOptionArray(),
        ));
        ...
        $this->addColumn('response', array(
            'header' => Mage::helper('cpwms')->__('Response'),
//            'width' => '200px',
            'index' => 'response',
            'type'  => 'text',
            'renderer' => new Cafepress_CPWms_Block_Adminhtml_Review_Grid_Renderer_Response()
        ));
        
        if (!Mage::app()->isSingleStoreMode()){
            if ($this->_getStoreIdByOrder()){
                $this->addColumn('store', array(
                    'header' => Mage::helper('cpwms')->__('Store'),
                    'width' => '80px',
//                    'align' => 'right',
                    'filter'    => false,
                    'sortable'  => false,
                    'index'     => $this->_getStoreIdByOrder(),
                    'renderer' => new Cafepress_CPWms_Block_Adminhtml_Review_Grid_Renderer_Store()
                ));
            } else {
                $this->addColumn('store', array(
                    'header' => Mage::helper('cpwms')->__('Store'),
