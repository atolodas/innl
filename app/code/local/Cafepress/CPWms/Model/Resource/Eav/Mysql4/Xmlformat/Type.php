...
class Cafepress_CPWms_Model_Resource_Eav_Mysql4_Xmlformat_Type extends Mage_Core_Model_Abstract
{
    const TYPE_ORDER    = 1;
    ...
    {
        $result = array();
        $developer = Mage::helper('cpwms')->isDeveloper();
        foreach (self::$formatTypes as $type){
            if ($type['develop']){
                if ($developer == $type['develop']){
                    $result[$type['id']] = Mage::helper('cpwms')->__($type['title']);
                }
            } else {
                $result[$type['id']] = Mage::helper('cpwms')->__($type['title']);
            }
