...
class Cafepress_CPWms_Model_Xmlformat_Format_Downfileparsresp extends Cafepress_CPWms_Model_Xmlformat_Format_Abstract
{
...
//            Mage::log('Files:'.implode(', ',$filesName).' was download and saved.',null,'downloadfileparsrespons.log');
    ...
    {
        return Mage::getModel('cpwms/xmlformat_outbound')->fileNotOpen($filename,'downloadfileparsrespons.log');
    }
    ...
    public function processResponse($cap=false)
    {
        ...
        foreach($this->_files as $key => $file){
            if (Mage::registry('wms_response_filename')){
                Mage::unregister('wms_response_filename');
            }
            Mage::register('wms_response_filename', $file['filename']);
            if($file['content']){
    ...
    public function getFiles($pattern, $type = 'shipment') {
         $result = Mage::getModel('cpwms/xmlformat_outbound')->getFiles($pattern, $type);
         if (!is_array($result)){
             return array();
         }
         return $result;
    ...
    {
        return Mage::getModel('cpwms/xmlformat_outbound')->deleteFilesFromFtp($this->_filesForDelete);
    }
