...
class Cafepress_CPWms_Model_Xmlformat_Format_Entity_Shipment extends Cafepress_CPWms_Model_Abstract
{
...
//                    ->save()
            ...
            $order = Mage::getSingleton('cpwms/sales_order')->load($this->getOrder()->getId());
            $filename = Mage::registry('wms_response_filename');
...
//            $shipment->save();
            ...
            $date = Mage::helper('cpwms/date')->localizeDate($date);
...
//        Zend_Debug::dump($val);
    ...
    public function shipOrder($order,$importData) {
        try {
            ...
            {
                $this->addTracks($shipment, $importData);
                $shipment->setCreatedAt(Mage::helper('cpwms')->formatDateShip($this->_dateShipment));
                $shipment->setEmailSent($this->_emailSent);
                $shipment->save();
                if ($this->_emailSent){
                    $shipment->sendEmail();
