...
class Cafepress_CPWms_Model_Xmlformat_Format_Order extends Cafepress_CPWms_Model_Xmlformat_Format_Abstract
{
...
//    }
	...
	//	Zend_Debug::dump($formatId);
		if($formatId != false)
		{
			$this->format = Mage::getModel('cpwms/xmlformat')
    ...
    {
        if (Mage::helper('cpwms')->isXML($response) && Mage::helper('cpwms')->isXML($responseFormat)){
    ...
    public function divideCondition($format_id){
        $condition = Mage::getModel('cpwms/xmlformat')->load($format_id)->getCondition();
