...
class Cafepress_CPWms_Model_Xmlformat_Outbound extends Cafepress_CPWms_Model_Abstract
{
...
//            $fienameComplete = "$method://$server/$folder/".$filename;
            ...
            $_SESSION['wms_log_url_of_request'] = $fienameComplete;
            $localfile = $file;
            ...
            if ($order!=false){
                if (!$creditmemo){
                   if ($error_no==0) {
                    ...
                    if ($error_no==0) {
                        $error = false;
                        $creditmemo->setData('wms_file_status',(Mage::helper('cpwms')->getStatusId('Sent')));
                        $creditmemo->save();
                    } else {
                        $error = 'File upload error. '.$fienameComplete;
                        $creditmemo->setData('wms_file_status',(Mage::helper('cpwms')->getStatusId('Fail Sending')));
                        $creditmemo->save();
                    }
                }
    ...
    {
        $_SESSION['wms_log_function'] = 'HTTP';
        ...
        $_SESSION['wms_log_url_of_request'] = $server['url'];
...
//		if(!$open) { $xml->startElement($nodeValueKey); $open++; }
    ...
    {
        $_SESSION['wms_log_function'] = $custom_url_or_function;
		if($order != false)
		{
		...
		$mxml->endElement();
		$response = Mage::helper('cpwms')->replaceCharsXML($mxml->outputMemory());
    ...
    public function saveXml($xml, $order, $formatId) {
        ...
        $_SESSION['wms_log_link_to_file'] = $file_path;
        @unlink($file);
    ...
    public function saveCreditMemoXml($xml,$order,$creditmemo, $formatId) {
        $type = Mage::getModel('cpwms/resource_eav_mysql4_xmlformat_type')->getType('creditmemo');
        $creditmemoModel = Mage::getModel('cpwms/xmlformat_format_creditmemo')
            ->setStoreId($order->getStoreId())
//                            ->setOrderById($order->getId())
            ->setOrder($order)
...
//            Mage::log('Connect to FTP:'.$login.'@'.$server.'/'.$folder,null,'wms.log');
            $_SESSION['wms_log_url_of_request'] = "ftp://$server/$folder/";
            ...
            $downloads_file = array_diff($items,$local_files);
