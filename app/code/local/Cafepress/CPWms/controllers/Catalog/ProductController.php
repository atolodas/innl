...
class Cafepress_CPWms_Catalog_ProductController extends Mage_Adminhtml_Controller_Action
{

    protected function _initAction() {
        $this->loadLayout()
                ->_setActiveMenu('catalog/cpwms')
                ->_addBreadcrumb(Mage::helper('cpwms')->__('Cafe Press Product Manager'), Mage::helper('cpwms')->__('Cafe Press Product Manager'));

        $this->_title($this->__('cpwms'))->_title($this->__('Cafe Press Product Manager'));
        return $this;
    }
    ...
    {
        $this->_initAction()
            ->_addContent($this->getLayout()->createBlock('cpwms/catalog_product'))
            ->renderLayout();
    }
...
//        $product = $this->_initProduct();
    ...
    public function createTokenAction()
    {
        ...
        $userToken = Mage::getSingleton('cpwms/cafepress_token')->createToken();
//        $userToken = '123-123-123';
...
//        die($action);
        ...
        if($action) {
            switch ($action){
                ...
                case 'selectmerchant':{
                    $merchandiseData = Mage::getModel('cpwms/cafepress_merchandise')->getDataById($data['product_type']);
                    Mage::getModel('cpwms/cafepress_product')->setProduct($product)->saveMerchandiseData($merchandiseData);
                    $actionNext = 'createproduct';
                } break;
                case 'selectmerchantback':{
//                    $merchandiseData = Mage::getModel('cpwms/cafepress_merchandise')->getDataById($data['product_type']);
//                    Mage::getModel('cpwms/cafepress_product')->setProduct($product)->setMerchandiseData($merchandiseData);
                    $actionNext = 'createproduct';
                } break;
                case 'createproduct':{
//                    Mage::getModel('cpwms/cafepress_product')->setProduct($product)->saveCafepress($data['create_product_xml1']);
                    $xml = Mage::helper('cpwms')->getXmlByXmlData($data['create_product_xml']);
                    Mage::getModel('cpwms/cafepress_product')->setProduct($product)->saveCafepress($xml);
            ...
            if(!isset($data['leaveOld'])){
                ...
                foreach($galleryData['images'] as $image){
                    if($image['file'] == $product->getCafepressImage()){
                        $gallery->getBackend()->removeImage($product, $image['file']);
                    }
                }

                $product->addImageToMediaGallery($tmp_file_path, 'cafepress_image', false, false);
...
//                $actionNext = 'token';
        ...
        switch ($actionNext){
            case 'createproduct':{
                $merchandiseData = Mage::getModel('cpwms/cafepress_product')->setProduct($product)->createNewCafepress();
            }break;
        }
        
        Mage::register('cpwms_activ_tab',$actionNext);
...
//        $this->renderLayout();
        ...
        $this->getLayout()->createBlock('cpwms/catalog_product_grid')->toHtml());
    }
    ...
    public function createAssociatedAction(){
        ...
        foreach($simples as $simple_params){
            ...
            $color_id = Mage::getModel('cpwms/cafepress_product')->getLocalColorId($params[0]);
            if(count($params) > 1){
                $size_id = Mage::getModel('cpwms/cafepress_product')->getLocalSizeId($params[1]);
            } else{
                $size_id = false;
            }
            ...
            $product_images = Mage::getModel('cpwms/cafepress_product')->getProductImages($sxml, $params[0]);
        ...
        Mage::getModel('cpwms/cafepress_product')->attachSimpleToConfigurable($configurable, $productIds);
    }
