 ...
 */
class DP_All_Block_Jsinit extends Mage_Adminhtml_Block_Template {
    ...
    protected function _prepareLayout() {
        ...
        if ($this->_section == 'dpall') {
            $this->getLayout()
                    ->getBlock('head')
                    ->addJs('dp_all/dp_all.js');
            $this->setData('extensions', $this->_initExtensions());
        } elseif ($this->_section == 'dpstore') {
            // DP extensions store
            $this->getLayout()
                    ->getBlock('head')
                    ->addJs('dp_all/dp_all.js');
    ...
    protected function _toHtml() {
        if ($this->_section == 'dpall' || $this->_section == 'dpstore') {
    ...
    protected function _initExtensions() {
        ...
        foreach ($modules as $moduleName) {
            ...
            $platform = DP_All_Helper_Versions::convertPlatform($platform);
    ...
    public function getIcon($Extension) {
        if ($Extension->getUpgradeAvailable()) {
            $icon = 'dp_all/images/update.gif';
            $title = "Update available";
        } elseif (!$Extension->getIsPlatformValid()) {
            $icon = 'dp_all/images/bad.gif';
            $title = "Wrong Extension Platform";
        } else {
            $icon = 'dp_all/images/ok.gif';
    ...
    protected function _getStoreData() {
        ...
        if ($storeResponse !== false) {
            ...
            Mage::app()->saveCache($storeResponse, DP_All_Helper_Config::STORE_RESPONSE_CACHE_KEY);
        } else {
            $storeResponse = Mage::app()->loadCache(DP_All_Helper_Config::STORE_RESPONSE_CACHE_KEY);
            if (!$storeResponse) {
    ...
    protected function _getStoreConnection() {
        ...
        $url = rtrim(DP_All_Helper_Config::STORE_URL) . (sizeof($url) ? ("?" . implode("&", $url)) : "");
