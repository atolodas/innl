...
class EH_SocialLogin_Model_Twitter_Client
{
    const REDIRECT_URI_ROUTE = 'ehut_sociallogin/twitter/connect';
    const REQUEST_TOKEN_URI_ROUTE = 'ehut_sociallogin/twitter/request';

    const OAUTH_URI = 'https://api.twitter.com/oauth';
    const OAUTH2_SERVICE_URI = 'https://api.twitter.com/1.1';    

    const XML_PATH_ENABLED = 'ehut_sociallogin/twitter/enabled';
    const XML_PATH_CLIENT_ID = 'ehut_sociallogin/twitter/api_key';
    const XML_PATH_CLIENT_SECRET = 'ehut_sociallogin/twitter/secret';
    ...
    protected function _httpRequest($url, $method = 'GET', $params = array())
    {
        ...
        if($response->isError()) {
            ...
            if(($status == 400 || $status == 401 || $status == 429)) {
                ...
                throw new EH_SocialLogin_TwitterOAuthException($message);
            } else {
                $message = sprintf(
                    Mage::helper('ehut_sociallogin')
...
class EH_SocialLogin_TwitterOAuthException extends Exception
{}
