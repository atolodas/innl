...
class Hwg_Attributemanager_Block_Adminhtml_Customer_Edit_Tab_Main extends Mage_Adminhtml_Block_Widget_Form
{
 ...
 /*******************************************************/
        ...
        $fieldset->addField('public_account', 'checkbox', array(
            'name' => 'customer_form[]',
            'label' => Mage::helper('catalog')->__('Public Account'),
			'value' => 'public_account',
			'checked' => isset($customerForm['public_account'])?$customerForm['public_account']:'',
        ));

        $fieldset->addField('search_form', 'checkbox', array(
            'name' => 'customer_form[]',
            'label' => Mage::helper('catalog')->__('Search Form'),
			'value' => 'search_form',
			'checked' => isset($customerForm['search_form'])?$customerForm['search_form']:'',
        ));

	 $fieldset->addField('pro_reg', 'checkbox', array(
            'name' => 'customer_form[]',
            'label' => Mage::helper('catalog')->__('Pro registration'),
                        'value' => 'pro_reg',
                        'checked' => isset($customerForm['pro_reg'])?$customerForm['pro_reg']:'',
        ));

       $groups = Mage::getModel('customer/group')->getCollection();

        foreach($groups as $group) {
            if($group->getId()==0 || $group->getId()==1) continue;
            $fcode = strtolower(str_replace(' ','_',$group->getCode()));

            $fieldset->addField($fcode.'_reg', 'checkbox', array(
                'name' => 'customer_form[]',
                'label' => Mage::helper('catalog')->__($group->getCode().' Registration Form'),
                'value' => $fcode.'_reg',
                'checked' => isset($customerForm[$fcode.'_reg'])?$customerForm[$fcode.'_reg']:'',
            ));

            $fieldset->addField($fcode.'_account', 'checkbox', array(
                'name' => 'customer_form[]',
                'label' => Mage::helper('catalog')->__($group->getCode().' Account'),
                'value' => $fcode.'_account',
                'checked' => isset($customerForm[$fcode.'_account'])?$customerForm[$fcode.'_account']:'',
            ));
        }
