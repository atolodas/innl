...
DROP TABLE IF EXISTS {$this->getTable('oggcomment')};
CREATE TABLE {$this->getTable('oggcomment')} (
  `oggcomment_id` int(11) unsigned NOT NULL auto_increment,
	`oggetto_id` int(11)  NULL,
	`customer_id` int(11)  NULL,
	`created_at` datetime NULL,
	`product_comment` text,
  PRIMARY KEY (`oggcomment_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

");
$installer->run("

DROP TABLE IF EXISTS {$this->getTable('oggcomment_lck')};
CREATE TABLE {$this->getTable('oggcomment_lck')} ( 	
	`flag` varchar(4),
	`value` INT DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

INSERT INTO `{$installer->getTable('oggcomment_lck')}` VALUES ('LCK','1');
");
