...
class Shaurmalab_Commercebug_AjaxController extends Mage_Core_Controller_Front_Action
{
    ...
    public function togglehintsAction()
    {
        ...
        $c = $cache->load(Shaurmalab_Commercebug_Model_Observer::TEMPLATE_HINTS_ON);
        $c = $c == 'on' ? 'off' : 'on';        
        $cache->save($c, Shaurmalab_Commercebug_Model_Observer::TEMPLATE_HINTS_ON, array(Mage_Core_Model_Config::CACHE_TAG), (60 * 60 * 24 * 1));
    ...
    {
        $cache = $this->_getCacheObject();
        $c = $cache->load(Shaurmalab_Commercebug_Model_Observer::BLOCK_HINTS_ON);
        $c = $c == 'on' ? 'off' : 'on';        
        $cache->save($c, Shaurmalab_Commercebug_Model_Observer::BLOCK_HINTS_ON, array(Mage_Core_Model_Config::CACHE_TAG), (60 * 60 * 24 * 1));
    ...
    {
        $cache = $this->_getCacheObject();
        $c = $cache->load(Shaurmalab_Commercebug_Model_Observer::MAGE_LOGGING_ON);
        $c = $c == 'on' ? 'off' : 'on';        
        $cache->save($c, Shaurmalab_Commercebug_Model_Observer::MAGE_LOGGING_ON, array(Mage_Core_Model_Config::CACHE_TAG), (60 * 60 * 24 * 1));
    ...
    {
        $cache = $this->_getCacheObject();
        $c = $cache->load(Shaurmalab_Commercebug_Model_Observer::CB_LOGGING_ON);
        $c = $c == 'on' ? 'off' : 'on';        
        $cache->save($c, Shaurmalab_Commercebug_Model_Observer::CB_LOGGING_ON, array(Mage_Core_Model_Config::CACHE_TAG), (60 * 60 * 24 * 1));
        $this->endWithHtml('Commerce Bug Logging ' . ucwords($c) .'');        
    }
    
    function getShim()
    {
        $shim = Shaurmalab_Commercebug_Model_Shim::getInstance();
        return $shim;		
    }
