 ...
 */
class Shaurmalab_Score_Block_Adminhtml_Score_Category_Edit_Form extends Shaurmalab_Score_Block_Adminhtml_Score_Category_Abstract
{
    /**
    ...
    {
        parent::__construct();
        $this->setTemplate('score/category/edit/form.phtml');
    }
    ...
    protected function _prepareLayout()
    {
        ...
        $this->setChild('tabs',
            $this->getLayout()->createBlock('score/adminhtml_score_category_tabs', 'tabs')
        );
        ...
        if (!$category->isReadonly()) {
            $this->setChild('save_button',
                $this->getLayout()->createBlock('adminhtml/widget_button')
                    ->setData(array(
                        'label'     => Mage::helper('score')->__('Save Category'),
        ...
        if (!in_array($categoryId, $this->getRootIds()) && $category->isDeleteable()) {
            $this->setChild('delete_button',
                $this->getLayout()->createBlock('adminhtml/widget_button')
                    ->setData(array(
                        'label'     => Mage::helper('score')->__('Delete Category'),
...
//        $params = array('section'=>'score');
     ...
     * @param string $alias
     * @param array $config
     * @return Shaurmalab_Score_Block_Adminhtml_Score_Category_Edit_Form
     */
    ...
    public function getHeader()
    {
        if ($this->hasStoreRootCategory()) {
            if ($this->getCategoryId()) {
                ...
                if ($parentId && ($parentId != Shaurmalab_Score_Model_Category::TREE_ROOT_ID)) {
                    return Mage::helper('score')->__('New Subcategory');
                } else {
                    return Mage::helper('score')->__('New Root Category');
                }
            }
        }
        return Mage::helper('score')->__('Set Root Category for Store');
    }
    ...
    public function getOggettosJson()
    {
        $entitys = $this->getCategory()->getOggettosPosition();
        if (!empty($entitys)) {
            return Mage::helper('core')->jsonEncode($entitys);
        }
        return '{}';
    }
