 ...
 */
class Shaurmalab_Score_Block_Adminhtml_Score_Category_Tree extends Shaurmalab_Score_Block_Adminhtml_Score_Category_Abstract
{

    protected $_withOggettoCount;

    public function __construct()
    {
        parent::__construct();
        $this->setTemplate('score/category/tree.phtml');
        $this->setUseAjax(true);
        $this->_withOggettoCount = true;
    }
    ...
    protected function _prepareLayout()
    {
        ...
        $this->setChild('add_sub_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')
                ->setData(array(
                    'label'     => Mage::helper('score')->__('Add Subcategory'),
        ...
        if ($this->canAddRootCategory()) {
            $this->setChild('add_root_button',
                $this->getLayout()->createBlock('adminhtml/widget_button')
                    ->setData(array(
                        'label'     => Mage::helper('score')->__('Add Root Category'),
    ...
    {
        return Shaurmalab_Score_Model_Abstract::DEFAULT_STORE_ID;
    }
    ...
    public function getCategoryCollection()
    {
        ...
        if (is_null($collection)) {
            ...
            /* @var $collection Shaurmalab_Score_Model_Resource_Eav_Mysql4_Category_Collection */
            $collection->addAttributeToSelect('name')
                ->addAttributeToSelect('is_active')
                ->setOggettoStoreId($storeId)
                ->setLoadOggettoCount($this->_withOggettoCount)
                ->setStoreId($storeId);
    ...
    {
        return $this->getUrl('*/score_category/jsonTree');
    }
    ...
    {
        return $this->getUrl('*/score_category/move', array('store'=>$this->getRequest()->getParam('store')));
    }
    ...
    public function getBreadcrumbsJavascript($path, $javascriptVarName)
    {
        ...
        $categories = Mage::getResourceSingleton('score/category_tree')
    ...
    public function buildNodeName($node)
    {
        ...
        if ($this->_withOggettoCount) {
             $result .= ' (' . $node->getOggettoCount() . ')';
        }
        return $result;
    }
    ...
    protected function _isCategoryMoveable($node)
    {
        ...
        Mage::dispatchEvent('adminhtml_score_category_tree_is_moveable',
    ...
    public function canAddRootCategory()
    {
        ...
        Mage::dispatchEvent(
            'adminhtml_score_category_tree_can_add_root_category',
            array(
    ...
    public function canAddSubCategory()
    {
        $options = new Varien_Object(array('is_allow'=>true));
        Mage::dispatchEvent(
            'adminhtml_score_category_tree_can_add_sub_category',
            array(
                'category' => $this->getCategory(),
                'options'   => $options,
                'store'    => $this->getStore()->getId()
