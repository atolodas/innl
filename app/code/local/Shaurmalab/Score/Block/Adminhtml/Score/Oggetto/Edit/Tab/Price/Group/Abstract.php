 ...
 */
abstract class Shaurmalab_Score_Block_Adminhtml_Score_Oggetto_Edit_Tab_Price_Group_Abstract
    ...
    implements Varien_Data_Form_Element_Renderer_Interface
{
    ...
    /**
     * Retrieve current entity instance
     *
     ...
     *
     * @param Varien_Data_Form_Element_Abstract $element
     * @return Shaurmalab_Score_Block_Adminhtml_Score_Oggetto_Edit_Tab_Price_Group_Abstract
     */
    ...
    public function getCustomerGroups($groupId = null)
    {
        if ($this->_customerGroups === null) {
            if (!Mage::helper('score')->isModuleEnabled('Mage_Customer')) {
                return array();
            }
    ...
    public function getWebsites()
    {
        ...
        $this->_websites = array(
            0 => array(
                'name' => Mage::helper('score')->__('All Websites'),
                'currency' => Mage::app()->getBaseCurrencyCode()
            )
        );

        if (!$this->isScopeGlobal() && $this->getOggetto()->getStoreId()) {
            /** @var $website Mage_Core_Model_Website */
            $website = Mage::app()->getStore($this->getOggetto()->getStoreId())->getWebsite();
            ...
            $entityWebsiteIds  = $this->getOggetto()->getWebsiteIds();
            foreach ($websites as $website) {
                /** @var $website Mage_Core_Model_Website */
                if (!in_array($website->getId(), $entityWebsiteIds)) {
                    continue;
                }
                $this->_websites[$website->getId()] = array(
                    'name' => $website->getName(),
    ...
    public function getDefaultWebsite()
    {
        if ($this->isShowWebsiteColumn() && !$this->isAllowChangeWebsite()) {
            return Mage::app()->getStore($this->getOggetto()->getStoreId())->getWebsiteId();
        }
        return 0;
    }
    ...
    {
        if (!$this->isShowWebsiteColumn() || $this->getOggetto()->getStoreId()) {
            return false;
        }
        return true;
    }
