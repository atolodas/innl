...
/**
 * Oggetto list toolbar
 *
 * @category    Mage
 * @package     Shaurmalab_Score
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Shaurmalab_Score_Block_Oggetto_List_Toolbar extends Mage_Core_Block_Template
{
    /**
     * Oggettos collection
     *
    ...
    protected function _construct()
    {
        ...
        $this->_orderField  = Mage::getStoreConfig(
            Shaurmalab_Score_Model_Config::XML_PATH_LIST_DEFAULT_SORT_BY
        );

        $this->_availableOrder = $this->_getConfig()->getAttributeUsedForSortByArray();

        switch (Mage::getStoreConfig('score/frontend/list_mode')) {
            case 'grid':
                $this->_availableMode = array('grid' => $this->__('Grid'),'columns' => $this->__('Cols'));
                break;
            ...
            case 'grid-list':
                $this->_availableMode = array('grid' => $this->__('Grid'), 'list' =>  $this->__('List'),'columns' => $this->__('Cols'));
                break;

            case 'list-grid':
                $this->_availableMode = array('list' => $this->__('List'), 'grid' => $this->__('Grid'),'columns' => $this->__('Cols'));
                break;
        }
        $this->setTemplate('score/oggetto/list/toolbar.phtml');
    }
    ...
    /**
     * Return oggettos collection instance
     *
    ...
    /**
     * Get grit oggettos sort order field
     *
     * @return string
     */
    public function getCurrentOrder()
    {
        ...
        if ($order && isset($orders[$order])) {
            if ($order == $defaultOrder) {
                Mage::getSingleton('score/session')->unsSortOrder();
            } else {
                $this->_memorizeParam('sort_order', $order);
            }
        } else {
            $order = Mage::getSingleton('score/session')->getSortOrder();
        }
    ...
    public function getCurrentDirection()
    {
        ...
        if ($dir && in_array($dir, $directions)) {
            if ($dir == $this->_direction) {
                Mage::getSingleton('score/session')->unsSortDirection();
            } else {
                $this->_memorizeParam('sort_direction', $dir);
            }
        } else {
            $dir = Mage::getSingleton('score/session')->getSortDirection();
        }
        // validate direction
        if (!$dir || !in_array($dir, $directions)) {
            $dir = $this->_direction;
     ...
     *
     * @param string $order
     * @param Shaurmalab_Score_Block_Oggetto_List_Toolbar
     */
    ...
    public function getCurrentMode()
    {
        ...
        if ($mode) {
            if ($mode == $defaultMode) {
                Mage::getSingleton('score/session')->unsDisplayMode();
            } else {
                $this->_memorizeParam('display_mode', $mode);
            }
        } else {
            $mode = Mage::getSingleton('score/session')->getDisplayMode();
        }
    ...
    public function getDefaultPerPageValue()
    {
        if ($this->getCurrentMode() == 'list') {
            ...
            return Mage::getStoreConfig('score/frontend/list_per_page');
        }
        elseif ($this->getCurrentMode() == 'grid') {
            if ($default = $this->getDefaultGridPerPage()) {
                return $default;
            }
            return Mage::getStoreConfig('score/frontend/grid_per_page');
        }
        return 0;
    }
    ...
    protected function _getAvailableLimit($mode)
    {
        ...
        $perPageConfigKey = 'score/frontend/' . $mode . '_per_page_values';
        $perPageValues = (string)Mage::getStoreConfig($perPageConfigKey);
        $perPageValues = explode(',', $perPageValues);
        $perPageValues = array_combine($perPageValues, $perPageValues);
        if (Mage::getStoreConfigFlag('score/frontend/list_allow_all')) {
    ...
    /**
     * Get specified oggettos limit display per page
     *
     * @return string
     */
    public function getLimit()
    {
        ...
        if ($limit && isset($limits[$limit])) {
            if ($limit == $defaultLimit) {
                Mage::getSingleton('score/session')->unsLimitPage();
            } else {
                $this->_memorizeParam('limit_page', $limit);
            }
        } else {
            $limit = Mage::getSingleton('score/session')->getLimitPage();
        }
        if (!$limit) {
            $limit = $defaultLimit;
        }
    ...
    {
        $pagerBlock = $this->getChild('oggetto_list_toolbar_pager');
        ...
        return $pagerBlock->getClassName();
    }
}
