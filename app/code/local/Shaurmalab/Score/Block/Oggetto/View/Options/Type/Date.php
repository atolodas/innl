 ...
 */
class Shaurmalab_Score_Block_Oggetto_View_Options_Type_Date extends Shaurmalab_Score_Block_Oggetto_View_Options_Abstract
{
    ...
    {
        return Mage::getSingleton('score/oggetto_option_type_date')->useCalendar();
    }
    ...
    public function getCalendarDateHtml()
    {
        ...
        $value = $this->getOggetto()->getPreconfiguredValues()->getData('options/' . $option->getId() . '/date');

        //$require = $this->getOption()->getIsRequire() ? ' required-entry' : '';
        $require = '';

        $yearStart = Mage::getSingleton('score/oggetto_option_type_date')->getYearStart();
        $yearEnd = Mage::getSingleton('score/oggetto_option_type_date')->getYearEnd();

        $calendar = $this->getLayout()
            ->createBlock('core/html_date')
            ->setId('options_'.$this->getOption()->getId().'_date')
            ->setName('options['.$this->getOption()->getId().'][date]')
            ->setClass('oggetto-custom-option datetime-picker input-text' . $require)
    ...
    {
        $fieldsSeparator = '&nbsp;';
        $fieldsOrder = Mage::getSingleton('score/oggetto_option_type_date')->getConfigData('date_fields_order');
    ...
    {
        if (Mage::getSingleton('score/oggetto_option_type_date')->is24hTimeFormat()) {
            $hourStart = 0;
            $hourEnd = 23;
            $dayPartHtml = '';
        } else {
            ...
            $dayPartHtml = $this->_getHtmlSelect('day_part')
                ->setOptions(array(
                    'am' => Mage::helper('score')->__('AM'),
                    'pm' => Mage::helper('score')->__('PM')
                ))
                ->getHtml();
        }
    ...
    protected function _getHtmlSelect($name, $value = null)
    {
        ...
        $select = $this->getLayout()->createBlock('core/html_select')
            ...
            ->setClass('oggetto-custom-option datetime-picker' . $require)
            ->setExtraParams()
        ...
        if (is_null($value)) {
            $value = $this->getOggetto()->getPreconfiguredValues()->getData('options/' . $option->getId() . '/' . $name);
        }
        if (!is_null($value)) {
