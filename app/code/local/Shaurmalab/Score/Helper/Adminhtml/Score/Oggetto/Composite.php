 ...
 */
class Shaurmalab_Score_Helper_Helper_Adminhtml_Score_Oggetto_Composite extends Mage_Core_Helper_Abstract
{
     /**
    ...
    public function renderConfigureResult($controller, Varien_Object $configureResult)
    {
        try {
            ...
            if (!$product->getId()) {
                Mage::throwException($this->__('Oggetto is not loaded.'));
            }
            ...
            if ($buyRequest) {
                Mage::helper('score/oggetto')->prepareProductOptions($product, $buyRequest);
            }
