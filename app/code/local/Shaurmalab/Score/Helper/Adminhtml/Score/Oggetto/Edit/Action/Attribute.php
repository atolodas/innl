 ...
 */
class Shaurmalab_Score_Helper_Adminhtml_Score_Oggetto_Edit_Action_Attribute extends Mage_Core_Helper_Data
{
    /**
    ...
    public function getProducts()
    {
        if (is_null($this->_products)) {
            $productsIds = $this->getEntityIds();

            if (!is_array($productsIds)) {
                $productsIds = array(0);
            }

            $this->_products = Mage::getResourceModel('score/oggetto_collection')
    ...
    public function getEntityIds()
    {
        ...
        if ($this->_getRequest()->isPost() && $this->_getRequest()->getActionName() == 'edit') {
            $session->setProductIds($this->_getRequest()->getParam('entity', null));
        }
    ...
    public function getAttributes()
    {
        if (is_null($this->_attributes)) {
            ...
            $productTypeIds  = $this->getProducts()->getOggettoTypeIds();
