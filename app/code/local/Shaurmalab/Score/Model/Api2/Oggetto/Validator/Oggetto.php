...
/**
 * API2 score_oggetto Validator
 *
 * @category   Mage
 * @package    Shaurmalab_Score
 * @author     Magento Core Team <core@magentocommerce.com>
 */
class Shaurmalab_Score_Model_Api2_Oggetto_Validator_Oggetto extends Mage_Api2_Model_Resource_Validator
{
    /**
    ...
    {
        if (isset($options['oggetto'])) {
            if ($options['oggetto'] instanceof Shaurmalab_Score_Model_Oggetto) {
                $this->_oggetto = $options['oggetto'];
            } else {
                throw new Exception("Passed parameter 'oggetto' is wrong.");
            }
        }
    ...
    public function isValidData(array $data)
    {
        if ($this->_isUpdate()) {
            ...
            if (!is_null($oggetto) && $oggetto->getId()) {
                $data['attribute_set_id'] = $oggetto->getAttributeSetId();
                $data['type_id'] = $oggetto->getTypeId();
            }
        }

        try {
            $this->_validateOggettoType($data);
            /** @var $oggettoEntity Mage_Eav_Model_Entity_Type */
            $oggettoEntity = Mage::getModel('eav/entity_type')->loadByCode(Shaurmalab_Score_Model_Oggetto::ENTITY);
            $this->_validateAttributeSet($data, $oggettoEntity);
            ...
            $this->_validateAttributes($data, $oggettoEntity);
     ...
     *
     * @param array $data
     * @param Mage_Eav_Model_Entity_Type $oggettoEntity
     * @return array
     */
    protected function _validateAttributes($data, $oggettoEntity)
    {
        ...
        // see Shaurmalab_Score_Model_Oggetto_Attribute_Source_Msrp_Type_Price::getAllOptions()
        ...
        foreach ($oggettoEntity->getAttributeCollection($data['attribute_set_id']) as $attribute) {
            ...
            if (!$applicable && !$attribute->isStatic() && $isSet) {
                $oggettoTypes = Shaurmalab_Score_Model_Oggetto_Type::getTypes();
                $this->_addError(sprintf('Attribute "%s" is not applicable for oggetto type "%s"', $attributeCode,
                    $oggettoTypes[$data['type_id']]['label']));
            }
    ...
    /**
     * Validate oggetto type
     *
     * @param array $data
     * @return bool
     */
    protected function _validateOggettoType($data)
    {
        if ($this->_isUpdate()) {
            return true;
        }
        ...
        if (!array_key_exists($data['type_id'], Shaurmalab_Score_Model_Oggetto_Type::getTypes())) {
            $this->_critical('Invalid oggetto type.', Mage_Api2_Model_Server::HTTP_BAD_REQUEST);
        }
    }
     ...
     * @return bool
     */
    protected function _validateAttributeSet($data, $oggettoEntity)
    {
        if ($this->_isUpdate()) {
            return true;
        }
        ...
        if (!$attributeSet->getId() || $oggettoEntity->getEntityTypeId() != $attributeSet->getEntityTypeId()) {
    ...
    /**
     * Validate oggetto gift options data
     *
     * @param array $data
     */
    ...
    protected function _validateWebsiteIdForGroupPrice($data, $fieldSet)
    {
        if (!isset($data['website_id'])) {
            ...
            /** @var $catalogHelper Shaurmalab_Score_Helper_Data */
            $scoreHelper = Mage::helper('score');
            $website = Mage::getModel('core/website')->load($data['website_id']);
            $isAllWebsitesValue = is_numeric($data['website_id']) && ($data['website_id'] == 0);
            $isGlobalPriceScope = (int)$scoreHelper->getPriceScope() == Shaurmalab_Score_Helper_Data::PRICE_SCOPE_GLOBAL;
    ...
    /**
     * Validate oggetto inventory data
     *
     * @param array $data
     */
