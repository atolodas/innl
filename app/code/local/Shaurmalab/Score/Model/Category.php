 ...
 */
class Shaurmalab_Score_Model_Category extends Shaurmalab_Score_Model_Abstract
{
    /**
     * Entity code.
     * Can be used as part of method name for entity processing
     */
    const ENTITY                = 'score_category';
    /**
     * Category display modes
     */
    const DM_OGGETTO            = 'OGGETTOS';
    const DM_PAGE               = 'PAGE';
    const DM_MIXED              = 'OGGETTOS_AND_PAGE';
    const TREE_ROOT_ID          = 1;

    const CACHE_TAG             = 'score_category';
     ...
     * @var string
     */
    protected $_eventPrefix     = 'score_category';
    ...
    private $_designAttributes  = array(
        ...
        'custom_apply_to_oggettos'
    );

    /**
     * Category tree model
     *
     * @var Shaurmalab_Score_Model_Resource_Eav_Mysql4_Category_Tree
     */
     ...
     *
     * @var Shaurmalab_Score_Model_Category_Url
     */
    protected $_urlModel;
    ...
    protected function _construct()
    {
        ...
        if ($flatHelper->isAvailable() && !Mage::app()->getStore()->isAdmin() && $flatHelper->isBuilt(true)
            ...
            $this->_init('score/category_flat');
     ...
     *
     * @return Shaurmalab_Score_Model_Resource_Eav_Mysql4_Category_Tree
     */
    public function getTreeModel()
    {
        return Mage::getResourceModel('score/category_tree');
    }
    ...
    public function getTreeModelInstance()
    {
        if (is_null($this->_treeModel)) {
            $this->_treeModel = Mage::getResourceSingleton('score/category_tree');
        }
    ...
    public function move($parentId, $afterCategoryId)
    {
         ...
         */
        $parent = Mage::getModel('score/category')
            ->setStoreId($this->getStoreId())
            ->load($parentId);

        if (!$parent->getId()) {
            Mage::throwException(
                Mage::helper('score')->__('Category move operation is not possible: the new parent category was not found.')
            );
        }

        if (!$this->getId()) {
            Mage::throwException(
                Mage::helper('score')->__('Category move operation is not possible: the current category was not found.')
            );
        } elseif ($parent->getId() == $this->getId()) {
            Mage::throwException(
                Mage::helper('score')->__('Category move operation is not possible: parent category is equal to child category.')
            );
        }
        ...
        */
        ...
        try {
            /**
             * score_category_tree_move_before and score_category_tree_move_after
             * events declared for backward compatibility
             */
            Mage::dispatchEvent('score_category_tree_move_before', $eventParams);
            ...
            Mage::dispatchEvent('score_category_tree_move_after', $eventParams);
    ...
    /**
     * Get category oggettos collection
     *
    ...
    /**
     * Retrieve array of oggetto id's for category
     *
     * array($oggettoId => $position)
     *
     * @return array
     */
    public function getOggettosPosition()
    {
        if (!$this->getId()) {
            return array();
        }

        $array = $this->getData('oggettos_position');
        if (is_null($array)) {
            $array = $this->getResource()->getOggettosPosition($this);
            $this->setData('oggettos_position', $array);
        }
        return $array;
    }
    ...
    public function getUrlModel()
    {
        if ($this->_urlModel === null) {
            $this->_urlModel = Mage::getSingleton('score/factory')->getCategoryUrlInstance();
        }
    ...
    public function getCategoryIdUrl()
    {
        ...
        $url = $this->getUrlInstance()->getUrl('score/category/view', array(
            's'=>$urlKey,
            'id'=>$this->getId(),
        ));
    ...
    {
        $str = Mage::helper('score/oggetto_url')->format($str);
    ...
    public function getImageUrl()
    {
        ...
        if ($image = $this->getImage()) {
            $url = Mage::getBaseUrl('media').'score/category/'.$image;
        }
        return $url;
    }
    ...
    public function getUrlPath()
    {
        ...
        if ($this->getParentId()) {
            $parentPath = Mage::getModel('score/category')->load($this->getParentId())->getCategoryPath();
    ...
    public function getParentCategory()
    {
        if (!$this->hasData('parent_category')) {
            $this->setData('parent_category', Mage::getModel('score/category')->load($this->getParentId()));
        }
    ...
    private function _getAttribute($attributeCode)
    {
        if (!$this->_useFlatResource) {
            ...
            $attribute = Mage::getSingleton('score/config')
...
//        }
    ...
    /**
     * Retrieve count oggettos of category
     *
     * @return int
     */
    public function getOggettoCount()
    {
        if (!$this->hasOggettoCount()) {
            $count = $this->_getResource()->getOggettoCount($this); // load oggetto count
            $this->setData('oggetto_count', $count);
        }
        return $this->getData('oggetto_count');
    }
    ...
    /**
     * Retrieve Available int Oggetto Listing sort by
     *
     * @return null|array
     */
    ...
    /**
     * Retrieve Available Oggetto Listing  Sort By
    ...
    public function getAvailableSortByOptions() {
        ...
        $defaultSortBy   = Mage::getSingleton('score/config')
    ...
    /**
     * Retrieve Oggetto Listing Default Sort By
     *
     * @return string
     */
    public function getDefaultSortBy() {
        if (!$sortBy = $this->getData('default_sort_by')) {
            $sortBy = Mage::getSingleton('score/config')
                ->getOggettoListDefaultSortBy($this->getStoreId());
        }
