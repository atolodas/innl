...
/**
 * Score Custom Category design Model
 *
 * @category   Mage
 * @package    Shaurmalab_Score
 * @author     Magento Core Team <core@magentocommerce.com>
 */
class Shaurmalab_Score_Model_Design extends Mage_Core_Model_Abstract
{
    const APPLY_FOR_OGGETTO     = 1;
     ...
     */
    const CATEGORY_APPLY_CATEGORY_AND_OGGETTO_RECURSIVE = 1;
    const CATEGORY_APPLY_CATEGORY_ONLY                  = 2;
    const CATEGORY_APPLY_CATEGORY_AND_OGGETTO_ONLY      = 3;
    const CATEGORY_APPLY_CATEGORY_RECURSIVE             = 4;

    /**
     * Apply design from score object
     *
     * @deprecated after 1.4.2.0-beta1
     *
     * @param array|Shaurmalab_Score_Model_Category|Shaurmalab_Score_Model_Oggetto $object
     * @param int $calledFrom
     * @return Shaurmalab_Score_Model_Design
     */
    public function applyDesign($object, $calledFrom = 0)
    {
        if ($calledFrom != self::APPLY_FOR_CATEGORY && $calledFrom != self::APPLY_FOR_OGGETTO) {
            return $this;
        }

        // If Flat Data enabled then use it but only on frontend
        if (Mage::helper('score/category_flat')->isAvailable() && !Mage::app()->getStore()->isAdmin()) {
        ...
        if ($pass == 0) {
            switch ($applyForObject) {
                case self::APPLY_FOR_CATEGORY:
                    break;
                case self::APPLY_FOR_OGGETTO:
                    $validApplyTo = array(
                        self::CATEGORY_APPLY_CATEGORY_AND_OGGETTO_RECURSIVE,
                        self::CATEGORY_APPLY_CATEGORY_AND_OGGETTO_ONLY
                    );
    ...
    protected function _inheritDesign($object, $calledFrom = 0)
    {
        ...
        if ($object instanceof Shaurmalab_Score_Model_Oggetto) {
        ...
        elseif ($object instanceof Shaurmalab_Score_Model_Category) {
            ...
            if ($useParentSettings) {
                if ($category &&
                    $category->getId() &&
                    $category->getLevel() > 1 &&
                    $category->getId() != Shaurmalab_Score_Model_Category::TREE_ROOT_ID) {
                    return $this->_inheritDesign($category, $calledFrom);
                }
            }

            if ($calledFrom == self::APPLY_FOR_OGGETTO) {
                $applyToOggettos = $object->getCustomApplyToOggettos();
                if (!$applyToOggettos) {
                    return $this;
                }
            }
    ...
    protected function _applyDesign($designUpdateData, $calledFrom = 0, $loaded = false, $pass = 0)
    {
        ...
        if (false === $loaded && is_object($designUpdateData)) {
            ...
            if ($designUpdateData instanceof Shaurmalab_Score_Model_Oggetto) {
                $_category = $designUpdateData->getCategory();
                $_designUpdateData = array_merge(
                    $_designUpdateData, array($_category)
                );
                $pass --;
            } elseif ($designUpdateData instanceof Shaurmalab_Score_Model_Category) {
    ...
    public function getDesignSettings($object)
    {
        ...
        if ($object instanceof Shaurmalab_Score_Model_Oggetto) {
            if ($category && $category->getCustomApplyToOggettos()) {
    ...
    /**
     * Extract custom layout settings from category or oggetto object
     *
     * @param Shaurmalab_Score_Model_Category|Shaurmalab_Score_Model_Oggetto $object
     * @return Varien_Object
     */
     ...
     * @param Varien_Object $oggettoSettings
     * @return Varien_Object
     */
    protected function _mergeSettings($categorySettings, $oggettoSettings)
    {
        if ($oggettoSettings->getCustomDesign()) {
            $categorySettings->setCustomDesign($oggettoSettings->getCustomDesign());
        }
        if ($oggettoSettings->getPageLayout()) {
            $categorySettings->setPageLayout($oggettoSettings->getPageLayout());
        }
        if ($oggettoSettings->getLayoutUpdates()) {
            $update = array_merge($categorySettings->getLayoutUpdates(), $oggettoSettings->getLayoutUpdates());
