...
/**
 * @method Shaurmalab_Score_Model_Layer_Filter_Price setInterval(array)
 * @method array getInterval()
 */
class Shaurmalab_Score_Model_Layer_Filter_Price extends Shaurmalab_Score_Model_Layer_Filter_Abstract
{
    /**
     * XML configuration paths for Price Layered Navigation
     */
    const XML_PATH_RANGE_CALCULATION       = 'score/layered_navigation/price_range_calculation';
    const XML_PATH_RANGE_STEP              = 'score/layered_navigation/price_range_step';
    const XML_PATH_RANGE_MAX_INTERVALS     = 'score/layered_navigation/price_range_max_intervals';
    const XML_PATH_ONE_PRICE_INTERVAL      = 'score/layered_navigation/one_price_interval';
    const XML_PATH_INTERVAL_DIVISION_LIMIT = 'score/layered_navigation/interval_division_limit';

    /**
     * Price layered navigation modes: Automatic (equalize price ranges), Automatic (equalize oggetto counts), Manual
     */
    const RANGE_CALCULATION_AUTO     = 'auto'; // equalize price ranges
    const RANGE_CALCULATION_IMPROVED = 'improved'; // equalize oggetto counts
     ...
     * Resource instance
     *
     * @var Shaurmalab_Score_Model_Resource_Eav_Mysql4_Layer_Filter_Price
     */
    protected $_resource;
     ...
     *
     * @return Shaurmalab_Score_Model_Resource_Eav_Mysql4_Layer_Filter_Price
     */
    protected function _getResource()
    {
        if (is_null($this->_resource)) {
            $this->_resource = Mage::getResourceModel('score/layer_filter_price');
        }
    ...
    /**
     * Get maximum price from layer oggettos set
     *
     * @return float
     */
    public function getMaxPriceInt()
    {
        ...
        if (is_null($maxPrice)) {
            $maxPrice = $this->getLayer()->getOggettoCollection()->getMaxPrice();
    ...
    /**
     * Get information about oggettos count in range
     *
     * @param   int $range
     * @return  int
     */
    ...
    protected function _renderItemLabel($range, $value)
    {
        ...
        return Mage::helper('score')->__('%s - %s', $fromPrice, $toPrice);
    }
    ...
    protected function _renderRangeLabel($fromPrice, $toPrice)
    {
        ...
        if ($toPrice === '') {
            return Mage::helper('score')->__('%s and above', $formattedFromPrice);
            ...
            return Mage::helper('score')->__('%s - %s', $formattedFromPrice, $store->formatPrice($toPrice));
        }
    }
    ...
    {
        /** @var $algorithmModel Shaurmalab_Score_Model_Layer_Filter_Price_Algorithm */
        $algorithmModel = Mage::getSingleton('score/layer_filter_price_algorithm');
        $collection = $this->getLayer()->getOggettoCollection();
        $appliedInterval = $this->getInterval();
        if ($appliedInterval
     ...
     *
     * @return Shaurmalab_Score_Model_Layer_Filter_Price
     */
    ...
    /**
     * Apply filter value to oggetto collection based on filter range and selected value
     *
     * @deprecated since 1.7.0.0
     * @param int $range
     * @param int $index
    ...
    public function getClearLinkText()
    {
        if (Mage::app()->getStore()->getConfig(self::XML_PATH_RANGE_CALCULATION) == self::RANGE_CALCULATION_IMPROVED
            ...
            return Mage::helper('score')->__('Clear Price');
        }
