...
/**
 * Score oggetto attribute api
 *
 * @category   Mage
 * @package    Shaurmalab_Score
 * @author     Magento Core Team <core@magentocommerce.com>
 */
class Shaurmalab_Score_Model_Oggetto_Attribute_Api extends Shaurmalab_Score_Model_Api_Resource
{
    /**
     * Oggetto entity type id
     *
     * @var int
     */
    ...
    public function __construct()
    {
        ...
        $this->_entityTypeId = Mage::getModel('eav/entity')->setType('score_oggetto')->getTypeId();
    }
    ...
    public function options($attributeId, $store = null)
    {
        ...
        $attribute = Mage::getModel('score/oggetto')
                ->setStoreId($storeId)
                ->getResource()
    ...
    {
        return Mage::getModel('score/oggetto_attribute_source_inputtype')->toOptionArray();
    }

    /**
     * Create new oggetto attribute
     *
    ...
    {
        /** @var $model Shaurmalab_Score_Model_Resource_Eav_Attribute */
    ...
    public function info($attribute)
    {
        ...
        $result = array(
            ...
            'used_in_oggetto_listing' => $model->getUsedInOggettoListing(),
    ...
    public function addOption($attribute, $data)
    {
        ...
        $helperScore = Mage::helper('score');

        $optionLabels = array();
        foreach ($data['label'] as $label) {
            $storeId = $label['store_id'];
            $labelText = $helperScore->stripTags($label['value']);
            if (is_array($storeId)) {
        ...
        // Mage_Adminhtml_Catalog_Oggetto_AttributeController::saveAction()
        $modelData = array(
            'option' => array(
                'value' => array(
    ...
    protected function _prepareDataForSave(&$data)
    {
        ...
        if ($data['scope'] == 'global') {
            $data['is_global'] = Shaurmalab_Score_Model_Resource_Eav_Attribute::SCOPE_GLOBAL;
        } else if ($data['scope'] == 'website') {
            $data['is_global'] = Shaurmalab_Score_Model_Resource_Eav_Attribute::SCOPE_WEBSITE;
        } else {
            $data['is_global'] = Shaurmalab_Score_Model_Resource_Eav_Attribute::SCOPE_STORE;
        }
        ...
        if (isset($data['frontend_label']) && is_array($data['frontend_label'])) {
            ...
            foreach ($data['frontend_label'] as $label) {
                $storeId = $label['store_id'];
                $labelText = $helperScore->stripTags($label['label']);
        ...
        if (!empty($data['default_value'])) {
            $data['default_value'] = $helperScore->stripTags($data['default_value']);
        }
    }
    ...
    {
        $model = Mage::getResourceModel('score/eav_attribute')
...
} // Class Shaurmalab_Score_Model_Oggetto_Attribute_Api End
