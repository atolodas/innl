...
/**
 * Score oggetto media gallery attribute backend model
 *
 * @category   Mage
 * @package    Shaurmalab_Score
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Shaurmalab_Score_Model_Oggetto_Attribute_Backend_Media extends Mage_Eav_Model_Entity_Attribute_Backend_Abstract
{
    protected $_renamedImages = array();

    /**
     * Load attribute data after oggetto loaded
     *
    ...
    public function beforeSave($object)
    {
        ...
        Mage::dispatchEvent('score_oggetto_media_save_before', array('oggetto' => $object, 'images' => $value));
    ...
    public function afterSave($object)
    {
        ...
        $images = Mage::getResourceModel('score/oggetto')
     ...
     * @return string
     */
    public function addImage(Shaurmalab_Score_Model_Oggetto $oggetto, $file,
        $mediaAttribute = null, $move = false, $exclude = true)
    {
        $file = realpath($file);

        if (!$file || !file_exists($file)) {
            Mage::throwException(Mage::helper('score')->__('Image does not exist.'));
        }

        Mage::dispatchEvent('score_oggetto_media_add_image', array('oggetto' => $oggetto, 'image' => $file));

        $pathinfo = pathinfo($file);
        $imgExtensions = array('jpg','jpeg','gif','png');
        if (!isset($pathinfo['extension']) || !in_array(strtolower($pathinfo['extension']), $imgExtensions)) {
            Mage::throwException(Mage::helper('score')->__('Invalid image file type.'));
        }
        ...
        try {
            ...
            Mage::throwException(Mage::helper('score')->__('Failed to move file: %s', $e->getMessage()));
        }

        $fileName = str_replace(DS, '/', $fileName);

        $attrCode = $this->getAttribute()->getAttributeCode();
        $mediaGalleryData = $oggetto->getData($attrCode);
        $position = 0;
        ...
        $oggetto->setData($attrCode, $mediaGalleryData);

        if (!is_null($mediaAttribute)) {
            $this->setMediaAttribute($oggetto, $mediaAttribute, $fileName);
        }
     ...
     */
    public function addImagesWithDifferentMediaAttributes(Shaurmalab_Score_Model_Oggetto $oggetto,
        ...
        foreach ($fileAndAttributesArray as $key => $value) {
            ...
            if ($keyInAddedFiles === false) {
                $savedFileName = $this->addImage($oggetto, $filePath . $value['file'], null, $move, $exclude);
            ...
            if (!is_null($value['mediaAttribute'])) {
                $this->setMediaAttribute($oggetto, $value['mediaAttribute'], $savedFileName);
            }
     ...
     */
    public function updateImage(Shaurmalab_Score_Model_Oggetto $oggetto, $file, $data)
    {
        $fieldsMap = array(
            'label'    => 'label',
            'position' => 'position',
     ...
     * @return Shaurmalab_Score_Model_Oggetto_Attribute_Backend_Media
     */
    public function removeImage(Shaurmalab_Score_Model_Oggetto $oggetto, $file)
    {
        $attrCode = $this->getAttribute()->getAttributeCode();
     ...
     */
    public function getImage(Shaurmalab_Score_Model_Oggetto $oggetto, $file)
    {
        $attrCode = $this->getAttribute()->getAttributeCode();
        $mediaGalleryData = $oggetto->getData($attrCode);
        if (!isset($mediaGalleryData['images']) || !is_array($mediaGalleryData['images'])) {
     ...
     * @return Shaurmalab_Score_Model_Oggetto_Attribute_Backend_Media
     */
    public function clearMediaAttribute(Shaurmalab_Score_Model_Oggetto $oggetto, $mediaAttribute)
    {
        $mediaAttributeCodes = array_keys($oggetto->getMediaAttributes());

        if (is_array($mediaAttribute)) {
            foreach ($mediaAttribute as $atttribute) {
                if (in_array($atttribute, $mediaAttributeCodes)) {
                    $oggetto->setData($atttribute, null);
                }
            }
        } elseif (in_array($mediaAttribute, $mediaAttributeCodes)) {
            $oggetto->setData($mediaAttribute, null);
        }
     ...
     * @return Shaurmalab_Score_Model_Oggetto_Attribute_Backend_Media
     */
    public function setMediaAttribute(Shaurmalab_Score_Model_Oggetto $oggetto, $mediaAttribute, $value)
    {
        $mediaAttributeCodes = array_keys($oggetto->getMediaAttributes());
        ...
        if (is_array($mediaAttribute)) {
            foreach ($mediaAttribute as $atttribute) {
                if (in_array($atttribute, $mediaAttributeCodes)) {
                    $oggetto->setData($atttribute, $value);
                }
            }
        } elseif (in_array($mediaAttribute, $mediaAttributeCodes)) {
            $oggetto->setData($mediaAttribute, $value);
        }
     ...
     *
     * @return Shaurmalab_Score_Model_Resource_Eav_Mysql4_Oggetto_Attribute_Backend_Media
     */
    protected function _getResource()
    {
        return Mage::getResourceSingleton('score/oggetto_attribute_backend_media');
    }
    ...
    protected function _getUniqueFileName($file, $dirsep) {
        if (Mage::helper('core/file_storage_database')->checkDbUsage()) {
            $destFile = Mage::helper('core/file_storage_database')
                ->getUniqueFilename(
                    Mage::getSingleton('score/oggetto_media_config')->getBaseMediaUrlAddition(),
                    $file
                );
        } else {
    ...
    protected function _copyImage($file)
    {
        try {
            ...
            Mage::throwException(
                Mage::helper('score')->__('Failed to copy file %s. Please, delete media with non-existing images and try again.', $file)
            );
        }
...
} // Class Shaurmalab_Score_Model_Oggetto_Attribute_Backend_Media End
