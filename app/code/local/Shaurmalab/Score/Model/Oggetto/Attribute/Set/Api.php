...
/**
 * Score oggetto attribute set api
 *
 * @category   Mage
 * @package    Shaurmalab_Score
 * @author     Magento Core Team <core@magentocommerce.com>
 */
class Shaurmalab_Score_Model_Oggetto_Attribute_Set_Api extends Mage_Api_Model_Resource_Abstract
{
    /**
    ...
    {
        $entityType = Mage::getModel('score/oggetto')->getResource()->getEntityType();
    ...
    public function create($attributeSetName, $skeletonSetId)
    {
        ...
        // get score oggetto entity type id
        $entityTypeId = Mage::getModel('score/oggetto')->getResource()->getTypeId();
     ...
     * @param bool $forceOggettosRemove
     * @return bool
     */
    public function remove($attributeSetId, $forceOggettosRemove = false)
    {
        // if attribute set has related goods and $forceOggettosRemove is not set throw exception
        if (!$forceOggettosRemove) {
            /** @var $scoreOggettosCollection Shaurmalab_Score_Model_Resource_Eav_Mysql4_Oggetto_Collection */
            $scoreOggettosCollection = Mage::getModel('score/oggetto')->getCollection()
                    ->addFieldToFilter('attribute_set_id', $attributeSetId);
            if (count($scoreOggettosCollection)) {
                $this->_fault('attribute_set_has_related_oggettos');
            }
        }
    ...
    public function groupAdd($attributeSetId, $groupName)
    {
        ...
        $group->setAttributeSetId($attributeSetId)
                ->setAttributeGroupName(
                    Mage::helper('score')->stripTags($groupName)
                );
        if ($group->itemExists()) {
    ...
    /**
       ...
       {
           /** @var $group Shaurmalab_Score_Model_Oggetto_Attribute_Group */
           $group = Mage::getModel('score/oggetto_attribute_group')->load($attributeGroupId);
           if (!$group->getId()) {
               $this->_fault('invalid_attribute_group_id');
           }
...
} // Class Shaurmalab_Score_Model_Oggetto_Attribute_Set_Api End
