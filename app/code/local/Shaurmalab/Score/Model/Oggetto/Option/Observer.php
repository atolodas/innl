...
/**
 * Score Custom Options Observer
 *
 * @category    Mage
 * @package     Shaurmalab_Score
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Shaurmalab_Score_Model_Oggetto_Option_Observer
{
    /**
     * Copy quote custom option files to order custom option files
     *
     * @param Varien_Object $observer
     * @return Shaurmalab_Score_Model_Oggetto_Option_Observer
     */
    public function copyQuoteFilesToOrderFiles($observer)
    {
        ...
        if (is_array($quoteItem->getOptions())) {
            foreach ($quoteItem->getOptions() as $itemOption) {
                ...
                if (isset($code[1]) && is_numeric($code[1]) && ($option = $quoteItem->getOggetto()->getOptionById($code[1]))) {
                    if ($option->getType() == Shaurmalab_Score_Model_Oggetto_Option::OPTION_TYPE_FILE) {
