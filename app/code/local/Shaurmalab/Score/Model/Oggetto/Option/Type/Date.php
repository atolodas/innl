...
/**
 * Score oggetto option date type
 *
 * @category   Mage
 * @package    Shaurmalab_Score
 * @author     Magento Core Team <core@magentocommerce.com>
 */
class Shaurmalab_Score_Model_Oggetto_Option_Type_Date extends Shaurmalab_Score_Model_Oggetto_Option_Type_Default
{
    /**
    ...
    public function validateUserValue($values)
    {
        ...
        if ($isValid) {
            ...
            if (!$dateValid) {
                Mage::throwException(Mage::helper('score')->__('Please specify date required option(s).'));
            } elseif (!$timeValid) {
                Mage::throwException(Mage::helper('score')->__('Please specify time required option(s).'));
            } else {
    ...
    public function getFormattedOptionValue($optionValue)
    {
        if ($this->_formattedOptionValue === null) {
            ...
            $option = $this->getOption();
            if ($this->getOption()->getType() == Shaurmalab_Score_Model_Oggetto_Option::OPTION_TYPE_DATE) {
                $format = Mage::app()->getLocale()->getDateFormat(Mage_Core_Model_Locale::FORMAT_TYPE_MEDIUM);
                $result = Mage::app()->getLocale()->date($optionValue, Zend_Date::ISO_8601, null, false)
                    ->toString($format);
            } elseif ($this->getOption()->getType() == Shaurmalab_Score_Model_Oggetto_Option::OPTION_TYPE_DATE_TIME) {
            ...
            } elseif ($this->getOption()->getType() == Shaurmalab_Score_Model_Oggetto_Option::OPTION_TYPE_TIME) {
     ...
     * @return Shaurmalab_Score_Model_Oggetto_Option_Type_Date
     */
    ...
    protected function _dateExists()
    {
        return in_array($this->getOption()->getType(), array(
            Shaurmalab_Score_Model_Oggetto_Option::OPTION_TYPE_DATE,
            Shaurmalab_Score_Model_Oggetto_Option::OPTION_TYPE_DATE_TIME
        ));
    }
    ...
    protected function _timeExists()
    {
        return in_array($this->getOption()->getType(), array(
            Shaurmalab_Score_Model_Oggetto_Option::OPTION_TYPE_DATE_TIME,
            Shaurmalab_Score_Model_Oggetto_Option::OPTION_TYPE_TIME
        ));
    }
}
