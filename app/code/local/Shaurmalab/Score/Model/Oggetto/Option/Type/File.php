...
/**
 * Score oggetto option file type
 *
 * @category   Mage
 * @package    Shaurmalab_Score
 * @author     Magento Core Team <core@magentocommerce.com>
 */
class Shaurmalab_Score_Model_Oggetto_Option_Type_File extends Shaurmalab_Score_Model_Oggetto_Option_Type_Default
{
    /**
     ...
     * @return Shaurmalab_Score_Model_Oggetto_Option_Type_File
     */
    ...
    protected function _validateUploadedFile()
    {
        ...
        try {
            ...
            if (isset($_SERVER['CONTENT_LENGTH']) && $_SERVER['CONTENT_LENGTH'] > $this->_getUploadMaxFilesize()) {
                ...
                Mage::throwException(
                    Mage::helper('score')->__("The file you uploaded is larger than %s Megabytes allowed by server", $value)
                );
            } else {
                switch($this->getProcessMode())
                {
                    case Shaurmalab_Score_Model_Oggetto_Type_Abstract::PROCESS_MODE_FULL:
                        Mage::throwException(
                            Mage::helper('score')->__('Please specify the oggetto\'s required option(s).')
                        );
                        break;
                    default:
                        $this->setUserValue(null);
        ...
        if ($upload->isUploaded($file) && $upload->isValid($file)) {
            ...
            $this->getOggetto()->getTypeInstance(true)->addFileQueue(array(
    ...
    protected function _validateFile($optionValue)
    {
        $option = $this->getOption();
        /**
         * @see Shaurmalab_Score_Model_Oggetto_Option_Type_File::_validateUploadFile()
    ...
    protected function _getValidatorErrors($errors, $fileInfo)
    {
        ...
        foreach ($errors as $errorCode) {
            if ($errorCode == Zend_Validate_File_ExcludeExtension::FALSE_EXTENSION) {
                $result[] = Mage::helper('score')->__("The file '%s' for '%s' has an invalid extension", $fileInfo['title'], $option->getTitle());
            ...
            {
                $result[] = Mage::helper('score')->__("Maximum allowed image size for '%s' is %sx%s px.", $option->getTitle(), $option->getImageSizeX(), $option->getImageSizeY());
            } elseif ($errorCode == Zend_Validate_File_FilesSize::TOO_BIG) {
                $result[] = Mage::helper('score')->__("The file '%s' you uploaded is larger than %s Megabytes allowed by server", $fileInfo['title'], $this->_bytesToMbytes($this->_getUploadMaxFilesize()));
            }
        }
        return $result;
    }
    ...
    protected function _getOptionHtml($optionValue)
    {
        ...
        try {
            if (isset($value) && isset($value['width']) && isset($value['height'])
                ...
                $sizes = $value['width'] . ' x ' . $value['height'] . ' ' . Mage::helper('score')->__('px.');
            } else {
                $sizes = '';
            }
            ...
            Mage::throwException(Mage::helper('score')->__("File options format is not valid."));
        }
    }
