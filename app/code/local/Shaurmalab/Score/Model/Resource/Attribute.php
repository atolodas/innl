...
/**
 * Score attribute resource model
 *
 * @category    Mage
 * @package     Shaurmalab_Score
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Shaurmalab_Score_Model_Resource_Attribute extends Mage_Eav_Model_Resource_Entity_Attribute
{
    /**
     * Perform actions before object save
     *
     * @param Mage_Core_Model_Abstract $object
     * @return Shaurmalab_Score_Model_Resource_Attribute
     */
    ...
    protected function _clearUselessAttributeValues(Mage_Core_Model_Abstract $object)
    {
        ...
        if ($object->isScopeGlobal()
            ...
            && Shaurmalab_Score_Model_Resource_Eav_Attribute::SCOPE_GLOBAL != $origData['is_global']
        ) {
    ...
    public function deleteEntity(Mage_Core_Model_Abstract $object)
    {
        ...
        if ($result) {
            $attribute = Mage::getSingleton('eav/config')
                ->getAttribute(Shaurmalab_Score_Model_Oggetto::ENTITY, $result['attribute_id']);

            if ($this->isUsedBySuperOggettos($attribute, $result['attribute_set_id'])) {
                Mage::throwException(Mage::helper('eav')->__("Attribute '%s' used in configurable oggettos", $attribute->getAttributeCode()));
            }
    ...
    /**
     * Defines is Attribute used by super oggettos
     *
     * @param Mage_Core_Model_Abstract $object
     * @param int $attributeSet
     * @return int
     */
    public function isUsedBySuperOggettos(Mage_Core_Model_Abstract $object, $attributeSet = null)
    {
        $adapter      = $this->_getReadAdapter();
        $attrTable    = $this->getTable('score/oggetto_super_attribute');
        $oggettoTable = $this->getTable('score/oggetto');

        $bind = array('attribute_id' => $object->getAttributeId());
        $select = clone $adapter->select();
        $select->reset()
            ->from(array('main_table' => $attrTable), array('psa_count' => 'COUNT(oggetto_super_attribute_id)'))
            ->join(array('entity' => $oggettoTable), 'main_table.oggetto_id = entity.entity_id')
