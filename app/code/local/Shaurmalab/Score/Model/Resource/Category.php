 ...
 */
class Shaurmalab_Score_Model_Resource_Category extends Shaurmalab_Score_Model_Resource_Abstract
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->setType(Shaurmalab_Score_Model_Category::ENTITY)
            ->setConnection(
                $resource->getConnection('score_read'),
                $resource->getConnection('score_write')
            );
     ...
     * @return Shaurmalab_Score_Model_Resource_Category
     */
    ...
    {
        if (!$this->_tree) {
            $this->_tree = Mage::getResourceModel('score/category_tree')
                ->load();
        }
        return $this->_tree;
    }
     ...
     * save related oggettos ids and update path value
     *
     * @param Varien_Object $object
     * @return Shaurmalab_Score_Model_Resource_Category
     */
    protected function _afterSave(Varien_Object $object)
    {
        ...
        $this->_saveCategoryOggettos($object);
     ...
     * Update path field
     *
     * @param Shaurmalab_Score_Model_Category $object
     * @return Shaurmalab_Score_Model_Resource_Category
     */
    ...
    protected function _getMaxPosition($path)
    {
        ...
        $select  = $adapter->select()
            ->from($this->getTable('score/category'), 'MAX(' . $positionField . ')')
    ...
    /**
     * Save category oggettos relation
     *
     * @param Shaurmalab_Score_Model_Category $category
     * @return Shaurmalab_Score_Model_Resource_Category
     */
    protected function _saveCategoryOggettos($category)
    {
        $category->setIsChangedOggettoList(false);
        $id = $category->getId();
        /**
         * new category-oggetto relationships
         */
        $oggettos = $category->getPostedOggettos();
        ...
        /**
         * old category-oggetto relationships
         */
        $oldOggettos = $category->getOggettosPosition();

        $insert = array_diff_key($oggettos, $oldOggettos);
        $delete = array_diff_key($oldOggettos, $oggettos);

        /**
         * Find oggetto ids which are presented in both arrays
         * and saved before (check $oldOggettos array)
         */
        $update = array_intersect_key($oggettos, $oldOggettos);
        $update = array_diff_assoc($update, $oldOggettos);

        $adapter = $this->_getWriteAdapter();

        /**
         * Delete oggettos from category
         */
        if (!empty($delete)) {
            $cond = array(
                'oggetto_id IN(?)' => array_keys($delete),
                'category_id=?' => $id
            );
            $adapter->delete($this->_categoryOggettoTable, $cond);
        }

        /**
         * Add oggettos to category
         */
        if (!empty($insert)) {
            $data = array();
            foreach ($insert as $oggettoId => $position) {
                $data[] = array(
                    'category_id' => (int)$id,
            ...
            $adapter->insertMultiple($this->_categoryOggettoTable, $data);
        }

        /**
         * Update oggetto positions in category
         */
        if (!empty($update)) {
            foreach ($update as $oggettoId => $position) {
                $where = array(
                    'category_id = ?'=> (int)$id,
                    'oggetto_id = ?' => (int)$oggettoId
                );
                $bind  = array('position' => (int)$position);
                $adapter->update($this->_categoryOggettoTable, $bind, $where);
            }
        }

        if (!empty($insert) || !empty($delete)) {
            $oggettoIds = array_unique(array_merge(array_keys($insert), array_keys($delete)));
            Mage::dispatchEvent('score_category_change_oggettos', array(
                'category'      => $category,
        ...
        if (!empty($insert) || !empty($update) || !empty($delete)) {
            $category->setIsChangedOggettoList(true);

            /**
             * Setting affected oggettos to category for third party engine index refresh
             */
            $oggettoIds = array_keys($insert + $delete + $update);
            $category->setAffectedOggettoIds($oggettoIds);
        }
        return $this;
    }

    /**
     * Get positions of associated to category oggettos
     *
     * @param Shaurmalab_Score_Model_Category $category
     * @return array
     */
    public function getOggettosPosition($category)
    {
        $select = $this->_getWriteAdapter()->select()
            ->from($this->_categoryOggettoTable, array('oggetto_id', 'position'))
    ...
    protected function _getIsActiveAttributeId()
    {
        if ($this->_isActiveAttributeId === null) {
            $bind = array(
                'score_category' => Shaurmalab_Score_Model_Category::ENTITY,
                'is_active'        => 'is_active',
            );
            $select = $this->_getReadAdapter()->select()
                ...
                ->where('entity_type_code = :score_category')
    ...
    {
        $oggettoTable = Mage::getSingleton('core/resource')->getTableName('score/category_oggetto');

        $select = $this->getReadConnection()->select()
            ->from(
                array('main_table' => $oggettoTable),
                array(new Zend_Db_Expr('COUNT(main_table.oggetto_id)'))
            )
     ...
     * @return Varien_Data_Tree_Node_Collection|Shaurmalab_Score_Model_Resource_Category_Collection
     */
    public function getCategories($parent, $recursionLevel = 0, $sorted = false, $asCollection = false, $toLoad = true)
    {
        $tree = Mage::getResourceModel('score/category_tree');
        /* @var $tree Shaurmalab_Score_Model_Resource_Category_Tree */
    ...
    public function getParentCategories($category)
    {
        ...
        $categories = Mage::getResourceModel('score/category_collection')
    ...
    public function getParentDesignCategory($category)
    {
        ...
        $collection = $category->getCollection()
            ...
            ->addAttributeToSelect('custom_apply_to_oggettos')
            ->addFieldToFilter('entity_id', array('in' => $pathIds))
     ...
     *
     * @param Shaurmalab_Score_Model_Category $category
     * @param Shaurmalab_Score_Model_Category $newParent
     * @param null|int $afterCategoryId
     * @return Shaurmalab_Score_Model_Resource_Category
     */
    public function changeParent(Shaurmalab_Score_Model_Category $category, Shaurmalab_Score_Model_Category $newParent,
