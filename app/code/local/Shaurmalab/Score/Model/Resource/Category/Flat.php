 ...
 */
class Shaurmalab_Score_Model_Resource_Category_Flat extends Mage_Index_Model_Resource_Abstract
{
    /**
    ...
    /**
     * Store flag which defines if Score Category Flat Data has been initialized
     *
     * @var array
     */
    ...
    {
        $this->_init('score/category_flat', 'entity_id');
    }
     ...
     * @return Shaurmalab_Score_Model_Resource_Category_Flat
     */
     ...
     * @return string
     */
    public function getMainStoreTable($storeId = Shaurmalab_Score_Model_Abstract::DEFAULT_STORE_ID)
    {
        ...
        if ($this->getUseStoreTables() && $storeId) {
            ...
            $table = $this->getTable(array('score/category_flat', $suffix));
        } else {
    ...
    protected function _initInactiveCategoryIds()
    {
        ...
        Mage::dispatchEvent('score_category_tree_init_inactive_category_ids', array('tree' => $this));
        return $this;
    }
     ...
     *
     * @param Shaurmalab_Score_Model_Category|int $parentNode
    ...
    protected function _loadNodes($parentNode = null, $recursionLevel = 0, $storeId = 0, $onlyActive = true)
    {
        ...
        $startLevel = 1;
        $parentPath = '';
        if ($parentNode instanceof Shaurmalab_Score_Model_Category) {
        ...
        /** @var $urlRewrite Shaurmalab_Score_Helper_Category_Url_Rewrite_Interface */
        ...
        Mage::dispatchEvent('score_category_flat_loadnodes_before', array('select' => $select));

        $arrNodes = $_conn->fetchAll($select);
        $nodes = array();
        foreach ($arrNodes as $node) {
            $node['id'] = $node['entity_id'];
            $nodes[$node['id']] = Mage::getModel('score/category')->setData($node);
        }
    ...
    public function getNodes($parentId, $recursionLevel = 0, $storeId = 0)
    {
        if (!$this->_loaded) {
            ...
            if ($parentNode = $this->_getReadAdapter()->fetchRow($selectParent)) {
                ...
                $parentNode = Mage::getModel('score/category')->setData($parentNode);
    ...
    public function rebuild($stores = null)
    {
        ...
        $rootId = Shaurmalab_Score_Model_Category::TREE_ROOT_ID;
        $categories = array();
        $categoriesIds = array();
    ...
    protected function _createTable($store)
    {
        ...
        $table->addForeignKey(
            $_writeAdapter->getForeignKeyName(
                $tableName, 'entity_id', $this->getTable('score/category'), 'entity_id'
            ),
            'entity_id', $this->getTable('score/category'), 'entity_id',
    ...
    {
        $helper = Mage::getResourceHelper('score');
        $columns = array();
        $columnsToSkip = array('entity_type_id', 'attribute_set_id');
        $describe = $this->_getWriteAdapter()->describeTable($this->getTable('score/category'));
    ...
    /**
     * Return array of attribute codes for entity type 'score_category'
     *
     * @return array
     */
    protected function _getAttributes()
    {
        if ($this->_attributeCodes === null) {
            $select = $this->_getWriteAdapter()->select()
                ...
                ->where(
                    $this->getTable('eav/entity_type') . '.entity_type_code = ?', Shaurmalab_Score_Model_Category::ENTITY
                );
    ...
    protected function _getAttributeTypeValues($type, $entityIds, $storeId)
    {
        $select = $this->_getWriteAdapter()->select()
            ->from(
                array('def' => $this->getTable(array('score/category', $type))),
                array('entity_id', 'attribute_id')
            )
            ->joinLeft(
                array('store' => $this->getTable(array('score/category', $type))),
            ...
            ->where('def.store_id IN (?)', array(Shaurmalab_Score_Model_Abstract::DEFAULT_STORE_ID, $storeId));
     ...
     *
     * @param Shaurmalab_Score_Model_Category|int $category
    ...
    public function synchronize($category = null, $storeIds = array())
    {
        ...
        } else if ($category instanceof Shaurmalab_Score_Model_Category) {
    ...
    public function move(array $affectedCategoryIds)
    {
        $write  = $this->_getWriteAdapter();
        $select = $write->select()
            ->from($this->getTable('score/category'), array('entity_id', 'path'))
    ...
    {
        $scoreCategoryTable = $this->getTable('score/category');
        $_staticFields = array(
            'parent_id',
            'path',
            'level',
        ...
        $prevParent = Mage::getModel('score/category')->load($prevParentId);
        $parent = Mage::getModel('score/category')->load($parentId);
        if ($prevParent->getStore()->getWebsiteId() != $parent->getStore()->getWebsiteId()) {
            ...
            $select = $this->_getReadAdapter()->select()
                ->from($scoreCategoryTable, 'path')
                ->where('entity_id = ?', $categoryId);

            $categoryPath = $this->_getWriteAdapter()->fetchOne($select);

            $select = $this->_getWriteAdapter()->select()
                ->from($scoreCategoryTable, 'entity_id')
            ...
            foreach ($_categories as $_category) {
                foreach ($parent->getStoreIds() as $storeId) {
                    $_tmpCategory = Mage::getModel('score/category')
                        ->setStoreId($storeId)
            ...
            foreach ($parent->getStoreIds() as $store) {
                ...
                $update = "UPDATE {$mainStoreTable}, {$scoreCategoryTable} SET";
                foreach ($_staticFields as $field) {
                    $update .= " {$mainStoreTable}.".$field."={$scoreCategoryTable}.".$field.",";
                }
                $update = substr($update, 0, -1);
                $update .= " WHERE {$mainStoreTable}.entity_id = {$scoreCategoryTable}.entity_id AND " .
                    "($scoreCategoryTable}.path like '{$parent->getPath()}/%' OR " .
                    "{$scoreCategoryTable}.path like '{$prevParent->getPath()}/%')";
...
//        $this->_move($categoryId, $prevParentPath, $parentPath);
    ...
    protected function _prepareDataForAllFields($category, $replaceFields = array())
    {
        ...
        $data = array();
        $idFieldName = Mage::getSingleton('score/category')->getIdFieldName();
    ...
    public function getAttribute($attribute)
    {
        return Mage::getSingleton('score/config')
            ->getAttribute(Shaurmalab_Score_Model_Category::ENTITY, $attribute);
    }
    ...
    /**
     * Get oggettos count in category
     *
     * @param Shaurmalab_Score_Model_Category $category
     * @return integer
     */
    public function getOggettoCount($category)
    {
        $select =  $this->_getReadAdapter()->select()
            ->from(
                $this->getTable('score/category_oggetto'),
                "COUNT({$this->getTable('score/category_oggetto')}.oggetto_id)"
            )
            ->where("{$this->getTable('score/category_oggetto')}.category_id = ?", $category->getId())
            ->group("{$this->getTable('score/category_oggetto')}.category_id");
        return (int) $this->_getReadAdapter()->fetchOne($select);
    }
    ...
    public function getParentCategories($category, $isActive = true)
    {
        ...
        foreach ($result as $row) {
            ...
            $categories[$row['entity_id']] = Mage::getModel('score/category')->setData($row);
        }
        return $categories;
    }
    ...
    public function getParentDesignCategory($category)
    {
        ...
        return Mage::getModel('score/category')->setData($result);
    }
    ...
    public function getStoresRootCategories($storeIds = null)
    {
        if (is_null($this->_storesRootCategories)) {
            $select = $this->_getWriteAdapter()->select()
                ...
                ->where('cs.store_id <> ?', Shaurmalab_Score_Model_Abstract::DEFAULT_STORE_ID);
     ...
     * Check if Score Category Flat Data has been initialized
     *
     * @deprecated use Shaurmalab_Score_Model_Resource_Category_Flat::isBuilt() instead
     *
     * @return bool
     */
