 ...
 */
class Shaurmalab_Score_Model_Resource_Category_Tree extends Varien_Data_Tree_Dbp
{
    const ID_FIELD    = 'id';
     ...
     *
     * @var Shaurmalab_Score_Model_Resource_Category_Collection
     */
    ...
    public function __construct()
    {
        ...
        parent::__construct(
            ...
            $resource->getTableName('score/category'),
            array(
     ...
     * @return Shaurmalab_Score_Model_Resource_Category_Tree
     */
     ...
     *
     * @param Shaurmalab_Score_Model_Resource_Category_Collection $collection
    ...
    protected function _getIsActiveAttributeId()
    {
        ...
        if (is_null($this->_isActiveAttributeId)) {
            $bind = array(
                'entity_type_code' => Shaurmalab_Score_Model_Category::ENTITY,
    ...
    protected function _getInactiveItemIds($collection, $storeId)
    {
        ...
        $table = Mage::getSingleton('core/resource')->getTableName(array('score/category', 'int'));
        $bind = array(
    ...
    protected function _getDefaultCollection($sorted = false)
    {
        ...
        /** @var $collection Shaurmalab_Score_Model_Resource_Category_Collection */
    ...
    {
        Mage::dispatchEvent('score_category_tree_move_before', array(
            'category'      => $category,
    ...
    public function move($category, $newParent, $prevNode = null)
    {
        ...
        Mage::getResourceSingleton('score/category')->move($category->getId(), $newParent->getId());
    ...
    {
        Mage::app()->cleanCache(array(Shaurmalab_Score_Model_Category::CACHE_TAG));

        Mage::dispatchEvent('score_category_tree_move_after', array(
            'category'  => $category,
            'prev_node' => $prevNode,
            'parent'    => $newParent
        ));
     ...
     * @param bool $updateAnchorOggettoCount
     * @return Shaurmalab_Score_Model_Resource_Category_Tree
     */
    public function loadByIds($ids, $addCollectionData = true, $updateAnchorOggettoCount = true)
    {
        ...
        if ($updateAnchorOggettoCount) {
            $this->_updateAnchorOggettoCount($arrNodes);
        }
        $childrenItems = array();
    ...
    public function loadBreadcrumbsArray($path, $addCollectionData = true, $withRootNode = false)
    {
        ...
        if (!empty($pathIds)) {
            ...
            $this->_updateAnchorOggettoCount($result);
        }
        return $result;
    }

    /**
     * Replace oggettos count with self oggettos count, if category is non-anchor
     *
     * @param array $data
     */
    protected function _updateAnchorOggettoCount(&$data)
    {
        foreach ($data as $key => $row) {
            if (0 === (int)$row['is_anchor']) {
                $data[$key]['oggetto_count'] = $row['self_oggetto_count'];
            }
        }
    }
     ...
     * Also the correct oggetto_count is selected, depending on is the category anchor or not.
     *
     * @param bool $sorted
    ...
    protected function _createCollectionDataSelect($sorted = true, $optionalAttributes = array())
    {
        ...
        foreach ($attributes as $attributeCode) {
            ...
            $attribute = Mage::getResourceSingleton('score/category')->getAttribute($attributeCode);
        ...
        // count children oggettos qty plus self oggettos qty
        $categoriesTable         = Mage::getSingleton('core/resource')->getTableName('score/category');
        $categoriesOggettosTable = Mage::getSingleton('core/resource')->getTableName('score/category_oggetto');
        ...
        $subSelect = $this->_conn->select()
            ...
            ->joinLeft(
                array('scp' => $categoriesOggettosTable),
                'see.entity_id=scp.category_id',
                array('COUNT(DISTINCT scp.oggetto_id)'))
            ->where('see.entity_id = e.entity_id')
            ->orWhere('see.path LIKE ?', $subConcat);
        $select->columns(array('oggetto_count' => $subSelect));

        $subSelect = $this->_conn->select()
            ->from(array('cp' => $categoriesOggettosTable), 'COUNT(cp.oggetto_id)')
            ->where('cp.category_id = e.entity_id');

        $select->columns(array('self_oggetto_count' => $subSelect));
