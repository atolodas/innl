...
/**
 * Score Layer Price Filter resource model
 *
 * @category    Mage
 * @package     Shaurmalab_Score
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Shaurmalab_Score_Model_Resource_Layer_Filter_Price extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
    ...
    protected function _replaceTableAlias($conditionString)
    {
        ...
        $oldAlias = array(
            Shaurmalab_Score_Model_Resource_Oggetto_Collection::INDEX_TABLE_ALIAS . '.',
            $adapter->quoteIdentifier(Shaurmalab_Score_Model_Resource_Oggetto_Collection::INDEX_TABLE_ALIAS) . '.',
        );
        $newAlias = array(
            Shaurmalab_Score_Model_Resource_Oggetto_Collection::MAIN_TABLE_ALIAS . '.',
            $adapter->quoteIdentifier(Shaurmalab_Score_Model_Resource_Oggetto_Collection::MAIN_TABLE_ALIAS) . '.',
        );
     ...
     *
     * @param Shaurmalab_Score_Model_Layer_Filter_Price $filter
    ...
    protected function _getSelect($filter)
    {
        ...
        if (!is_null($collection->getScorePreparedSelect())) {
            $select = clone $collection->getScorePreparedSelect();
        } else {
        ...
        if (!isset($fromPart[Shaurmalab_Score_Model_Resource_Oggetto_Collection::INDEX_TABLE_ALIAS])
            || !isset($fromPart[Shaurmalab_Score_Model_Resource_Oggetto_Collection::MAIN_TABLE_ALIAS])
        ) {
            return $select;
        }

        // processing FROM part
        $priceIndexJoinPart = $fromPart[Shaurmalab_Score_Model_Resource_Oggetto_Collection::INDEX_TABLE_ALIAS];
        ...
        $fromPart[Shaurmalab_Score_Model_Resource_Oggetto_Collection::MAIN_TABLE_ALIAS] = $priceIndexJoinPart;
        unset($fromPart[Shaurmalab_Score_Model_Resource_Oggetto_Collection::INDEX_TABLE_ALIAS]);
        ...
        $excludeJoinPart = Shaurmalab_Score_Model_Resource_Oggetto_Collection::MAIN_TABLE_ALIAS . '.entity_id';
    ...
    protected function _dispatchPreparePriceEvent($filter, $select)
    {
         ...
         */
        Mage::dispatchEvent('scoreindex_prepare_price_select', $eventArgs);
    ...
    {
        return $filter->getLayer()->getOggettoCollection()->getMaxPrice();
    }

    /**
     * Price expression generated by oggettos collection
     *
     * @param Shaurmalab_Score_Model_Layer_Filter_Price $filter
     * @param Varien_Db_Select $select
    ...
    {
        $priceExpression = $filter->getLayer()->getOggettoCollection()->getPriceExpression($select);
        $additionalPriceExpression = $filter->getLayer()->getOggettoCollection()->getAdditionalPriceExpression($select);
    ...
    {
        $currencyRate = $filter->getLayer()->getOggettoCollection()->getCurrencyRate();
        if ($decrease) {
    ...
    /**
     * Get full price expression generated by oggettos collection
     *
     * @param Shaurmalab_Score_Model_Layer_Filter_Price $filter
     * @param Varien_Db_Select $select
     * @return Zend_Db_Expr
    ...
    protected function _getFullPriceExpression($filter, $select)
    {
        return new Zend_Db_Expr('ROUND((' . $this->_getPriceExpression($filter, $select) . ') * '
            . $filter->getLayer()->getOggettoCollection()->getCurrencyRate() . ', 2)');
    }

    /**
     * Retrieve array with oggettos counts per price range
     *
     * @param Shaurmalab_Score_Model_Layer_Filter_Price $filter
     * @param int $range
     * @return array
     ...
     * @return Shaurmalab_Score_Model_Resource_Layer_Filter_Price
     */
    public function applyFilterToCollection($filter, $range, $index)
    {
        $select = $filter->getLayer()->getOggettoCollection()->getSelect();
        $priceExpr = $this->_getPriceExpression($filter, $select);
        $filter->getLayer()->getOggettoCollection()
            ->getSelect()
    ...
    /**
     * Load range of oggetto prices, preceding the price
     *
     * @param Shaurmalab_Score_Model_Layer_Filter_Price $filter
     * @param float $price
     * @param int $index
    ...
    /**
     * Load range of oggetto prices, next to the price
     *
     * @param Shaurmalab_Score_Model_Layer_Filter_Price $filter
     * @param float $price
    ...
    /**
     * Apply price range filter to oggetto collection
     *
     * @param Shaurmalab_Score_Model_Layer_Filter_Price $filter
     * @return Shaurmalab_Score_Model_Resource_Layer_Filter_Price
     */
