...
/**
 * Score oggetto media gallery attribute backend resource
 *
 * @category    Mage
 * @package     Shaurmalab_Score
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Shaurmalab_Score_Model_Resource_Oggetto_Attribute_Backend_Media extends Mage_Core_Model_Resource_Db_Abstract
{
    const GALLERY_TABLE       = 'score/oggetto_attribute_media_gallery';
    const GALLERY_VALUE_TABLE = 'score/oggetto_attribute_media_gallery_value';
    const GALLERY_IMAGE_TABLE = 'score/oggetto_attribute_media_gallery_image';
    ...
    /**
     * Load gallery images for oggetto
     *
     * @param Shaurmalab_Score_Model_Oggetto $oggetto
     * @param Shaurmalab_Score_Model_Oggetto_Attribute_Backend_Media $object
     * @return array
     */
    public function loadGallery($oggetto, $object)
    {
        $adapter = $this->_getReadAdapter();

        $positionCheckSql = $adapter->getCheckSql('value.position IS NULL', 'default_value.position', 'value.position');

        // Select gallery images for oggetto
        $select = $adapter->select()
            ...
            ->joinLeft(
                ...
                $adapter->quoteInto('main.value_id = value.value_id AND value.store_id = ?', (int)$oggetto->getStoreId()),
            ...
            ->where('main.entity_id = ?', $oggetto->getId())
     ...
     *
     * @param array $result
     * @return Shaurmalab_Score_Model_Resource_Oggetto_Attribute_Backend_Media
     */
     ...
     * @param int $originalOggettoId
     * @param int $newOggettoId
     * @return Shaurmalab_Score_Model_Resource_Oggetto_Attribute_Backend_Media
     */
    public function duplicate($object, $newFiles, $originalOggettoId, $newOggettoId)
    {
        $select = $this->_getReadAdapter()->select()
            ->from($this->getMainTable(), array('value_id', 'value'))
            ->where('attribute_id = ?', $object->getAttribute()->getId())
            ->where('entity_id = ?', $originalOggettoId);
