...
/**
 * Score Oggetto Indexer Abstract Resource Model
 *
 * @category    Mage
 * @package     Shaurmalab_Score
 * @author      Magento Core Team <core@magentocommerce.com>
 */
abstract class Shaurmalab_Score_Model_Resource_Oggetto_Indexer_Abstract extends Mage_Index_Model_Resource_Abstract
{
    /**
     * Retrieve score_oggetto attribute instance by attribute code
     *
    ...
    {
        return Mage::getSingleton('eav/config')->getAttribute(Shaurmalab_Score_Model_Oggetto::ENTITY, $attributeCode);
    }
     ...
     * @return Shaurmalab_Score_Model_Resource_Oggetto_Indexer_Abstract
     */
    ...
    /**
     * Add join for score/oggetto_website table
     * Joined table has alias pw
     *
     * @param Varien_Db_Select $select          the select object
     * @param string|Zend_Db_Expr $website      the limitation of website_id
     * @param string|Zend_Db_Expr $oggetto      the limitation of oggetto_id
     * @return Shaurmalab_Score_Model_Resource_Oggetto_Indexer_Abstract
     */
    protected function _addOggettoWebsiteJoinToSelect($select, $website, $oggetto)
    {
        $select->join(
            array('pw' => $this->getTable('score/oggetto_website')),
            "pw.oggetto_id = {$oggetto} AND pw.website_id = {$website}",
            array()
        );
    ...
    /**
     * Retrieve oggetto relations by children
     *
    ...
    public function getRelationsByChild($childIds)
    {
        ...
        $select = $write->select()
            ->from($this->getTable('score/oggetto_relation'), 'parent_id')
    ...
    /**
     * Retrieve oggetto relations by parents
     *
    ...
    public function getRelationsByParent($parentIds)
    {
        ...
        if (!empty($parentIds)) {
            $write = $this->_getWriteAdapter();
            $select = $write->select()
                ->from($this->getTable('score/oggetto_relation'), 'child_id')
