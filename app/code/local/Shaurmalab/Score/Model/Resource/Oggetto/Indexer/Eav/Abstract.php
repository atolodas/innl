...
/**
 * Score Oggetto Eav Attributes abstract indexer resource model
 *
 * @category    Mage
 * @package     Shaurmalab_Score
 * @author      Magento Core Team <core@magentocommerce.com>
 */
abstract class Shaurmalab_Score_Model_Resource_Oggetto_Indexer_Eav_Abstract
    extends Shaurmalab_Score_Model_Resource_Oggetto_Indexer_Abstract
{
    ...
    /**
     * Remove Not Visible oggettos from temporary data index
     *
     * @return Shaurmalab_Score_Model_Resource_Oggetto_Indexer_Eav_Abstract
     */
    protected function _removeNotVisibleEntityFromIndex()
    {
        ...
        $condition = $write->quoteInto('=?',Shaurmalab_Score_Model_Oggetto_Visibility::VISIBILITY_NOT_VISIBLE);
    ...
    /**
     * Prepare data index for oggetto relations
     *
    ...
    protected function _prepareRelationIndex($parentIds = null)
    {
        ...
        $select = $write->select()
            ->from(array('l' => $this->getTable('score/oggetto_relation')), 'parent_id')
            ->join(
     ...
     * the score/eav_attribute table must have alias is ca
     *
     * @return string
     */
