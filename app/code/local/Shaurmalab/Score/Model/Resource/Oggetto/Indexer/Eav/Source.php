...
/**
 * Score Oggetto Eav Select and Multiply Select Attributes Indexer resource model
 *
 * @category    Mage
 * @package     Shaurmalab_Score
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Shaurmalab_Score_Model_Resource_Oggetto_Indexer_Eav_Source
    extends Shaurmalab_Score_Model_Resource_Oggetto_Indexer_Eav_Abstract
{
    /**
    ...
    protected function _getIndexableAttributes($multiSelect)
    {
        $select = $this->_getReadAdapter()->select()
            ->from(array('ca' => $this->getTable('score/eav_attribute')), 'attribute_id')
            ->join(
     ...
     * @return Shaurmalab_Score_Model_Resource_Oggetto_Indexer_Eav_Source
     */
    ...
    protected function _prepareSelectIndex($entityIds = null, $attributeId = null)
    {
        ...
        $subSelect = $adapter->select()
            ...
            ->joinLeft(
                array('d' => $this->getValueTable('score/oggetto', 'int')),
        ...
        $select = $adapter->select()
            ...
            ->joinLeft(
                array('pis' => $this->getValueTable('score/oggetto', 'int')),
        ...
        $select->where(Mage::getResourceHelper('score')->getIsNullNotNullCondition('pis.value', 'pid.value'));
    ...
    protected function _prepareMultiselectIndex($entityIds = null, $attributeId = null)
    {
        ...
        $oggettoValueExpression = $adapter->getCheckSql('pvs.value_id > 0', 'pvs.value', 'pvd.value');
        $select = $adapter->select()
            ->from(
                array('pvd' => $this->getValueTable('score/oggetto', 'varchar')),
            ...
            ->join(
                ...
                array('store_id'))
            ->joinLeft(
                array('pvs' => $this->getValueTable('score/oggetto', 'varchar')),
                'pvs.entity_id = pvd.entity_id AND pvs.attribute_id = pvd.attribute_id'
                    . ' AND pvs.store_id=cs.store_id',
                array('value' => $oggettoValueExpression))
            ->where('pvd.store_id=?', Shaurmalab_Score_Model_Abstract::DEFAULT_STORE_ID)
            ->where('cs.store_id!=?', Shaurmalab_Score_Model_Abstract::DEFAULT_STORE_ID)
            ->where('pvd.attribute_id IN(?)', $attrIds);

        $statusCond = $adapter->quoteInto('=?', Shaurmalab_Score_Model_Oggetto_Status::STATUS_ENABLED);
