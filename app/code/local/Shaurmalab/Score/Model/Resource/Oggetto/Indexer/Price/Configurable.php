 ...
 */
class Shaurmalab_Score_Model_Resource_Oggetto_Indexer_Price_Configurable
    extends Shaurmalab_Score_Model_Resource_Oggetto_Indexer_Price_Default
{
     ...
     *
     * @return Shaurmalab_Score_Model_Resource_Oggetto_Indexer_Price_Configurable
     */
    ...
    {
        if ($this->useIdxTable()) {
            return $this->getTable('score/oggetto_price_indexer_cfg_option_aggregate_idx');
        }
        return $this->getTable('score/oggetto_price_indexer_cfg_option_aggregate_tmp');
    }
    ...
    {
        if ($this->useIdxTable()) {
            return $this->getTable('score/oggetto_price_indexer_cfg_option_idx');
        }
        return $this->getTable('score/oggetto_price_indexer_cfg_option_tmp');
    }
    ...
    /**
     * Calculate minimal and maximal prices for configurable oggetto options
    ...
    protected function _applyConfigurableOption()
    {
        ...
        $select = $write->select()
            ...
            ->join(
                array('l' => $this->getTable('score/oggetto_super_link')),
                'l.parent_id = i.entity_id',
                array('parent_id', 'oggetto_id'))
            ->columns(array('customer_group_id', 'website_id'), 'i')
            ->join(
                array('a' => $this->getTable('score/oggetto_super_attribute')),
                'l.parent_id = a.oggetto_id',
                array())
            ->join(
                array('cp' => $this->getValueTable('score/oggetto', 'int')),
                'l.oggetto_id = cp.entity_id AND cp.attribute_id = a.attribute_id AND cp.store_id = 0',
                array())
            ->joinLeft(
                array('apd' => $this->getTable('score/oggetto_super_attribute_pricing')),
                'a.oggetto_super_attribute_id = apd.oggetto_super_attribute_id'
                    . ' AND apd.website_id = 0 AND cp.value = apd.value_index',
                array())
            ->joinLeft(
                array('apw' => $this->getTable('score/oggetto_super_attribute_pricing')),
                'a.oggetto_super_attribute_id = apw.oggetto_super_attribute_id'
                    . ' AND apw.website_id = i.website_id AND cp.value = apw.value_index',
                array())
            ->join(
                array('le' => $this->getTable('score/oggetto')),
                'le.entity_id = l.oggetto_id',
                array())

            ->where('le.required_options=0')
            ->group(array('l.parent_id', 'i.customer_group_id', 'i.website_id', 'l.oggetto_id'));
