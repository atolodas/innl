 ...
 * For correctly work need define oggetto type id
 *
 * @category    Mage
 * @package     Shaurmalab_Score
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Shaurmalab_Score_Model_Resource_Oggetto_Indexer_Price_Default
    extends Shaurmalab_Score_Model_Resource_Oggetto_Indexer_Abstract
    implements Shaurmalab_Score_Model_Resource_Oggetto_Indexer_Price_Interface
{
    /**
     * Oggetto type code
     *
    ...
    /**
     * Oggetto Type is composite flag
     *
     * @var bool
     */
    ...
    /**
     * Set Oggetto Type code
     *
     * @param string $typeCode
     * @return Shaurmalab_Score_Model_Resource_Oggetto_Indexer_Price_Default
     */
    ...
    public function getTypeId()
    {
        if (is_null($this->_typeId)) {
            Mage::throwException(Mage::helper('score')->__('A oggetto type is not defined for the indexer.'));
        }
        return $this->_typeId;
    }

    /**
     * Set Oggetto Type Composite flag
     *
     * @param bool $flag
     * @return Shaurmalab_Score_Model_Resource_Oggetto_Indexer_Price_Default
     */
    ...
    /**
     * Check oggetto type is composite
     *
     * @return bool
     */
    ...
    {
        if ($this->useIdxTable()) {
            return $this->getTable('score/oggetto_price_indexer_final_idx');
        }
        return $this->getTable('score/oggetto_price_indexer_final_tmp');
    }
    ...
    /**
     * Prepare oggettos default final price in temporary index table
     *
    ...
    protected function _prepareFinalPriceData($entityIds = null)
    {
        ...
        $select = $write->select()
            ...
            ->join(
                ...
                'pw.oggetto_id = e.entity_id AND pw.website_id = cw.website_id',
                array())
            ->joinLeft(
        ...
        // add enable oggettos limitation
        $statusCond = $write->quoteInto('=?', Shaurmalab_Score_Model_Oggetto_Status::STATUS_ENABLED);
        ...
        $select = $write->select()
            ->join(array('wd' => $this->_getWebsiteDateTable()),
                ...
                array());
        Mage::dispatchEvent('prepare_score_oggetto_price_index_table', array(
    ...
    {
        if ($this->useIdxTable()) {
            return $this->getTable('score/oggetto_price_indexer_option_aggregate_idx');
        }
        return $this->getTable('score/oggetto_price_indexer_option_aggregate_tmp');
    }
    ...
    {
        if ($this->useIdxTable()) {
            return $this->getTable('score/oggetto_price_indexer_option_idx');
        }
        return $this->getTable('score/oggetto_price_indexer_option_tmp');
    }
    ...
    protected function _applyCustomOption()
    {
        ...
        $select = $write->select()
            ...
            ->join(
                ...
                array())
            ->join(
                array('o' => $this->getTable('score/oggetto_option')),
                'o.oggetto_id = i.entity_id',
                array('option_id'))
            ->join(
                array('ot' => $this->getTable('score/oggetto_option_type_value')),
                'ot.option_id = o.option_id',
                array())
            ->join(
                array('otpd' => $this->getTable('score/oggetto_option_type_price')),
                'otpd.option_type_id = ot.option_type_id AND otpd.store_id = 0',
                array())
            ->joinLeft(
                array('otps' => $this->getTable('score/oggetto_option_type_price')),
        ...
        $select = $write->select()
            ...
            ->join(
                ...
                array('option_id'))
            ->join(
                array('opd' => $this->getTable('score/oggetto_option_price')),
                'opd.option_id = o.option_id AND opd.store_id = 0',
                array())
            ->joinLeft(
                array('ops' => $this->getTable('score/oggetto_option_price')),
