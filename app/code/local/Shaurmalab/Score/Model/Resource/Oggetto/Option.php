...
/**
 * Score oggetto custom option resource model
 *
 * @category    Mage
 * @package     Shaurmalab_Score
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Shaurmalab_Score_Model_Resource_Oggetto_Option extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
    ...
    {
        $this->_init('score/oggetto_option', 'option_id');
    }
     ...
     *
     * @param Mage_Core_Model_Abstract $object
     * @return Shaurmalab_Score_Model_Resource_Oggetto_Option
     */
    protected function _saveValuePrices(Mage_Core_Model_Abstract $object)
    {
        $priceTable   = $this->getTable('score/oggetto_option_price');
        ...
        if ($object->getType() == Shaurmalab_Score_Model_Oggetto_Option::OPTION_TYPE_FIELD
            || $object->getType() == Shaurmalab_Score_Model_Oggetto_Option::OPTION_TYPE_AREA
            || $object->getType() == Shaurmalab_Score_Model_Oggetto_Option::OPTION_TYPE_FILE
            || $object->getType() == Shaurmalab_Score_Model_Oggetto_Option::OPTION_TYPE_DATE
            || $object->getType() == Shaurmalab_Score_Model_Oggetto_Option::OPTION_TYPE_DATE_TIME
            || $object->getType() == Shaurmalab_Score_Model_Oggetto_Option::OPTION_TYPE_TIME
        ) {
            //save for store_id = 0
    ...
    protected function _saveValueTitles(Mage_Core_Model_Abstract $object)
    {
        $readAdapter  = $this->_getReadAdapter();
        $writeAdapter = $this->_getWriteAdapter();
        $titleTable = $this->getTable('score/oggetto_option_title');
    ...
    public function deletePrices($optionId)
    {
        $this->_getWriteAdapter()->delete(
            $this->getTable('score/oggetto_option_price'),
            array(
                'option_id = ?' => $optionId
            )
        );
    ...
    {
        $this->_getWriteAdapter()->delete(
            $this->getTable('score/oggetto_option_title'),
            array(
                'option_id = ?' => $optionId
            )
        );
    ...
    /**
     * Duplicate custom options for oggetto
     *
     * @param Shaurmalab_Score_Model_Oggetto_Option $object
     * @param int $oldOggettoId
     * @param int $newOggettoId
     * @return Shaurmalab_Score_Model_Oggetto_Option
     */
    public function duplicate(Shaurmalab_Score_Model_Oggetto_Option $object, $oldOggettoId, $newOggettoId)
    {
        ...
        // read and prepare original oggetto options
        $select = $read->select()
            ->from($this->getTable('score/oggetto_option'))
            ->where('oggetto_id = ?', $oldOggettoId);

        $query = $read->query($select);

        while ($row = $query->fetch()) {
            $optionsData[$row['option_id']] = $row;
            $optionsData[$row['option_id']]['oggetto_id'] = $newOggettoId;
            unset($optionsData[$row['option_id']]['option_id']);
        }

        // insert options to duplicated oggetto
        ...
        foreach ($optionsCond as $oldOptionId => $newOptionId) {
            // title
            $table = $this->getTable('score/oggetto_option_title');
            ...
            // price
            $table = $this->getTable('score/oggetto_option_price');
    ...
    public function getSearchableData($oggettoId, $storeId)
    {
        ...
        $defaultOptionJoin = implode(
            ' AND ',
            array('option_title_default.option_id=oggetto_option.option_id',
            $adapter->quoteInto('option_title_default.store_id = ?', Shaurmalab_Score_Model_Abstract::DEFAULT_STORE_ID))
        );

        $storeOptionJoin = implode(
            ' AND ',
            array(
                'option_title_store.option_id=oggetto_option.option_id',
                $adapter->quoteInto('option_title_store.store_id = ?', (int) $storeId))
        );

        $select = $adapter->select()
            ->from(array('oggetto_option' => $this->getMainTable()), null)
            ->join(
                array('option_title_default' => $this->getTable('score/oggetto_option_title')),
                $defaultOptionJoin,
                array()
            )
            ->joinLeft(
                array('option_title_store' => $this->getTable('score/oggetto_option_title')),
                $storeOptionJoin,
                array('title' => $titleCheckSql)
            )
            ->where('oggetto_option.oggetto_id = ?', $oggettoId);
        ...
        $select = $adapter->select()
            ->from(array('oggetto_option' => $this->getMainTable()), null)
            ->join(
                array('option_type' => $this->getTable('score/oggetto_option_type_value')),
                'option_type.option_id=oggetto_option.option_id',
                array()
            )
            ->join(
                array('option_title_default' => $this->getTable('score/oggetto_option_type_title')),
                $defaultOptionJoin,
                array()
            )
            ->joinLeft(
                array('option_title_store' => $this->getTable('score/oggetto_option_type_title')),
                $storeOptionJoin,
                array('title' => $titleCheckSql)
            )
            ->where('oggetto_option.oggetto_id = ?', $oggettoId);
