...
/**
 * Score oggetto options collection
 *
 * @category    Mage
 * @package     Shaurmalab_Score
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Shaurmalab_Score_Model_Resource_Oggetto_Option_Collection extends Mage_Core_Model_Resource_Db_Collection_Abstract
{
    /**
    ...
    {
        $oggettoOptionTitleTable = $this->getTable('score/oggetto_option_title');
        ...
        $this->getSelect()
            ->join(array('default_option_title' => $oggettoOptionTitleTable),
                'default_option_title.option_id = main_table.option_id',
                array('default_title' => 'title'))
            ->joinLeft(
                array('store_option_title' => $oggettoOptionTitleTable),
            ...
            ->where('default_option_title.store_id = ?', Shaurmalab_Score_Model_Abstract::DEFAULT_STORE_ID);
    ...
    {
        $oggettoOptionPriceTable = $this->getTable('score/oggetto_option_price');
        $adapter        = $this->getConnection();
        ...
        $this->getSelect()
            ->joinLeft(
                array('default_option_price' => $oggettoOptionPriceTable),
                'default_option_price.option_id = main_table.option_id AND '
                    . $adapter->quoteInto(
                        'default_option_price.store_id = ?',
                        Shaurmalab_Score_Model_Abstract::DEFAULT_STORE_ID
                    ),
                array(
                    'default_price' => 'price',
            ...
            ->joinLeft(
                array('store_option_price' => $oggettoOptionPriceTable),
    ...
    public function addValuesToResult($storeId = null)
    {
        ...
        if (!empty($optionIds)) {
            /** @var $values Shaurmalab_Score_Model_Option_Value_Collection */
            $values = Mage::getModel('score/oggetto_option_value')
                ->getCollection()
                ->addTitleToResult($storeId)
    ...
    /**
     * Add oggetto_id filter to select
     *
     * @param array|Shaurmalab_Score_Model_Oggetto|int $oggetto
     * @return Shaurmalab_Score_Model_Resource_Oggetto_Option_Collection
     */
    public function addOggettoToFilter($oggetto)
    {
        if (empty($oggetto)) {
            $this->addFieldToFilter('oggetto_id', '');
        } elseif (is_array($oggetto)) {
            $this->addFieldToFilter('oggetto_id', array('in' => $oggetto));
        } elseif ($oggetto instanceof Shaurmalab_Score_Model_Oggetto) {
            $this->addFieldToFilter('oggetto_id', $oggetto->getId());
        } else {
            $this->addFieldToFilter('oggetto_id', $oggetto);
        }
