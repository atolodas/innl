 ...
 */
class Shaurmalab_Score_Model_Url
{
    /**
     ...
     * Resource model
     *
     * @var Shaurmalab_Score_Model_Resource_Eav_Mysql4_Url
     */
    protected $_resourceModel;

    /**
     * Categories cache for oggettos
     *
     * @var array
     */
    ...
    /**
     * Flag to overwrite config settings for Score URL rewrites history maintainance
     *
     * @var bool
     */
     ...
     *
     * @var Shaurmalab_Score_Model_Category
     */
    ...
    protected function _addCategoryUrlPath($category)
    {
        ...
        if (self::$_categoryForUrlPath === null) {
            self::$_categoryForUrlPath = Mage::getModel('score/category');
        }
    ...
    public function getResource()
    {
        if (is_null($this->_resourceModel)) {
            $this->_resourceModel = Mage::getResourceModel('score/url');
        }
    ...
    {
        return $this->getResource()->getOggettoModel();
    }
     ...
     *
     * @param bool $flag
     * @return Shaurmalab_Score_Model_Url
     */
    ...
    public function getShouldSaveRewritesHistory($storeId = null)
    {
        ...
        return Mage::helper('score')->shouldSaveUrlRewritesHistory($storeId);
    }
    ...
    public function refreshRewrites($storeId = null)
    {
        ...
        $this->refreshOggettoRewrites($storeId);
        $this->getResource()->clearCategoryOggetto($storeId);
     ...
     * @param bool $refreshOggettos
     * @return Shaurmalab_Score_Model_Url
     */
    protected function _refreshCategoryRewrites(Varien_Object $category, $parentPath = null, $refreshOggettos = true)
    {
        ...
        if ($refreshOggettos) {
            $this->_refreshCategoryOggettoRewrites($category);
        }

        foreach ($category->getChilds() as $child) {
            $this->_refreshCategoryRewrites($child, $category->getUrlPath() . '/', $refreshOggettos);
        }
    ...
    /**
     * Refresh oggetto rewrite
     *
     * @param Varien_Object $oggetto
     * @param Varien_Object $category
     * @return Shaurmalab_Score_Model_Url
     */
    protected function _refreshOggettoRewrite(Varien_Object $oggetto)
    {
        $category = Mage::getModel('score/category')->load(1);
        
        if($oggetto->getName() || $oggetto->getUrlKey()) {
            if ($oggetto->getUrlKey() == '') {
                $urlKey = $this->getOggettoModel()->formatUrlKey($oggetto->getName());
            }
            else {
                $urlKey = $this->getOggettoModel()->formatUrlKey($oggetto->getUrlKey());
            }
        }

        $idPath      = $this->generatePath('id', $oggetto, $category);
        $targetPath  = $this->generatePath('target', $oggetto, $category);
        $requestPath = $this->getOggettoRequestPath($oggetto, $category);
        ...
        if($oggetto->getName() || $oggetto->getUrlKey()) {
            ...
            if ($updateKeys && $oggetto->getUrlKey() != $urlKey && $urlKey) {
                $oggetto->setUrlKey($urlKey);
                $this->getResource()->saveOggettoAttribute($oggetto, 'url_key');
            }
            if ($updateKeys && $oggetto->getUrlPath() != $requestPath) {
                $oggetto->setUrlPath($requestPath);
                $this->getResource()->saveOggettoAttribute($oggetto, 'url_path');
            }
        }
    ...
    /**
     * Refresh oggettos for catwgory
     *
     * @param Varien_Object $category
     * @return Shaurmalab_Score_Model_Url
     */
    protected function _refreshCategoryOggettoRewrites(Varien_Object $category)
    {
        ...
        $firstIteration = true;
        while ($process == true) {
            $oggettos = $this->getResource()->getOggettosByCategory($category, $lastEntityId);
            if (!$oggettos) {
                if ($firstIteration) {
                    $this->getResource()->deleteCategoryOggettoStoreRewrites(
                        $category->getId(),
                        array(),
                        $category->getStoreId()
                    );
            ...
            foreach ($oggettos as $oggetto) {
                // Oggetto always must have rewrite in root category
                $this->_refreshOggettoRewrite($oggetto, $rootCategory);
                $this->_refreshOggettoRewrite($oggetto, $category);
            }
     ...
     * @return Shaurmalab_Score_Model_Url
     */
    public function refreshCategoryRewrite($categoryId, $storeId = null, $refreshOggettos = true)
    {
        if (is_null($storeId)) {
            foreach ($this->getStores() as $store) {
                $this->refreshCategoryRewrite($categoryId, $store->getId(), $refreshOggettos);
            }
            return $this;
        }
        ...
        $this->_refreshCategoryRewrites($category, null, $refreshOggettos);
    ...
    /**
     * Refresh oggetto rewrite urls for one store or all stores
     * Called as a reaction on oggetto change that affects rewrites
     *
     * @param int $oggettoId
     * @param int|null $storeId
     * @return Shaurmalab_Score_Model_Url
     */
    public function refreshOggettoRewrite($oggettoId, $storeId = null)
    {
        if (is_null($storeId)) {
            foreach ($this->getStores() as $store) {
                $this->refreshOggettoRewrite($oggettoId, $store->getId());
            }
            return $this;
        }

        $oggetto = $this->getResource()->getOggetto($oggettoId, $storeId);
        if ($oggetto) {
            $store = $this->getStores($storeId);
            $storeRootCategoryId = 1;

            // List of categories the oggetto is assigned to, filtered by being within the store's categories root
            $categories = $this->getResource()->getCategories(array(1), $storeId); // $oggetto->getCategoryIds()
            $this->_rewrites = $this->getResource()->prepareRewrites($storeId, '', $oggettoId);

            // Add rewrites for all needed categories
            // If oggetto is assigned to any of store's categories -
            // we also should use store root category to create root oggetto url rewrite
            ...
            // Create oggetto url rewrites
            foreach ($categories as $category) {
                $this->_refreshOggettoRewrite($oggetto, $category);
            }

            // Remove all other oggetto rewrites created earlier for this store - they're invalid now
            $excludeCategoryIds = array_keys($categories);
            $this->getResource()->clearOggettoRewrites($oggettoId, $storeId, $excludeCategoryIds);

            unset($categories);
            unset($oggetto);
        } else {
            // Oggetto doesn't belong to this store - clear all its url rewrites including root one
            $this->getResource()->clearOggettoRewrites($oggettoId, $storeId, array());
        }
    ...
    /**
     * Refresh all oggetto rewrites for designated store
     *
     * @param int $storeId
     * @return Shaurmalab_Score_Model_Url
     */
    public function refreshOggettoRewrites($storeId)
    {
        ...
        while ($process == true) {
            $oggettos = $this->getResource()->getOggettosByStore($storeId, $lastEntityId);
            if (!$oggettos) {
                $process = false;
                break;
            }
            $this->_rewrites = $this->getResource()->prepareRewrites($storeId, false, array_keys($oggettos));

            $loadCategories = array();
            foreach ($oggettos as $oggetto) {
            	if(!$oggetto->getCategoryIds()) $oggetto->setCategoryIds(array(1)); // force assign oggetto to category, but not saving.
                foreach ($oggetto->getCategoryIds() as $categoryId) {
            ...
            foreach ($oggettos as $oggetto) {
                $this->_refreshOggettoRewrite($oggetto, $this->_categories[$storeRootCategoryId]);
                foreach ($oggetto->getCategoryIds() as $categoryId) {
                    if ($categoryId != $storeRootCategoryId && isset($this->_categories[$categoryId])) {
                        if (strpos($this->_categories[$categoryId]['path'], $storeRootCategoryPath . '/') !== 0) {
                            continue;
                        }
                        $this->_refreshOggettoRewrite($oggetto, $this->_categories[$categoryId]);
                    }
                }
            }
    ...
    {
        if (strpos($idPath, 'oggetto') !== false) {
            $suffix = $this->getOggettoUrlSuffix($storeId);
        } else {
     ...
     * @return string
     */
    public function getOggettoUrlSuffix($storeId)
    {
        return Mage::helper('score/oggetto')->getOggettoUrlSuffix($storeId);
    }
    ...
    {
        return Mage::helper('score/category')->getCategoryUrlSuffix($storeId);
    }
    ...
    public function getCategoryRequestPath($category, $parentPath)
    {
        ...
        $parentPath = Mage::helper('score/category')->getCategoryUrlPath($parentPath,
    ...
    /**
     * Get unique oggetto request path
     *
     * @param   Varien_Object $oggetto
     * @param   Varien_Object $category
     * @return  string
     */
    public function getOggettoRequestPath($oggetto, $category)
    {
        if ($oggetto->getUrlKey() == '') {
            $urlKey = $this->getOggettoModel()->formatUrlKey($oggetto->getName());
        } else {
        ...
        $idPath  = $this->generatePath('id', $oggetto, $category);
        /**
         * Prepare oggetto base request path
         */
        if ($category->getLevel() > 1) {
            // To ensure, that category has path either from attribute or generated now
            $this->_addCategoryUrlPath($category);
            $categoryUrl = Mage::helper('score/category')->getCategoryUrlPath($category->getUrlPath(),
                false, $storeId);
        ...
        if (isset($this->_rewrites[$idPath])) {
             ...
             */
            if ($oggetto->getUrlKey() == '' && !empty($requestPath)
        ...
        /**
         * Check 2 variants: $requestPath and $requestPath . '-' . $oggettoId
         */
        $validatedPath = $this->getResource()->checkRequestPaths(
            array($requestPath.$suffix, $requestPath.'-'.$oggetto->getId().$suffix),
            $storeId
        );
    ...
    /**
     * Generate either id path, request path or target path for oggetto and/or category
     *
     * For generating id or system path, either oggetto or category is required
     ...
     */
    public function generatePath($type = 'target', $oggetto = null, $category = null, $parentPath = null)
    {
        if (!$oggetto && !$category) {
            Mage::throwException(Mage::helper('core')->__('Please specify either a category or a oggetto, or both.'));
        }
        ...
        if ('id' === $type) {
            ...
            if ($category && $category->getLevel() > 1) {
                return 'oggetto/' . $oggetto->getId() . '/' . $category->getId();
            }
            return 'oggetto/' . $oggetto->getId();
        }
        ...
        if ('request' === $type) {
            ...
            // for oggetto & category
            if (!$category) {
                Mage::throwException(Mage::helper('core')->__('A category object is required for determining the oggetto request path.')); // why?
            }
            ...
            $oggettoUrlSuffix  = $this->getOggettoUrlSuffix($category->getStoreId());
            if ($category->getLevel() > 1) {
                // To ensure, that category has url path either from attribute or generated now
                $this->_addCategoryUrlPath($category);
                $categoryUrl = Mage::helper('score/category')->getCategoryUrlPath($category->getUrlPath(),
                    false, $category->getStoreId());
                return $this->getUnusedPath($category->getStoreId(), $categoryUrl . '/' . $urlKey . $oggettoUrlSuffix,
                    $this->generatePath('id', $oggetto, $category)
                );
            }

            // for oggetto only
            return $this->getUnusedPath($category->getStoreId(), $urlKey . $oggettoUrlSuffix,
                $this->generatePath('id', $oggetto)
            );
        }

        // generate target_path
        if (!$oggetto) {
            return 'score/category/view/id/' . $category->getId();
        }
        if ($category && $category->getLevel() > 1) {
            return 'score/oggetto/view/id/' . $oggetto->getId() . '/category/' . $category->getId();
        }
        return 'score/oggetto/view/id/' . $oggetto->getId();
    }
    ...
    /**
     * Create Custom URL Rewrite for old oggetto/category URL after url_key changed
