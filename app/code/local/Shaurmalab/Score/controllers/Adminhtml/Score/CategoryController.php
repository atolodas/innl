...
/**
 * Score category controller
 *
 * @category   Mage
 * @package    Score_Adminhtml
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Shaurmalab_Score_Adminhtml_Score_CategoryController extends Mage_Adminhtml_Controller_Action
{
    /**
...
//            $this->_initLayoutMessages('adminhtml/session');
        ...
        $this->loadLayout();
        $this->_setActiveMenu('score/categories');
        $this->getLayout()->getBlock('head')->setCanLoadExtJs(true)
            ->setContainerCssClass('catalog-categories');

        $this->_addBreadcrumb(Mage::helper('score')->__('Manage Score Categories'),
             Mage::helper('score')->__('Manage Categories')
        );
    ...
    public function categoriesJsonAction()
    {
        ...
        if ($categoryId = (int) $this->getRequest()->getPost('id')) {
            ...
            $this->getResponse()->setBody(
                $this->getLayout()->createBlock('score/adminhtml_score_category_tree')
    ...
    public function saveAction()
    {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if (!$category->getId()) {
                ...
                if (!$parentId) {
                    if ($storeId) {
                        ...
                        $parentId = Shaurmalab_Score_Model_Category::TREE_ROOT_ID;
                    }
                }
                $parentCategory = Mage::getModel('score/category')->load($parentId);
            ...
            if (isset($data['category_entitys']) &&
                !$category->getEntitysReadonly()) {
                $entitys = array();
                parse_str($data['category_entitys'], $entitys);
                $category->setPostedEntitys($entitys);
            }

            Mage::dispatchEvent('score_category_prepare_save', array(
                'category' => $category,
            ...
            try {
                ...
                if ($validate !== true) {
                    foreach ($validate as $code => $error) {
                        if ($error === true) {
                            Mage::throwException(Mage::helper('score')->__('Attribute "%s" is required.', $category->getResource()->getAttribute($code)->getFrontend()->getLabel()));
                        }
                        else {
                ...
                $category->save();
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('score')->__('The category has been saved.'));
                $refreshTree = 'true';
            }
            catch (Exception $e){
    ...
    public function moveAction()
    {
        ...
        if (!$category) {
            $this->getResponse()->setBody(Mage::helper('score')->__('Category move error'));
            return;
        }
        /**
        ...
        $category->setData('save_rewrites_history', Mage::helper('score')->shouldSaveUrlRewritesHistory());
        try {
            ...
            $this->getResponse()->setBody(Mage::helper('score')->__('Category move error %s', $e));
            Mage::logException($e);
        }
    ...
    public function deleteAction()
    {
        if ($id = (int) $this->getRequest()->getParam('id')) {
            try {
                $category = Mage::getModel('score/category')->load($id);
                Mage::dispatchEvent('score_controller_category_delete', array('category'=>$category));

                Mage::getSingleton('admin/session')->setDeletedPath($category->getPath());

                $category->delete();
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('score')->__('The category has been deleted.'));
            }
            catch (Mage_Core_Exception $e){
                ...
                Mage::getSingleton('adminhtml/session')->addError(Mage::helper('score')->__('An error occurred while trying to delete the category.'));
                $this->getResponse()->setRedirect($this->getUrl('*/*/edit', array('_current'=>true)));
                return;
            }
        }
    ...
    /**
     * Grid Action
     * Display list of entitys related to current category
     *
     * @return void
     */
    public function gridAction()
    {
        ...
        $this->getResponse()->setBody(
            $this->getLayout()->createBlock('score/adminhtml_score_category_tab_entity', 'category.entity.grid')
                ->toHtml()
        );
    }
    ...
    public function treeAction()
    {
        ...
        $block = $this->getLayout()->createBlock('score/adminhtml_score_category_tree');
        $root  = $block->getRoot();
    ...
    */
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('score/categories');
    }
}
