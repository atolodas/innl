...
$conn->addColumn($installer->getTable('core_url_rewrite'), 'oggetto_id', 'int unsigned NULL AFTER `category_id`');
$installer->run("
...
UPDATE `{$installer->getTable('core_url_rewrite')}`
    SET `oggetto_id`=SUBSTRING_INDEX(SUBSTR(`id_path` FROM 9),'/',1)
    WHERE `id_path` RLIKE 'oggetto/[0-9]+$';
UPDATE `{$installer->getTable('core_url_rewrite')}`
    SET `category_id`=SUBSTRING_INDEX(SUBSTR(`id_path` FROM 9),'/',-1),
    `oggetto_id`=SUBSTRING_INDEX(SUBSTR(`id_path` FROM 9),'/',1)
    WHERE `id_path` LIKE 'oggetto/%/%';
...
REPLACE INTO `{$installer->getTable('core_url_rewrite_temporary')}` (`url_rewrite_id`)
    SELECT `ur`.`url_rewrite_id` FROM `{$installer->getTable('core_url_rewrite')}` as `ur`
            LEFT JOIN `{$installer->getTable('score_category_entity')}` as `cc` ON `ur`.`category_id`=`cc`.`entity_id`
        WHERE `ur`.`category_id` IS NOT NULL
            AND `cc`.`entity_id` IS NULL;
REPLACE INTO `{$installer->getTable('core_url_rewrite_temporary')}` (`url_rewrite_id`)
    SELECT `ur`.`url_rewrite_id` FROM `{$installer->getTable('core_url_rewrite')}` as `ur`
        LEFT JOIN `{$installer->getTable('score_oggetto_entity')}` as `cp` ON `ur`.`oggetto_id`=`cp`.`entity_id`
        WHERE `ur`.`oggetto_id` IS NOT NULL
...
$conn->addConstraint(
    'FK_CORE_URL_REWRITE_OGGETTO', $installer->getTable('core_url_rewrite'), 'oggetto_id',
