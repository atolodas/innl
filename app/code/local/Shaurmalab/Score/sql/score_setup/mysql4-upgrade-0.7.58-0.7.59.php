...
$installer->run("
DROP TABLE IF EXISTS `{$installer->getTable('score/category_flat')}`;
CREATE TABLE `{$installer->getTable('score/category_flat')}` (
    ...
    KEY `path` (`path`),
    KEY `IDX_LEVEL` (`level`),
    CONSTRAINT `FK_CATEGORY_FLAT_CATEGORY_ID` FOREIGN KEY (`entity_id`) REFERENCES `{$installer->getTable('score/category')}` (`entity_id`) ON DELETE CASCADE ON UPDATE CASCADE,
