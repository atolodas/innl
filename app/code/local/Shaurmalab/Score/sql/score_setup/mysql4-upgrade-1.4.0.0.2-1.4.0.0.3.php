...
$sql    = $installer->getConnection()->quoteInto("SELECT * FROM `{$installer->getTable('score/eav_attribute')}` WHERE attribute_id IN (?)", $attributes);
...
foreach ($data as $row) {
    ...
    $installer->run("UPDATE `{$installer->getTable('score/eav_attribute')}`
