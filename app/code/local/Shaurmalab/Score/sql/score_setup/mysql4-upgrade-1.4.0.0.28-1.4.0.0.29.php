...
$scoreCategoryEntityIntTable = $installer->getAttributeTable($entityTypeId, $designApplyAttributeId);
...
$installer->addAttribute($entityTypeId, 'custom_apply_to_oggettos', array(
    'type'          => 'int',
    'input'         => 'select',
...
$applyToOggettosAttributeId = $installer->getAttributeId($entityTypeId, 'custom_apply_to_oggettos');
...
$attributeIdExpr = new Zend_Db_Expr(
    ...
    $applyToOggettosAttributeId .')');
$oggettoValueExpr = new Zend_Db_Expr('IF (e.value IN (1,3), 1, 0)');
$valueExpr = new Zend_Db_Expr('IF (e_a.attribute_id = e.attribute_id, 1, '. $oggettoValueExpr .')');
$select = $installer->getConnection()->select()
    ->from(
        array('e' => $scoreCategoryEntityIntTable),
        array(
            'entity_type_id',
...
$sqlQuery = $select->insertFromSelect($scoreCategoryEntityIntTable, $insertArray, false);
