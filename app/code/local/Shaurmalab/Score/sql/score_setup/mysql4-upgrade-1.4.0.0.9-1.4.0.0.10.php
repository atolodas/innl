...
$table = $this->getTable('score/category_oggetto_index');

/**
 * Remove data duplicates
 */
$stmt = $installer->getConnection()->query(
    'SELECT * FROM ' . $table . ' GROUP BY category_id, oggetto_id, store_id HAVING count(*)>1'
);

while ($row = $stmt->fetch()) {
    $condition = 'category_id=' . $row['category_id']
        . ' AND oggetto_id=' . $row['oggetto_id']
