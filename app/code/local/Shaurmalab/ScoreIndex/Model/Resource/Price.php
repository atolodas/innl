 ...
 */
class Shaurmalab_ScoreIndex_Model_Resource_Price extends Shaurmalab_ScoreIndex_Model_Resource_Abstract
{
    /**
    ...
    {
        $this->_init('scoreindex/price', 'index_id');
    }
     ...
     * @param unknown_type $index
     * @param unknown_type $tableName
     * @return Shaurmalab_ScoreIndex_Model_Resource_Price
     */
    public function applyFilterToCollection($collection, $attribute, $range, $index, $tableName = 'price_table')
    {
        /**
         * Distinct required for removing duplicates in case when we have grouped oggettos
         * which contain multiple rows for one oggetto id
         */
    ...
    public function getMinimalPrices($ids)
    {
        ...
        $select = $this->_getReadAdapter()->select();
        $select->from(array('price_table'=>$this->getTable('scoreindex/minimal_price')),
