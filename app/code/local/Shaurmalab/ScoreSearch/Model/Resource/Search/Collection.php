 ...
 */
class Shaurmalab_ScoreSearch_Model_Resource_Search_Collection extends Shaurmalab_Score_Model_Resource_Oggetto_Collection
{
    /**
    ...
    protected function _getAttributesCollection()
    {
        if (!$this->_attributesCollection) {
            $this->_attributesCollection = Mage::getResourceModel('score/oggetto_attribute_collection')
                ->load();
    ...
    protected function _getSearchInOptionSql($query)
    {
        ...
        // search in scoreindex for products as part of configurable/grouped/bundle products (current store)
        $where = array();
        foreach ($options as $option) {
        ...
        if ($where) {
            $selects[] = (string)$this->getConnection()->select()
                ->from($resource->getTableName('scoreindex/eav'), 'entity_id')
                ->where(implode(' OR ', $where))
