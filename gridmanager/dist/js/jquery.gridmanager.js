...
(function($  ){
    ...
    $.gridmanager = function(el, options){
        ...
        gm.$elem = $(el);
        gm.el = el;
        gm.$elem.data("gridmanager", gm);

         /**
         * API
         * @method appendHTMLSelectedCols
         * @param {string} htmlNew - htmlNew to append to selected columns
         * @returns null
         */
        gm.appendHTMLSelectedCols = function(htmlNew) {
          var canvas=gm.$elem.find("#" + gm.options.canvasId);
          var cols = canvas.find(gm.options.colSelector);
          $.each(cols, function(){
            if($(this).hasClass(gm.options.gmEditClassSelected)) {
              $('.'+gm.options.gmEditRegion, this).append(htmlNew);
            }
          });
        };
         /**
...
/*------------------------------------------ Canvas & Controls ---------------------------------------*/
        ...
        /**
         * Build and append the canvas, making sure existing htmlNew in the user's div is wrapped. Will also trigger Responsive classes to existing markup if specified
         * @method createCanvas
         * @returns null
         */
        gm.createCanvas = function(){
          gm.log("+ Create Canvas");
           var htmlNew=gm.$elem.html();
                gm.$elem.html("");
                $('<div/>', {'id': gm.options.canvasId, 'html':htmlNew }).appendTo(gm.$elem);
                // Add responsive classes after initial loading of htmlNew, otherwise we lose the rows
                if(gm.options.addResponsiveClasses) {
                   gm.addResponsiveness(gm.$elem.find("#" + gm.options.canvasId));
                }
                ...
                if(gm.options.autoEdit){
                   gm.initMarkup(
                   gm.$elem.find("#" + gm.options.canvasId)
        ...
        /**
         * Add missing reponsive classes to existing htmlNew
         * @method addResponsiveness
         * @param {} htmlNew
         * @returns CallExpression
         */
        gm.addResponsiveness = function(htmlNew) {
          if(htmlNew === '') { return; }
          var desktopRegex = gm.options.colDesktopClass+'(\\d+)',
              ...
              //new_html = '';
          return $(htmlNew).find(':regex(class,'+desktopRegex+'|'+tabletRegex+'|'+phoneRegex+')').each(function(i, el) {
        ...
        */
        ...
        gm.switchLayoutMode = function(mode) {
          var canvas=gm.$elem.find("#" + gm.options.canvasId), temp_html = canvas.html(), regex1 = '', regex2 = '', uimode = '';
        ...
        gm.initControls = function(){
          var canvas=gm.$elem.find("#" + gm.options.canvasId);
           ...
           gm.$elem.on("click", ".gm-preview", function(){
               if(gm.status){
                gm.deinitCanvas();
        ...
        /*
          ...
          @elem - element to clear htmlNew comments on
      ...
      /**
        ...
        /*
          Run (if set) any custom init/deinit filters on the gridmanager canvas
            @canvasElem - canvas wrapper container with the entire layout htmlNew
...
/*------------------------------------------ Editable Regions ---------------------------------------*/
        ...
        gm.initNewContentElem = function(newElem) {
          ...
          if(typeof newElem === 'undefined') {
            parentCols = gm.$elem.find('.'+gm.options.colClass);
          } else {
        ...
        gm.editableAreaFilter = function(canvasElem, isInit) {
          if(isInit) {
            ...
            htmlNew = htmlNew.replace(regex, rep);
            $(canvasElem).html(htmlNew);
...
/*------------------------------------------ BTNs ---------------------------------------*/
        ...
        /**
         * Returns htmlNew string of buttons
        ...
        gm.generateClickHandler= function(colWidths){
          ...
          var canvas=gm.$elem.find("#" + gm.options.canvasId);
              gm.$elem.on("click", string, function(e){
...
/*------------------------------------------ Useful functions ---------------------------------------*/
        ...
        gm.cleanup =  function(){
          ...
          var canvas,
              ...
              // cache canvas
              canvas = gm.$elem.find("#" + gm.options.canvasId);
  ...
  */
        ...
        controlAppend: "<div class='btn-group pull-right'><button title='Preview' type='button' class='btn btn-xs btn-primary gm-preview'><span class='fa fa-eye'></span></button></div>",
