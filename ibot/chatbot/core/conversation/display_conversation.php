  ...
  **/
  function get_json($convoArr, $conversation)
  {
    ...
    foreach ($conversation as $index => $conversation_subarray)
    {
      ...
      $show_json['usersay'] = strip_tags($_POST['say']); //stripslashes($conversation_subarray['input']);
      $show_json['botsay'] = stripslashes($conversation_subarray['response']);
      $show_json['bot_name'] = ucwords($convoArr['bot_properties']['name']);
       $show_json['timestamp'] = ucwords($conversation_subarray['userdate']);
      $i++;
    }
