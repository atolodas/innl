...
(function($){
	...
	var Redactor = function(element, options)
	{
		...
		this.opts = jQuery.extend({
			...
			buttons: ['html', '|', 'formatting', '|', 'bold', 'italic', 'deleted', '|', 'unorderedlist', 'orderedlist', 'outdent', 'indent', '|',
					'image', 'video', 'file', 'widgets', 'table', 'link', '|',
			...
			modal_image: String() + 
				...
				'<form id="redactorInsertImageForm" method="post" action="" enctype="multipart/form-data">' +
					'<div id="redactor_tab1" class="redactor_tab">' +
						'<input type="file" id="redactor_file" name="file" style="display:none" />' +
					'</div>' +
			...
			modal_video: String() + 
				...
				'<div id="redactor_modal_footer">' +
					...
					'</span>' +
				'</div>',
			modal_widgets: String() + 
					'<div class="col-md-4 tab" style="max-width: 500px; overflow: scroll;">'+widgets_list+'</div>' +
					'<div class="col-md-8">'+
					'<div id="widgetData"></div>'+
					'<textarea id="redactor_insert_widget_area" readonly style="display: none; width: 80%; height: 160px;"></textarea>' +
					'</div>' +
					'<div id="redactor_modal_footer" class="col-md-12">' +
						'<span class="redactor_btns_box">' +
							'<input type="button" name="upload" id="redactor_insert_widget_btn" value="' + RLANG.insert + '" />' +
							'<a href="javascript:void(null);" id="redactor_btn_modal_close">' + RLANG.cancel + '</a>' +
						'</span>' +
					'</div>',
	...
	Redactor.prototype = {
		...
		init: function()
		{
			...
			{
				jQuery.extend(this.opts.toolbar, this.opts.buttonsCustom);
			
				jQuery.each(this.opts.buttonsAdd, jQuery.proxy(function(i,s)
				{
			...
			{
				jQuery(this.doc).bind('paste', jQuery.proxy(function(e)
				{ 
					setTimeout(jQuery.proxy(function ()
					{
						var marker = Math.floor(Math.random() * 99999);
						var marker_text = '';
						//if (jQuery.browser.mozilla) marker_text = '&nbsp;';
						var node = jQuery('<span rel="pastemarkerend" id="pastemarkerend' + marker + '">' + marker_text + '</span>');
			...
			// keyup
			jQuery(this.doc).keyup(jQuery.proxy(function(e)
			{
			...
			{
				this.observeScroll();
				jQuery(document).scroll(jQuery.proxy(this.observeScroll, this));
			}
		...
		shortcuts: function()
		{
			jQuery(this.doc).keydown(jQuery.proxy(function(e)
			{
				var key = e.keyCode || e.which;
		...
		getPath: function()
		{
			...
			jQuery('script').each(jQuery.proxy(function(i,s)
			{
				if (s.src)
				{
		...
		{
			// container
			this.$box = jQuery('<div class="redactor_box"></div>');
	
			// frame
			this.$frame = jQuery('<iframe frameborder="0" scrolling="auto" style="height: ' + this.height + ';" class="redactor_frame"></iframe>');
		...
		enable: function(html)
		{
			...
			if (this.doc !== null)
			{
				...
				// }
				return jQuery(this.doc).find('#page');
			}
			else
			{
				return false;
		...
		setDoc: function(html)
		{
			...
			jscss + 
			// '<link href="/gridmanager/dist/css/jquery.gridmanager-light.css?nocache=2" rel="stylesheet">'+
			// '<script src="/gridmanager/demo/js/jquery.js"></script>'+
   //  		'<script src="/gridmanager/demo/js/jquery-ui.js"></script>'+
			// '<script src="/gridmanager/demo/js/jquery-ui.js"></script>'+
			// '<script src="/gridmanager/dist/js/jquery.gridmanager.js?nocache=2"></script>'+
			// '<script>'+
		...
		{
			if (jQuery(this.doc).find('body').height() < jQuery(this.$frame.get(0).contentWindow).height())
			{		
				jQuery(this.doc).click(jQuery.proxy(function(e) { this.$editor.focus(); }, this));
			}
		},
		observeImages: function()
		{
			...
			jQuery(this.doc).find('img').each(jQuery.proxy(function(i,s)
			{
			//	if (jQuery.browser.msie) jQuery(s).attr('unselectable', 'on');
				this.resizeImage(s);
		...
		{
			jQuery(this.doc).find('body').find('table').click(jQuery.proxy(this.tableObserver, this));
		},
		...
		observeAutoResize: function()
		{
			...
			jQuery(this.doc).find('body').css({ 'overflow': 'hidden' });
			this.setAutoSize(false);
			jQuery(this.doc).keyup(jQuery.proxy(this.setAutoSize, this));
		},
		...
		execCommand: function(cmd, param)
		{
			if (this.opts.visual && this.doc)
			{
				try
				{
					...
					// }
					// else if (cmd === 'formatblock' && jQuery.browser.msie)
					// {
		...
		formatNewLine: function(e)
		{
			...
			if (parent.nodeName === 'DIV' && parent.id === 'page')
			{
				...
				var element = jQuery(this.getCurrentNode());
				if (element.get(0).tagName === 'DIV' && (element.html() === '' || element.html() === '<br>'))
				{
					var newElement = jQuery('<p>').append(element.clone().get(0).childNodes);
		...
		{
			var html = jQuery.trim(this.$editor.html());
			...
			if (html === '')
			{
				...
				var node = jQuery(nodehtml).get(0);
		...
		stripTags: function(html) 
		{
			...
			{
				if(jQuery.inArray($1.toLowerCase(), depr) > '-1') { 
					return '';
				} else { return $0; }
		...
		cleanStyles: function (properties, str, m)
		{
			...
			jQuery.each(prop, function(z,s)
			{
				...
				{
					if (jQuery.inArray(jQuery.trim(p[0]), properties) != '-1') t += s + '; ';
				}
			});
		...
		pasteCleanUp: function(marker)
		{
			...
			// jQuery.each(matches, jQuery.proxy(function(i,m)
			// {
			// 	var attr = m.split('=');
			// 	if (jQuery.inArray(attr[0], attributes) == '-1') 
			// 	{
			// 		//html = html.replace(m, '');
			// 	}
			...
			// RETURN FOCUS
			var node = jQuery(this.doc.body).find('#pastemarkerend' + marker).get(0);
			this.setFocusNode(node);
			...
			{
				// remove pastemarker
				jQuery(this.doc.body).find('span[rel=pastemarkerend]').remove();
		...
		formattingAddBefore: function(html)
		{
			...
			{
				var eee = btags[i];
				html = html.replace(new RegExp(eee,'gi'),eee);
			}
		...
		formattingAddAfter: function(html)
		{
			...
			{
				var aaa = atags[i];
				html = html.replace(new RegExp(aaa,'gi'),aaa);
			}
		...
		toggle: function()
		{
			...
			if (this.opts.visual)
			{
				...
				html = jQuery.trim(this.formatting(html));
		...
		autoSave: function()
		{
			...
			{
				jQuery.post(this.opts.autosave, { data: this.getCode() });
		...
		buildToolbar: function()
		{
			...
			this.$toolbar = jQuery('<ul>').addClass('redactor_toolbar');
			this.$box.prepend(this.$toolbar);
			
			jQuery.each(this.opts.buttons, jQuery.proxy(function(i,key)
			{
				...
				if (key !== '|' && typeof this.opts.toolbar[key] !== 'undefined') 
				{
					...
					if (key === 'fullscreen') 
					{
						jQuery(li).addClass('redactor_toolbar_right');
					}
					...
					{
						var dropdown = jQuery('<div class="redactor_dropdown" style="display: none;">');
						...
						this.dropdowns.push(dropdown.appendTo(jQuery(document.body)));
	
						// observing dropdown
						this.hdlHideDropDown = jQuery.proxy(function(e) { this.hideDropDown(e, dropdown, key); }, this);
						this.hdlShowDropDown = jQuery.proxy(function(e) { this.showDropDown(e, dropdown, key); }, this);
	
						a.click(this.hdlShowDropDown);
					}
					
					this.$toolbar.append(jQuery(li).append(a));
				}

				
				if (key === '|')
				{
					this.$toolbar.append(jQuery('<li class="redactor_separator"></li>'));
				}

			}, this));

			jQuery(document).click(this.hdlHideDropDown);
			jQuery(this.doc).click(this.hdlHideDropDown);

		},
		buildButton: function(key, s)
		{
			var button = jQuery('<a href="javascript:void(null);" title="' + s.title + '" class="redactor_btn_' + key + '"><span>&nbsp;</span></a>');
			
			if (typeof s.func === 'undefined')
			{
				button.click(jQuery.proxy(function() { this.execCommand(s.exec, key); }, this));
			}
			else if (s.func !== 'show')
			{
				button.click(jQuery.proxy(function(e) { this[s.func](e); }, this));
			}

			if (typeof s.callback !== 'undefined') 
			{
				button.click(jQuery.proxy(function(e) { s.callback(this, e, key); }, this));
			}
		...
		buildDropdown: function(dropdown, obj)
		{
			jQuery.each(obj, jQuery.proxy(
				function (x, d)
				{
					...
					{
						drop_a = jQuery('<a class="redactor_separator_drop">');
					}
					else
					{
						drop_a = jQuery('<a href="javascript:void(null);" class="' + d.className + '">' + d.title + '</a>');

						if (typeof(d.func) === 'undefined')
						{
							jQuery(drop_a).click(jQuery.proxy(function() { this.execCommand(d.exec, x); }, this));
						}
						else
						{
							jQuery(drop_a).click(jQuery.proxy(function(e) { this[d.func](e); }, this));
						}
					}

					jQuery(dropdown).append(drop_a);
		...
		buildColorPicker: function(dropdown, key)
		{
			...
			for (var i = 0; i < len; ++i)
			{
				...
				var swatch = jQuery('<a rel="' + color + '" href="javascript:void(null);" class="redactor_color_link"></a>').css({ 'backgroundColor': color });
				jQuery(dropdown).append(swatch);
				...
				{ 
					...
					// 		_self.execCommand(mode, jQuery(this).attr('rel'));
			...
			var elnone = jQuery('<a href="javascript:void(null);" class="redactor_color_none"></a>').html(RLANG.none);

			if (key === 'backcolor')
			{
				elnone.click(jQuery.proxy(this.setBackgroundNone, this));
			}
			else
			{
				elnone.click(jQuery.proxy(this.setColorNone, this));
			}

			jQuery(dropdown).append(elnone);

			return dropdown;
		},
		setBackgroundNone: function()
		{
			jQuery(this.getParentNode()).css('background-color', 'transparent');
			this.syncCode();
		},
		setColorNone: function()
		{
			jQuery(this.getParentNode()).attr('color', '').css('color', '');
			this.syncCode();
		},
		...
		showDropDown: function(e, dropdown, key)
		{
			...
			{	
				...
				{
					jQuery(dropdown).css({ position: 'fixed', left: left + 'px', top: '30px' }).show();
				}
				else 
				{
					var top = this.$toolbar.offset().top + 30;
					jQuery(dropdown).css({ position: 'absolute', left: left + 'px', top: top + 'px' }).show();
				}
			}
		...
		hideAllDropDown: function()
		{
			...
			jQuery('.redactor_dropdown').hide();
		},
		hideDropDown: function(e, dropdown, key)
		{
			if (!jQuery(e.target).parent().hasClass('dropact'))
			{
				jQuery(dropdown).removeClass('act');
		...
		getBtn: function(key)
		{
			return jQuery(this.$toolbar.find('a.redactor_btn_' + key));
		},
		...
		{
			this.$toolbar.append(jQuery('<li>').append(this.buildButton(key, obj)));
		},
		...
		fullscreen: function()
		{
			...
			if (this.opts.fullscreen === false)
			{
				...
				jQuery(document.body).prepend(this.$box).css('overflow', 'hidden');
	
				this.$editor = this.enable(html);
				
				jQuery(this.doc).click(jQuery.proxy(this.hideAllDropDown, this));
				...
				jQuery(window).resize(jQuery.proxy(this.fullScreenResize, this));
			...
			else
			{
				...
				jQuery(window).unbind('resize', jQuery.proxy(this.fullScreenResize, this));
		...
		fullScreenResize: function()
		{
			...
			var height = jQuery(window).height() - 42;
			this.$box.width(jQuery(window).width() - 2);
			this.$frame.height(height);
		...
		buildResizer: function()
		{
			...
			this.$resizer = jQuery('<div class="redactor_resizer">&mdash;</div>');
			this.$box.append(this.$resizer);	
			this.$resizer.mousedown(jQuery.proxy(this.initResize, this));
		...
		initResize: function(e)
		{
			...
			this.stopResizeHdl = jQuery.proxy(this.stopResize, this);
			this.startResizeHdl = jQuery.proxy(this.startResize, this);
			this.resizeHdl =  jQuery.proxy(this.resize, this);

			jQuery(document).mousedown(this.startResizeHdl);
			jQuery(document).mouseup(this.stopResizeHdl);
			jQuery(this.splitter).mouseup(this.stopResizeHdl);
		...
		{
			jQuery(document).mousemove(this.resizeHdl);
		},
		resize: function(e)
		{
			if (e.preventDefault)
		...
		{
			jQuery(document).unbind('mousemove', this.resizeHdl);
			jQuery(document).unbind('mousedown', this.startResizeHdl);
			jQuery(document).unbind('mouseup', this.stopResizeHdl);
			jQuery(this.splitter).unbind('mouseup', this.stopResizeHdl);
		...
		resizeImage: function(resize)
		{
			...
			var start_x;
			var start_y;
			var ratio = jQuery(resize).width()/jQuery(resize).height();
			...
			jQuery(resize).hover(function() { jQuery(resize).css('cursor', 'nw-resize'); }, function() { jQuery(resize).css('cursor','default'); clicked=false; });

			jQuery(resize).mousedown(function(e)
			{
				if (e.preventDefault)
				{
					e.preventDefault();
				...
				start_x = Math.round(e.pageX - jQuery(resize).eq(0).offset().left);
				start_y = Math.round(e.pageY - jQuery(resize).eq(0).offset().top);
			});
			
			jQuery(resize).mouseup(jQuery.proxy(function(e)
			{
				clicked = false;
				this.syncCode();
				if (this.opts.autoresize === true) this.setAutoSize(false);
			...
			jQuery(resize).click(jQuery.proxy(function(e)
			{
				if (clicker)
				{
					this.imageEdit(e);
			...
			jQuery(resize).mousemove(function(e)
			{
				if (clicked)
				{
					clicker = false;
					...
					{
						jQuery(resize).width(new_w);
					}
					if (y === 1 || (typeof(y) === "number" && new_h < y && new_h > min_h))
					{
						jQuery(resize).height(new_h);
					}
		...
		showTable: function()
		{
			this.modalInit(RLANG.table, 'table', 300, jQuery.proxy(function()
				{				
					jQuery('#redactor_insert_table_btn').click(jQuery.proxy(this.insertTable, this));
				}, this),
				
				function()
				{
					jQuery('#redactor_table_rows').focus();
				}			
			);
		},
		insertTable: function()
		{
			var rows = jQuery('#redactor_table_rows').val();
			var columns = jQuery('#redactor_table_columns').val();
			
			var table_box = jQuery('<div></div>');
			
			var tableid = Math.floor(Math.random() * 99999);
			var table = jQuery('<table id="table' + tableid + '"><tbody></tbody></table>');
			...
			jQuery(table_box).append(table);
			var html = jQuery(table_box).html();
			...
			this.$table = jQuery(this.doc).find('body').find('#table' + tableid);
			this.$table.click(jQuery.proxy(this.tableObserver, this));
			
			if (this.opts.autoresize === true) this.setAutoSize(false);
		},
		tableObserver: function(e)
		{
			this.$table = jQuery(e.target).parents('table');
			...
			this.$tbody = jQuery(e.target).parents('tbody');
			this.$thead = jQuery(this.$table).find('thead');
	
			this.$current_td = jQuery(e.target);
			this.$current_td.addClass('current');
			
			this.$current_tr = jQuery(e.target).parents('tr');
		},
		deleteTable: function()
		{
			jQuery(this.$table).remove();
			this.$table = false;
			this.syncCode();
		},
		deleteRow: function()
		{
			jQuery(this.$current_tr).remove();
			this.syncCode();
		},
		deleteColumn: function()
		{
			var index = jQuery(this.$current_td).get(0).cellIndex;
		...
		addHead: function()
		{
			if (jQuery(this.$table).find('thead').size() !== 0)
			{
				this.deleteHead();
			}
			else
			{
				var tr = jQuery(this.$table).find('tr').first().clone();
				tr.find('td').html('&nbsp;');
				this.$thead = jQuery('<thead></thead>');
				this.$thead.append(tr);
				jQuery(this.$table).prepend(this.$thead);
				this.syncCode();
			}
		},
		deleteHead: function()
		{
			jQuery(this.$thead).remove();
			this.$thead = false;
			this.syncCode();
		},
		...
		{
			var new_tr = jQuery(this.$current_tr).clone();
			new_tr.find('td').html('&nbsp;');
			if (type === 'after')
			{
				jQuery(this.$current_tr).after(new_tr);
			}
			else
			{
				jQuery(this.$current_tr).before(new_tr);
			}
		...
		insertColumn: function(type)
		{
			...
			{
				var current = jQuery(s).find('td').eq(index);
				...
				if (type === 'after')
				{
					jQuery(current).after(td);
				}
				else
				{
					jQuery(current).before(td);
				}
		...
		{
			this.modalInit(RLANG.widgets, 'widgets', 600, jQuery.proxy(function()
				{
					jQuery('#redactor_insert_widget_btn').click(jQuery.proxy(this.insertWidget, this));
				}, this),
				
				function()
				{
					// CODE FOR INITIAL WIDGET WINDOW
				}
			);
		},
		// INSERT WIDGETS
		...
		{
			var data = jQuery('#redactor_insert_widget_area').val();
		...
		showVideo: function()
		{
			...
			this.modalInit(RLANG.video, 'video', 600, jQuery.proxy(function()
				{
					jQuery('#redactor_insert_video_btn').click(jQuery.proxy(this.insertVideo, this));
				}, this),
				
				function()
				{
					jQuery('#redactor_insert_video_area').focus();
				}
			);
		},
		insertVideo: function()
		{
			var data = jQuery('#redactor_insert_video_area').val();
			data = this.stripTags(data);

			// if (jQuery.browser.msie)
			// {
			// 	jQuery(this.doc.getElementById('span' + this.spanid)).after(data).remove();
			// 	this.syncCode();
			// }
			// else
			// {
		...
		imageEdit: function(e)
		{
			...
			{
				jQuery('#redactor_file_alt').val($el.attr('alt'));
				jQuery('#redactor_image_edit_src').attr('href', $el.attr('src'));
				jQuery('#redactor_form_image_align').val($el.css('float'));

				if (jQuery(parent).get(0).tagName === 'A')
				{
					jQuery('#redactor_file_link').val(jQuery(parent).attr('href'));
				}

				jQuery('#redactor_image_delete_btn').click(jQuery.proxy(function() { this.imageDelete($el); }, this));
				jQuery('#redactorSaveBtn').click(jQuery.proxy(function() { this.imageSave($el); }, this));
		...
		imageSave: function(el)
		{
			...
			jQuery(el).attr('alt', jQuery('#redactor_file_alt').val());
	
			var floating = jQuery('#redactor_form_image_align').val();
		
			if (floating === 'left')
			{
				jQuery(el).css({ 'float': 'left', margin: '0 10px 10px 0' });
			}
			else if (floating === 'right')
			{
				jQuery(el).css({ 'float': 'right', margin: '0 0 10px 10px' });
			}
			else
			{
				jQuery(el).css({ 'float': 'none', margin: '0' });
			}
			
			// as link
			var link = jQuery.trim(jQuery('#redactor_file_link').val());
			if (link !== '')
			{
				if (jQuery(parent).get(0).tagName !== 'A')
				{
					jQuery(el).replaceWith('<a href="' + link + '">' + this.outerHTML(el) + '</a>');
				}
				else
				{
					jQuery(parent).attr('href', link);
				}
			}
			else
			{
				if (jQuery(parent).get(0).tagName === 'A')
				{
					jQuery(parent).replaceWith(this.outerHTML(el));
				}
			}
		...
		showImage: function()
		{
			...
			var handler = jQuery.proxy(function()
			{
				...
				{
//					jQuery.getJSON(this.opts.imageGetJson, jQuery.proxy(function(data) {
						var data = this.opts.imageGetJson;
						jQuery.each(data, jQuery.proxy(function(key, val)
						{
							var img = jQuery('<img src="' + val.thumb + '" rel="' + val.image + '" style="width:100px" />');
							jQuery('#redactor_image_box').append(img);
							jQuery(img).click(jQuery.proxy(this.imageSetThumb, this));
				...
				{
					jQuery('#redactor_tabs a').eq(1).remove();
				}
				...
				if (this.opts.imageUpload !== false)
				{
					...
					{
						if (jQuery('#redactor_file').size() !== 0)
						{
							jQuery('#redactor_file').dragupload(
							{
								url: this.opts.imageUpload,
								success: jQuery.proxy(this.imageUploadCallback, this)
							});
						}
					}

					// ajax upload
					this.uploadInit('redactor_file', { auto: true, url: this.opts.imageUpload, success: jQuery.proxy(this.imageUploadCallback, this)  });
				}
				else
				{
					...
					if (this.opts.imageGetJson === false) 
					{
						jQuery('#redactor_tabs').remove();
						jQuery('#redactor_tab3').show();
					}
					else 
					{
						var tabs = jQuery('#redactor_tabs a');
						tabs.eq(0).remove();
						tabs.eq(1).addClass('redactor_tabs_act');
						jQuery('#redactor_tab2').show();
					}
				}

				jQuery('#redactor_upload_btn').click(jQuery.proxy(this.imageUploadCallbackLink, this));

			}, this);
			
			var endCallback = jQuery.proxy(function()
			{
				if (this.opts.imageUpload === false && this.opts.imageGetJson === false)
				{
					jQuery('#redactor_file_link').focus();
				}				
			}, this);
		...
		{
			this._imageSet('<img alt="" src="' + jQuery(e.target).attr('rel') + '" />', true);
		},
		imageUploadCallbackLink: function()
		{
			if (jQuery('#redactor_file_link').val() !== '')
			{
				var data = '<img src="' + jQuery('#redactor_file_link').val() + '" />';
		...
		_imageSet: function(json, link)
		{
			...
			// if (jQuery.browser.msie)
			// {
			// 	jQuery(this.doc.getElementById('span' + this.spanid)).after(html).remove();
			// 	this.syncCode();
			// }
			// else
			// {
			...
			jQuery(this.doc).find('img').load(jQuery.proxy(function()
			{
				this.setAutoSize(false); 
		...
		showLink: function()
		{
			var handler = jQuery.proxy(function()
			{
				...
				// 	{
				// 		this.insert_link_node = jQuery(parent);
				...
				jQuery('.redactor_link_text').val(text);
				...
				if (url.search('mailto:') === 0)
				{
					...
					jQuery('#redactor_tab_selected').val(2);
					jQuery('#redactor_link_mailto').val(url.replace('mailto:', ''));
				}
				else if (turl.search(/^#/gi) === 0)
				{
					this.setModalTab(3);	
					
					jQuery('#redactor_tab_selected').val(3);
					jQuery('#redactor_link_anchor').val(turl.replace(/^#/gi, ''));
				}
				else
				{
					jQuery('#redactor_link_url').val(url);
				}
				
				jQuery('#redactor_insert_link_btn').click(jQuery.proxy(this.insertLink, this));
			...
			{
				jQuery('#redactor_link_url').focus();
			};
		...
		insertLink: function()
		{
			var tab_selected = jQuery('#redactor_tab_selected').val();
			var link = '', text = '';
			
			if (tab_selected === '1') // url
			{
				link = jQuery('#redactor_link_url').val();
				text = jQuery('#redactor_link_url_text').val();
			}
			else if (tab_selected === '2') // mailto
			{
				link = 'mailto:' + jQuery('#redactor_link_mailto').val();
				text = jQuery('#redactor_link_mailto_text').val();
			}
			else if (tab_selected === '3') // anchor
			{
				link = '#' + jQuery('#redactor_link_anchor').val();
				text = jQuery('#redactor_link_anchor_text').val();
			}			

			this._insertLink('<a href="' + link + '">' +  text + '</a> ', jQuery.trim(text), link);
		...
		_insertLink: function(a, text, link)
		{
			if (text != '')
			{
				...
				{
					jQuery(this.insert_link_node).text(text);
					jQuery(this.insert_link_node).attr('href', link);
					this.syncCode();
				}
				else
				{
					// if (jQuery.browser.msie)
					// {
					// 	jQuery(this.doc.getElementById('span' + this.spanid)).replaceWith(a);
					// 	this.syncCode();
					// }
					// else
					// {
		...
		showFile: function()
		{
			...
			var handler = jQuery.proxy(function()
			{
				...
				jQuery('#redactor_filename').val(text);
				...
				this.uploadInit('redactor_file', { auto: true, url: this.opts.fileUpload, success: jQuery.proxy(function(data) {
		...
		fileUploadCallback: function(json)
		{
			...
			var text = jQuery('#redactor_filename').val();
			...
			// 	if (text !== '')
			// 	{
			// 		jQuery(this.doc.getElementById('span' + this.spanid)).replaceWith(link);
			// 	}
			// 	else
			// 	{
			// 		jQuery(this.doc.getElementById('span' + this.spanid)).after(link).remove();
			// 	}
		...
		{
			// modal overlay
			if (jQuery('#redactor_modal_overlay').size() === 0)
			{
				this.overlay = jQuery('<div id="redactor_modal_overlay" style="display: none;"></div>');
				jQuery('body').prepend(this.overlay);
			}

			if (this.opts.overlay)
			{
				jQuery('#redactor_modal_overlay').show();
				jQuery('#redactor_modal_overlay').click(jQuery.proxy(this.modalClose, this));
			}

			if (jQuery('#redactor_modal').size() === 0)
			{
				this.modal = jQuery('<div id="redactor_modal" style="display: none;"><div id="redactor_modal_close">&times;</div><div id="redactor_modal_header"></div><div id="redactor_modal_inner"></div></div>');
				jQuery('body').append(this.modal);
			}

			jQuery('#redactor_modal_close').click(jQuery.proxy(this.modalClose, this));

			this.hdlModalClose = jQuery.proxy(function(e) { if ( e.keyCode === 27) { this.modalClose(); } }, this);
			
			jQuery(document).keyup(this.hdlModalClose);
			jQuery(this.doc).keyup(this.hdlModalClose);

			jQuery('#redactor_modal_inner').html(this.opts['modal_' + url]);
			jQuery('#redactor_modal_header').html(title);
							
			// tabs
			if (jQuery('#redactor_tabs').size() !== 0)
			{
				var that = this;
				jQuery('#redactor_tabs a').each(function(i,s)
				{
					i++;
					jQuery(s).click(function()
					{
						jQuery('#redactor_tabs a').removeClass('redactor_tabs_act');
						...
						{
							var height = jQuery('#redactor_modal').outerHeight();
							jQuery('#redactor_modal').css('margin-top', '-' + (height+10)/2 + 'px');
						}
					});
				});
			}

			jQuery('#redactor_btn_modal_close').click(jQuery.proxy(this.modalClose, this));
			...
			if (this.isMobile() === false)
			{
				jQuery('#redactor_modal').css({ position: 'fixed', top: '50%', left: '50%', width: width + 'px', height: 'auto', marginTop: '-' + (height+10)/2 + 'px', marginLeft: '-' + (width+60)/2 + 'px' }).fadeIn('fast');
			}
			else
			{
				jQuery('#redactor_modal').css({ position: 'absolute', width: '96%', height: 'auto', top: '20px', left: '0', marginTop: '0px', marginLeft: '2%' }).show();
				this.scrolltop();				
			}
			...
			if (scroll === true)
			{
				jQuery('#redactor_image_box').height(300).css('overflow', 'auto');
			}
		...
		modalClose: function()
		{
			...
			jQuery('#redactor_modal_close').unbind('click', this.modalClose);
			jQuery('#redactor_modal').fadeOut('fast', jQuery.proxy(function()
			{
				jQuery('#redactor_modal_inner').html('');

				if (this.opts.overlay)
				{
					jQuery('#redactor_modal_overlay').hide();
					jQuery('#redactor_modal_overlay').unbind('click', this.modalClose);
				}
				
				jQuery(document).unbind('keyup', this.hdlModalClose);
				jQuery(this.doc).unbind('keyup', this.hdlModalClose);
		...
		uploadInit: function(element, options)
		{
			...
			jQuery.extend(this.uploadOptions, options);
	
			// Test input or form
			if (jQuery('#' + element).size() !== 0 && jQuery('#' + element).get(0).tagName === 'INPUT')
			{
				this.uploadOptions.input = jQuery('#' + element);
				this.element = jQuery('#' + element).get(0).form;
			}
			else
			{
			...
			this.element_action = jQuery(this.element).attr('action');
			
			// Auto or trigger
			if (this.uploadOptions.auto)
			{
				jQuery(this.uploadOptions.input).change(jQuery.proxy(function()
				{
			...
			{
				jQuery('#' + this.uploadOptions.trigger).click(jQuery.proxy(this.uploadSubmit, this));
			}
		},
		...
		uploadFrame : function()
		{
			...
			jQuery('#' + this.id).load(jQuery.proxy(this.uploadLoaded, this));
		...
		uploadForm : function(f, name)
		{
			if (this.uploadOptions.input)
			{
				...
				this.form = jQuery('<form  action="' + this.uploadOptions.url + '" method="POST" target="' + name + '" name="' + formId + '" id="' + formId + '" enctype="multipart/form-data"></form>');
				
				var oldElement = this.uploadOptions.input;
				var newElement = jQuery(oldElement).clone();
				jQuery(oldElement).attr('id', fileId);
				jQuery(oldElement).before(newElement);
				jQuery(oldElement).appendTo(this.form);
				jQuery(this.form).css('position', 'absolute');
				jQuery(this.form).css('top', '-2000px');
				jQuery(this.form).css('left', '-2000px');
				jQuery(this.form).appendTo('body');  
		...
		{
			return jQuery("<p>").append(jQuery(s).eq(0).clone()).html();
		},
	...
	// API
	jQuery.fn.getDoc = function() 
	{
		return jQuery(this.data('redactor').doc);
	};
	
	jQuery.fn.getFrame = function() 
	{
	...
	jQuery.fn.setCode = function(html)
	{
		this.data('redactor').setCode(html);
	};
	
	jQuery.fn.insertHtml = function(html)
	{
...
(function($){
	...
	// Initialization	
	jQuery.fn.dragupload = function(options)
	{		
		return this.each(function() {
	...
	Construct.prototype = {
		...
		{	
			...
			// {	
				...
				this.droparea = jQuery('<div class="redactor_droparea"></div>');
				this.dropareabox = jQuery('<div class="redactor_dropareabox">' + this.opts.text + '</div>');	
				this.dropalternative = '';//jQuery('<div class="redactor_dropalternative">' + this.opts.atext + '</div>')'';//;
				...
				// drag over
				this.dropareabox.bind('dragover', jQuery.proxy(function() { return this.ondrag(); }, this));
				
				// drag leave
				this.dropareabox.bind('dragleave', jQuery.proxy(function() { return this.ondragleave(); }, this));
		
				var uploadProgress = jQuery.proxy(function(e) 
				{ 
				...
				// drop
				this.dropareabox.get(0).ondrop = jQuery.proxy(function(event)
				{
...
function chooseWidget(formId) {
	var val = jQuery('#redactor_insert_widget_area').val();
	var replace = val;
	jQuery('#'+formId+' input, #'+formId+' select').each(
	    function(index){  
        	var input = jQuery(this);
        	replace = val.replace(new RegExp(input.attr('name')+"='.*'",'g'),input.attr('name')+"='"+input.val()+"'");
	    }
	);
	jQuery('#redactor_insert_widget_area').val(replace);
}