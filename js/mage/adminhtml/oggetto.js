...
Oggetto.Gallery = Class.create();
Oggetto.Gallery.prototype = {
    images : [],
    file2id : {
        'no_selection' :0
    },
    ...
    setOggettoImages : function(file) {
        $H(this.imageTypes)
                .each(
                        function(pair) {
...
Oggetto.Attributes = Class.create();
Oggetto.Attributes.prototype = {
    config : {},
    containerId :null,
    ...
    setConfig : function(config) {
        this.config = config;
        Oggetto.AttributesBridge.bind(this.getConfig().tab_id, this);
    },
    getConfig : function() {
        return this.config;
    },
...
Oggetto.Configurable = Class.create();
Oggetto.Configurable.prototype = {
    initialize : function(attributes, links, idPrefix, grid, readonly) {
        ...
        this.links = $H(links); // Associated Oggettos
        this.newOggettos = []; // For Oggetto that's created through Create
        ...
        this.grid.checkboxCheckCallback = this.registerOggetto.bind(this); // Associate/Unassociate
                                                                            // simple
                                                                            // Oggetto
    ...
    addNewOggetto : function(OggettoId, attributes) {
        if (this.checkAttributes(attributes)) {
            this.links.set(OggettoId, this.cloneAttributes(attributes));
        } else {
            this.newOggettos.push(OggettoId);
        }
    ...
    createEmptyOggetto : function() {
        this.createPopup(this.createEmptyUrl)
    },
    createNewOggetto : function() {
    ...
    registerOggetto : function(grid, element, checked) {
        if (checked) {
    ...
    updateOggetto : function(OggettoId, attributes) {
        var isAssociated = false;

        if (typeof this.links.get(OggettoId) != 'undefined') {
            isAssociated = true;
            this.links.unset(OggettoId);
        }
    ...
    updateGrid : function() {
        this.grid.reloadParams = {
            'Oggettos[]' :this.links.keys().size() ? this.links.keys() : [ 0 ],
            'new_Oggettos[]' :this.newOggettos
        };
    },
    ...
    quickCreateNewOggetto : function() {
        ...
        new Ajax.Request(this.createQuickUrl, {
            ...
            onComplete :this.quickCreateNewOggettoComplete.bind(this)
        });
    },
    quickCreateNewOggettoComplete : function(transport) {
        ...
        if (result.error) {
            if (result.error.fields) {
                ...
                $H(result.error.fields)
                        .each(
                                function(pair) {
                                    $('simple_Oggetto_' + pair.key).value = pair.value;
                                    $('simple_Oggetto_' + pair.key + '_autogenerate').checked = false;
                                    toggleValueElements(
                                            $('simple_Oggetto_' + pair.key + '_autogenerate'),
                                            $('simple_Oggetto_' + pair.key + '_autogenerate').parentNode);
                                    Validation.ajaxError(
                                            $('simple_Oggetto_' + pair.key),
        ...
        this.attributes.each( function(attribute) {
            if ($('simple_Oggetto_' + attribute.attribute_code)) {
                $('simple_Oggetto_' + attribute.attribute_code).value = '';
            }
        }.bind(this));

        this.links.set(result.Oggetto_id, result.attributes);
        this.updateGrid();
        this.updateValues();
        this.grid.reload();
    },
                ...
                .each( function(attribute) {
                    attributes
                            .push( {
                                ...
                                value_index :$('simple_Oggetto_' + attribute.attribute_code).value
                            });
                }.bind(this));
    ...
    showPricing : function(select, attributeCode) {
        ...
        select = $(select);
        if (select.value
                && !$('simple_Oggetto_' + attributeCode + '_pricing_container')) {
            Element
                    .insert(
                            select,
                            {
                                after :'<div class="left"></div> <div id="simple_Oggetto_' + attributeCode + '_pricing_container" class="left"></div>'
                            });
        ...
        var container = $('simple_Oggetto_' + attributeCode + '_pricing_container');
        ...
        if (select.value) {
            ...
            if (!value) {
                if (!container.down('.attribute-price')) {
                    ...
                    $('simple_Oggetto_' + attributeCode + '_pricing_value').value = null;
                    $('simple_Oggetto_' + attributeCode + '_pricing_type').value = null;
                }
                ...
                $('simple_Oggetto_' + attributeCode + '_pricing_value').value = value.pricing_value;
                $('simple_Oggetto_' + attributeCode + '_pricing_type').value = value.is_percent;
            } else {
                container.update('');
                $('simple_Oggetto_' + attributeCode + '_pricing_value').value = null;
                $('simple_Oggetto_' + attributeCode + '_pricing_type').value = null;
    ...
    updateSimplePricing : function(evt) {
        ...
        if (!element.priceField.value.blank()) {
            $('simple_Oggetto_' + element.attributeCode + '_pricing_value').value = element.priceField.value;
            $('simple_Oggetto_' + element.attributeCode + '_pricing_type').value = element.typeField.value;
        } else {
            $('simple_Oggetto_' + element.attributeCode + '_pricing_value').value = null;
            $('simple_Oggetto_' + element.attributeCode + '_pricing_type').value = null;
        }
    },
    updateSimpleForm : function() {
        this.attributes.each( function(attribute) {
            if ($('simple_Oggetto_' + attribute.attribute_code)) {
                this.showPricing(
                        $('simple_Oggetto_' + attribute.attribute_code),
                        attribute.attribute_code);
            }
        }.bind(this));
    },
    showNoticeMessage : function() {
        $('assign_Oggetto_warrning').show();
    }
}
